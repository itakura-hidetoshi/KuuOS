...
**KuuOS / 空OS**は、観測、信念、記憶、計画、判断、実行、学習、WORLD表現を、由来、文脈、履歴、所有者、有限の許可へ束縛して接続する公開研究アーキテクチャです。

KuuOS is a public, governance-gated, proof-facing, non-Markovian, Qi-process-aware architecture for relational AI systems.

目的は、候補、証拠、検証、権限、効果、数学的表現を同一視せず、縁起、二諦、中道、和、局所性、履歴、反証可能性を保持したまま連結することです。
...
この基準では、ゲージ場型自己組織化のv0.60からv0.69までが`main`へ統合されています。

open pull request上の実装は、検証結果の有無にかかわらず研究前線として分離します。
...
`run_kuuos_runtime_full_check_v0_55.py`は、後続層を含む累積検証入口として互換性を維持している名称です。
...
v0.69では、外部reviewの`APPROVE_EVIDENCE`がproduction application authorityを伴います。

ただし、そのreview関数はlive effectまたはstate writeを実行しません。

`REJECT_EVIDENCE`と`REQUEST_MORE_EVIDENCE`はapplication authorityを与えません。

decisionとpermissionの不一致はfail closedで拒否します。
...
KuuOSは文脈を普遍的な固定グラフへ縮約せず、局所チャート、局所切断、遷移、曲率、cocycle residue、holonomyとして扱います。
...
Horizon v0.12とContext Gauge Atlas v0.13は、複数時間幅、局所性、履歴依存性を保持します。

### Modular Evolution Mesh v0.1

Modular Evolution Meshは、固定された憲法境界の周囲へ型付きmoduleを登録し、能力依存を解決し、append-only ledgerで由来を保持します。

初期moduleはObserveOS、VerifyOS、MemoryOSです。
...
MemoryOSは、履歴、予測候補、矛盾残渣、blocker、解析的文脈を分離して保持します。
...
MemoryOSはraw evidence、truth promotion、PlanOS activation、ActOS invocation、WORLD updateを自動生成しません。
...
→ supplied ObserveOS commit receipt
→ supplied VerifyOS verification receipt
→ LearnOS future-only delta
→ WORLD v0.53 exact receipt-composition digest
...
### Module-Bundle Self-Organization v0.70

PR #860は、自己組織化をグラフ再配線ではなく、文脈代数上の加群、意味部分加群、authority filtration、End(M)値接続、その許容変形として再定式化します。
...
+ connection ∇
+ channel-preserving gauge group G
→ admissible deformation candidate
```

v0.70は現在open pull request上の研究前線であり、`main`の統合済み基準ではありません。

候補生成はlive effect、committed state mutation、MemoryOS overwrite、authority wideningを行いません。

### 再基底または統合判断を要する枝

| 系列 | Pull request | 現在の扱い |
|---|---:|---|
| CapabilityOS v0.60 | #832 | Qi-WORLD、複数WORLD、MemoryOSを能力候補へ統合する独立枝。現在の`main`との再基底とv0.70との責務整理が必要 |
| WORLD phase-transition declaration v0.60 | #825 | 四大相転移証人の独立枝。現行ゲージv0.60とのversion namespace衝突を解消して再評価する |
| Workflow consolidation | #835 | 15 workflowの統合枝。v0.60からv0.69追加後の参照を保持して再基底する |

`WORLD v0.60`と`KuuOS Gauge v0.60`は異なる系列です。

現行`main`へ統合されているv0.60は、Gauge-Field Self-Organization v0.60です。

## 4D Yang–Mills証明境界

4D Yang–Mills mass gapのcanonical proof repositoryは、[`itakura-hidetoshi/4d-mass-gap`](https://github.com/itakura-hidetoshi/4d-mass-gap)です。

KuuOSは証明状態、receipt、governance boundaryを参照できますが、KuuOS側のvalidatorまたはCI成功を最終定理権限へ昇格させません。

```text
KuuOS reference surface
...
docs/KUUOS_OS_ASSOCIATED_GAUGE_FIELDS_v0_61.md
docs/KUUOS_CONNECTION_IMPROVEMENT_CANDIDATE_v0_62.md
docs/KUUOS_CONNECTION_CANDIDATE_REVIEW_v0_63.md
docs/KUUOS_CONNECTION_EVALUATION_v0_64.md
docs/KUUOS_CONNECTION_STAGING_PACKAGE_v0_65.md
docs/KUUOS_CONNECTION_SHADOW_MATERIALIZATION_v0_66.md
docs/KUUOS_FINITE_GAUGE_VALIDATION_v0_67.md
docs/KUUOS_CONNECTION_EVIDENCE_v0_68.md
...
python3 scripts/check_kuuos_gauge_field_self_organization_v0_60.py
python3 scripts/check_kuuos_connection_improvement_candidate_v0_62.py
python3 scripts/check_kuuos_v063.py
python3 scripts/check_kuuos_evaluation_v064.py
python3 scripts/check_kuuos_staging_v065.py
python3 scripts/check_kuuos_shadow_v066.py
python3 scripts/check_kuuos_orbit_v067.py
python3 scripts/check_kuuos_evidence_v068.py
python3 scripts/check_kuuos_evidence_review_v069.py

lake -KleanArgs=-DwarningAsError=true \
  -KleanArgs=-DsorryAsError=true \
  build KuuOSFormalV0_60 KuuOSFormalV0_61 KuuOSFormalV0_62 KuuOSFormalV0_69
