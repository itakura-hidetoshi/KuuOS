...
公開状態のauthoritative branchは`main`です。最新の機能統合はPR #1285、**VerifyOS v0.15 Outcome Disposition Handoff v0.1**です。この公開面は、そのsquash merge commit `7345c0f6f98a5dec327296e4f7b7f9210956db63`を基準に同期しています。
...
subsystem versionは独立しています。ObserveOS v0.7、VerifyOS v0.15、PlanOS v1.23、DecisionOS v0.6、MemoryOS v1.00、self-organization v0.113を、一つの直線的version番号として扱いません。
...
VerifyOS v0.15は、canonicalでcompletedなv0.14 receiptを入力とし、検証結果を他OSへ渡すための**read-only disposition candidate**を構成します。候補生成とWORLD disposition、WORLD commit、policy activation、executionを分離します。
...
v0.15は次をexact bindingします。

- source v0.14 receipt、outcome、WORLD revision
- independent disposition-authority sourceとpolicy
- governance review、reviewer independence、bounded validity window
- evidence preservationとappeal route
- verification debtとreobservation requirement
- session、nonce、request、review、bundleのreplay closure
- history、lineage、responsibility monotonicity

14のdispositionを、adopt、reject、defer、reobservation candidate、source/outcome/authority/evidence/appeal/debt/window repair、replay rejection、current-state mutation rejection、authority escalation rejectionへ分離します。
...
reject candidateではsource evidenceを保持し、appeal reviewを必須とします。indeterminate routeではopen verification debtと`reobservation_required = true`を維持します。いずれの候補も`world_disposition_completed = false`、`world_commit_ready = false`です。

| Surface | Path |
|---|---|
| Structured subsystem index | `docs/VerifyOS/README.md` |
| Versioned specification | `docs/KUUOS_VERIFYOS_OUTCOME_DISPOSITION_HANDOFF_v0_1.md` |
| Runtime handoff | `runtime/kuuos_verifyos_outcome_disposition_handoff_v0_1.py` |
| Route checker | `scripts/check_verifyos_outcome_disposition_handoff_v0_1.py` |
...
| Dedicated workflow | `.github/workflows/verifyos-outcome-disposition-handoff-v0-1.yml` |

専用CIはPython surfaces、manifest、全14 candidate/repair/rejection routes、tampered request fail-closed、Evidence Cycle累積回帰、pinned dependency manifest、VerifyOS v0.15 formal root、repository formal aggregateをstrict modeで検証します。
...
| VerifyOS | independent verification handoff、independent evidence verification、read-only outcome disposition handoff |
| LearnOS | future-only learning delta |

```text
ObserveOS evidence != VerifyOS verdict
VerifyOS verdict != truth authority
VerifyOS disposition candidate != WORLD disposition
...
| VerifyOS v0.15 disposition aggregate | `formal/KuuOSVerifyOSV0_15.lean` |
