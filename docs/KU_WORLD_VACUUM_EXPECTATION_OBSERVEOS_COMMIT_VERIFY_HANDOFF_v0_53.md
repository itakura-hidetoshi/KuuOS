...
v0.53 composes the existing ObserveOS v0.3, VerifyOS v0.3 and LearnOS v0.3 receipt types over the WORLD v0.51 vacuum-expectation intake envelope.

It does not replace the separate WORLD v0.52 host-effect intake route.

It does not define parallel ObserveOS, VerifyOS or LearnOS receipts.

```text
v0.51 vacuum-expectation intake-ready envelope
...
The repository also retains the independent effect route:

```text
ActOS v0.4 canonical effectRecorded receipt
  → WORLD v0.52 host-effect intake candidate
  → ObserveOS v0.4 independent effect-grounded observation
```

The two routes are not identified.

```text
analytic vacuum-expectation candidate != ActOS host effect
...
The WORLD sidecar and the v0.53 runtime do not perform it.
...
v0.53 adds only a WORLD-side composition digest over the supplied LearnOS receipt.
...
The v0.53 runtime flags are fixed to false for:
