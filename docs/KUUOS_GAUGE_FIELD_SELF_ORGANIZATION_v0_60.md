...
v0.60が変更できるのは場の適応成分だけです。

接続、plaquette評価領域、チャート領域、field identityは変更できません。

これは、評価対象を削除して作用を人工的に小さくする経路を閉じます。
...
+ chart domain unchanged
+ field identity unchanged
+ connection unchanged
+ plaquette domain unchanged
...
明示的なrollback digestが供給されない場合は、変更前configurationの完全digestを既定rollback targetとして束縛します。
...
tests/test_kuuos_gauge_field_fixed_connection_v0_60.py
tests/test_kuuos_gauge_field_fixed_domain_v0_60.py
