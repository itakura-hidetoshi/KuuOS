...
from runtime.kuuos_runtime_daemon_qi_policy_bias_applier_v7_6 import build_qi_policy_bias_applier
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi policy bias applier v7.6")
    ...
    result = build_qi_policy_bias_applier(
        runtime_context=read_json(args.context),
        qi_policy_bias_applier_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_POLICY_BIAS_APPLIER_READY" else 1
