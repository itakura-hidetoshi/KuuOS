...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions loop run executor v6.2")
    ...
    result = build_qi_github_actions_loop_run_executor(
        runtime_context=read_json(args.context),
        loop_run_executor_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_LOOP_RUN_EXECUTOR_READY" else 1
