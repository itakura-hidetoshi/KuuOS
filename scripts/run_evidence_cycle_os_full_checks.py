#!/usr/bin/env python3
"""Run cumulative evidence-cycle checks through ObserveOS v0.4 maintenance monitoring."""
...
COMMANDS=(
    ...
    (sys.executable, 'scripts/check_observeos_future_only_maintenance_monitoring_observation_intake_v0_1.py',),
)
...
def main()->int:
    env=os.environ.copy(); env["PYTHONPATH"]=os.pathsep.join(x for x in (str(ROOT),env.get("PYTHONPATH","")) if x); env.setdefault("PYTHONUNBUFFERED","1")
    failures=[]
    for command in COMMANDS:
        code=run_command(command,env)
        if code: failures.append((command,code))
    if failures:
        for command,code in failures: print(f"FAIL: {' '.join(command)} exited with {code}")
        return 1
    print("\nPASS: Evidence Cycle OS validation completed through ObserveOS v0.4 future-only maintenance-monitoring observation intake")
    return 0
