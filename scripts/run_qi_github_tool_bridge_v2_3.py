...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub tool bridge v2.3")
    ...
    result = build_qi_github_tool_bridge(
        runtime_context=read_json(args.context),
        bridge_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status in {"QI_GITHUB_TOOL_BRIDGE_APPLIED", "QI_GITHUB_TOOL_BRIDGE_IDLE", "QI_GITHUB_TOOL_BRIDGE_PARTIAL"} else 1
