...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions routed E2E orchestrator v9.2")
    ...
    result = build_qi_github_actions_routed_e2e_orchestrator(
        runtime_context=read_json(args.context),
        routed_e2e_orchestrator_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_ROUTED_E2E_ORCHESTRATOR_READY" else 1
