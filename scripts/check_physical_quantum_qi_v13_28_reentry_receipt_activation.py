...
from runtime.kuuos_runtime_daemon_physical_quantum_qi_v13_28_reentry_receipt_activation import (
    build_physical_quantum_qi_v13_28_reentry_receipt_activation,
)
...
def prepare(
    ...
    kind: str,
    *,
    bad_activation_digest: bool = False,
    bad_packet_weighting: bool = False,
) -> None:
    status, action, weighting = parts(kind)
    weighting = dict(weighting)
    if bad_packet_weighting and kind == "reinforced":
    ...
    packet = {
        ...
        "source_digests": {
            "reentry_weighting_state": "reentry-state-v13-28",
            "feedback_to_reentry_weighting_bridge": "weighting-bridge-v13-28",
        },
    ...
    activation = {
        ...
        "source_v13_26_activation_record_digest": "v13-26-record-v13-28",
        "source_v12_9_reentry_weighting_state_digest": "v12-9-state-v13-28",
        "source_v13_12_reentry_ready_state_digest": "v13-12-ready-v13-28",
        "source_v13_0_candidate_weighting_cycle_state_digest": (
            "wrong-cycle-digest" if bad_activation_digest else cycle_digest
        ),
        "boundary": {
...
def activation_license(**overrides: Any) -> dict[str, Any]:
    value = {
        "license_status": "PHYSICAL_QUANTUM_QI_V13_28_REENTRY_RECEIPT_ACTIVATION_LICENSE_READY",
        "v13_27_activation_record_read_allowed": True,
        "v13_0_closed_loop_packet_read_allowed": True,
        "v13_0_candidate_cycle_state_read_allowed": True,
        "v13_13_bridge_invoke_allowed": True,
        "v13_1_receipt_ledger_invoke_allowed": True,
        "activation_record_write_allowed": True,
        "receipt_write_allowed": True,
        "audit_append_allowed": True,
        "v13_13_bridge_license": bridge_license(),
...
def context(root: pathlib.Path) -> dict[str, Any]:
    return {
        "physical_quantum_qi_v13_28_reentry_receipt_activation_enabled": True,
        "apply_physical_quantum_qi_v13_28_reentry_receipt_activation": True,
        "runtime_root": str(root),
    }


def run(root: pathlib.Path, license_packet: dict[str, Any]) -> dict[str, Any]:
    return build_physical_quantum_qi_v13_28_reentry_receipt_activation(
        runtime_context=context(root),
        v13_28_reentry_receipt_activation_license=license_packet,
    ).to_dict()
...
def main() -> int:
    with tempfile.TemporaryDirectory() as directory:
        ...
        for kind in ("reinforced", "probe", "barrier"):
            ...
            assert result["status"] == "PHYSICAL_QUANTUM_QI_V13_28_REENTRY_RECEIPT_ACTIVATION_READY", (kind, result)
            assert result["closed_loop_reentry_status"] == status
            assert result["reentry_weighting_action"] == action
            assert result["v13_13_bridge_invoked"] is True
        ...
        assert result["v13_13_bridge_invoked"] is False
        assert "v13_27_candidate_cycle_state_digest_mismatch" in result["blockers"]

        root = base / "bridge_failure"
        prepare(root, "reinforced", bad_packet_weighting=True)
        result = run(root, activation_license())
        assert result["status"].endswith("BLOCKED")
        assert result["v13_13_bridge_invoked"] is True
    ...
    print("physical_quantum_qi_v13_28_reentry_receipt_activation checks passed")
    return 0
