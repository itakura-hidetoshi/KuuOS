...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions full terminal autopilot v11.0")
    ...
    result = build_qi_github_actions_full_terminal_autopilot(
        runtime_context=read_json(args.context),
        full_terminal_autopilot_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_FULL_TERMINAL_AUTOPILOT_READY" else 1
