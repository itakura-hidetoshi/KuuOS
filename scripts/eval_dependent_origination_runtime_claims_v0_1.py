...
def claim_from_dict(data: dict[str, Any]) -> Any:
    return runtime.DependentOriginationClaim(**data)
...
def evaluate_file(path: pathlib.Path) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for item in data.get("claims", []):
        ...
        out = runtime.evaluate_dependent_origination(claim_from_dict(item.get("claim", {})))
...
def main() -> int:
    ...
    parser.add_argument("--input", default="specs/dependent_origination_runtime_claims_v0_1.json")
    ...
    if args.json:
        ...
        if errors:
            for err in errors:
                print("ERROR:", err)
            return 1
        print("PASS: Dependent Origination runtime claim evaluation completed")
    return 0
