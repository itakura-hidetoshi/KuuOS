...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions loop coordinator v6.0")
    ...
    result = build_qi_github_actions_loop_coordinator(
        runtime_context=read_json(args.context),
        loop_coordinator_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_LOOP_COORDINATOR_READY" else 1
