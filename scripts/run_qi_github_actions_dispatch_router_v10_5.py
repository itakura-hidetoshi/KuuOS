...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions dispatch router v10.5")
    ...
    result = build_qi_github_actions_dispatch_router(
        runtime_context=read_json(args.context),
        dispatch_router_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_DISPATCH_ROUTER_READY" else 1
