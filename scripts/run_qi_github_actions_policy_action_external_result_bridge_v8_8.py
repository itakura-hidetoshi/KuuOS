...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions policy action external result bridge v8.8")
    ...
    result = build_qi_github_actions_policy_action_external_result_bridge(
        ...
        policy_action_external_result_bridge_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_POLICY_ACTION_EXTERNAL_RESULT_BRIDGE_READY" else 1
