...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions PR live autopilot result ingestor v8.5")
    ...
    result = build_qi_github_actions_pr_live_autopilot_result_ingestor(
        runtime_context=read_json(args.context),
        autopilot_result_ingestor_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_PR_LIVE_AUTOPILOT_RESULT_INGESTOR_READY" else 1
