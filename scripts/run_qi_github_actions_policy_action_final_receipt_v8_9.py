...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions policy action final receipt v8.9")
    ...
    result = build_qi_github_actions_policy_action_final_receipt(
        ...
        policy_action_final_receipt_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_POLICY_ACTION_FINAL_RECEIPT_READY" else 1
