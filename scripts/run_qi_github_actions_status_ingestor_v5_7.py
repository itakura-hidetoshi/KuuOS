...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions status ingestor v5.7")
    ...
    result = build_qi_github_actions_result_ingestor(
        runtime_context=read_json(args.context),
        result_ingestor_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_RESULT_INGESTOR_READY" else 1
