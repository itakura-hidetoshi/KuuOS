...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions PR live external cycle runner v8.3")
    ...
    result = build_qi_github_actions_pr_live_external_cycle_runner(
        runtime_context=read_json(args.context),
        pr_live_external_cycle_runner_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_PR_LIVE_EXTERNAL_CYCLE_RUNNER_READY" else 1
