...
REQUIRED_TOKENS = {
    ...
    "docs/CI_LEDGER_EMPTINESS_DO_TWO_TRUTHS_RUNTIME_AUDIT_CHAIN_v0_1.md": [
        "CI configured; latest CI result not yet recorded",
...
def main() -> int:
    ...
    for file_path in REQUIRED_FILES:
        ...
        for token in FORBIDDEN_TOKENS:
            if token in text:
                errors.append(f"forbidden token in {file_path}: {token}")

    manifest_text = read(MANIFEST) if not errors or (ROOT / MANIFEST).exists() else ""
