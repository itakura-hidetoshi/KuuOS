...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the write-capable KuuOS bridge to GitHub's official MCP Server."
    )
    parser.add_argument(
        "runtime_root",
        help="Directory containing the v0.2 plan and authority JSON files.",
    )
    parser.add_argument(
        "--execute-external-actions",
        action="store_true",
        help=(
            "Enable the runtime half of the write gate. Plan, authority, per-operation "
            "approval, repository scope, and exact SHA gates must also pass."
        ),
    )
    ...
    return 0 if result.status in {
        ...
        "KUUOS_GITHUB_MCP_WRITE_BRIDGE_IDLE",
    } else 1
