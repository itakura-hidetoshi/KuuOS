...
CLI = ROOT / "scripts" / "run_tri_os_feedback_action_intake_decision_ledger_v9_5.py"
...
def ctx(root: pathlib.Path) -> dict[str, Any]:
    return {"tri_os_feedback_action_intake_decision_ledger_enabled": True, "apply_tri_os_feedback_action_intake_decision_ledger": True, "runtime_root": str(root)}


def lic(**overrides: Any) -> dict[str, Any]:
    value = {"license_status": "TRI_OS_FEEDBACK_ACTION_INTAKE_DECISION_LEDGER_LICENSE_READY", "action_intake_packet_read_allowed": True, "unified_ledger_append_allowed": True, "os_ledger_append_allowed": True, "summary_write_allowed": True, "receipt_write_allowed": True, "audit_append_allowed": True}
...
def run(root: pathlib.Path, name: str, p: dict[str, Any] | None, license_packet: dict[str, Any]) -> tuple[int, dict[str, Any], list[dict[str, Any]], dict[str, Any], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    runtime = root / name
    if p is not None:
        dump(runtime / "tri_os_feedback_action_intake_decision_packet.json", p)
    ...
    return (
        ...
        load_json(op),
        rows(runtime / "tri_os_feedback_action_intake_decision_ledger.jsonl"),
        load_json(runtime / "tri_os_feedback_action_intake_decision_summary.json"),
        rows(runtime / "memory_os_feedback_action_intake_ledger.jsonl"),
        rows(runtime / "plan_os_feedback_action_intake_ledger.jsonl"),
        rows(runtime / "run_governance_feedback_action_intake_ledger.jsonl"),
    )
...
def assert_ready(case: str, code: int, out: dict[str, Any], unified: list[dict[str, Any]], summary: dict[str, Any], mem_rows: list[dict[str, Any]], plan_rows: list[dict[str, Any]], run_rows: list[dict[str, Any]]) -> None:
    assert code == 0, case
    assert out["status"] == "TRI_OS_FEEDBACK_ACTION_INTAKE_DECISION_LEDGER_READY", case
    ...
    assert summary["boundary"]["action_intake_history_only"] is True, case
...
def main() -> int:
    with tempfile.TemporaryDirectory() as td:
        ...
        assert code == 1
        assert "tri_os_feedback_action_intake_decision_packet_missing_or_invalid" in out["blockers"]
        assert unified == []

        bad = packet()
        bad["tri_os_feedback_action_intake_considered"] = False
        code, out, unified, summary, mem_rows, plan_rows, run_rows = run(root, "bad_considered", bad, lic())
        assert code == 1
        assert "tri_os_feedback_action_intake_considered_not_true" in out["blockers"]
        assert unified == []
        ...
        assert code == 1
        assert "planos_action_decision_invalid" in out["blockers"]
        assert unified == []
        ...
        assert unified == []
    print("tri_os_feedback_action_intake_decision_ledger_v9_5 checks passed")
    return 0
