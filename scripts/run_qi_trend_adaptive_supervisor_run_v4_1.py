...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi trend adaptive supervisor run v4.1")
    ...
    result = build_qi_trend_adaptive_supervisor_run(
        runtime_context=read_json(args.context),
        adaptive_run_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_TREND_ADAPTIVE_SUPERVISOR_RUN_READY" else 1
