...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi epoch runner v1.2")
    ...
    result = build_qi_epoch_runner(
        runtime_context=read_json(args.context),
        pt_packet=read_json(args.pt),
        epoch_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status in {"QI_EPOCH_RUNNER_APPLIED", "QI_EPOCH_RUNNER_IDLE"} else 1
