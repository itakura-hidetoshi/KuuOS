...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi circulation change router v2.8")
    ...
    result = build_qi_circulation_change_router(
        runtime_context=read_json(args.context),
        router_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status in {"QI_CIRCULATION_CHANGE_ROUTER_ROUTED", "QI_CIRCULATION_CHANGE_ROUTER_IDLE"} else 1
