...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions terminal execution bridge v11.1")
    ...
    result = build_qi_github_actions_terminal_execution_bridge(
        runtime_context=read_json(args.context),
        terminal_execution_bridge_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_TERMINAL_EXECUTION_BRIDGE_READY" else 1
