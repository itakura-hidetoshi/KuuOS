...
from runtime.kuuos_runtime_daemon_qi_pt_state_step_v0_6 import build_qi_pt_state_step
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi PT state step v0.6")
    ...
    result = build_qi_pt_state_step(
        runtime_context=read_json(args.context),
        process_tensor_packet=read_json(args.pt),
        step_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status in {"QI_PT_STATE_STEP_APPLIED", "QI_PT_STATE_STEP_REPLAYED", "QI_PT_STATE_STEP_IDLE"} else 1
