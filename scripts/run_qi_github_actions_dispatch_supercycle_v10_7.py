...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions dispatch supercycle v10.7")
    ...
    result = build_qi_github_actions_dispatch_supercycle(
        runtime_context=read_json(args.context),
        dispatch_supercycle_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_DISPATCH_SUPERCYCLE_READY" else 1
