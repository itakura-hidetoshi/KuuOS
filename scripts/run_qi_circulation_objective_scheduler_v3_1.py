...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi circulation objective scheduler v3.1")
    ...
    result = build_qi_circulation_objective_scheduler(
        runtime_context=read_json(args.context),
        scheduler_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_CIRCULATION_SCHEDULER_READY" else 1
