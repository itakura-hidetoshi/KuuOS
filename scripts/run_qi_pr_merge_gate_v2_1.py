...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi PR merge gate v2.1")
    ...
    result = build_qi_pr_merge_gate(
        runtime_context=read_json(args.context),
        gate_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_PR_MERGE_GATE_PASSED" else 1
