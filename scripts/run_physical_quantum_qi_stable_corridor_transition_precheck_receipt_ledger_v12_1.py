...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Physical Quantum Qi stable corridor transition precheck receipt ledger v12.1")
    ...
    result = build_physical_quantum_qi_stable_corridor_transition_precheck_receipt_ledger(
        ...
        stable_corridor_transition_precheck_receipt_ledger_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "PHYSICAL_QUANTUM_QI_STABLE_CORRIDOR_TRANSITION_PRECHECK_RECEIPT_LEDGER_READY" else 1
