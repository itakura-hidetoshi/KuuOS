#!/usr/bin/env python3
"""
validate_invariant_gate_fixtures_v0_1.py

Stdlib-only fixture validator for the minimal KuuOS Invariant Gate runtime.
"""
...
FIXTURE_PATH = ROOT / "specs" / "invariant_gate_fixtures_v0_1.json"
EVALUATOR_PATH = ROOT / "examples" / "invariant_gate_minimal.py"


def load_evaluator() -> Any:
    module_name = "invariant_gate_minimal"
    spec = importlib.util.spec_from_file_location(module_name, EVALUATOR_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError("failed to load invariant gate module spec")
...
def validate_fixture(evaluator: Any, fixture: dict[str, Any]) -> list[str]:
    ...
    inp = evaluator.InvariantGateInput(**fixture["input"])
    out = evaluator.evaluate_gate(inp)
...
def main() -> int:
    ...
    print("PASS: Invariant Gate fixtures validated")
    return 0
