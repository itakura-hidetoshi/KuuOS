...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Write KuuOS Qi license candidate from a mutable phase boundary v0.1")
    parser.add_argument("--trend-summary", type=pathlib.Path, required=True)
    parser.add_argument("--phase-packet", type=pathlib.Path, required=True)
    parser.add_argument("--mode", type=str, default="dry_run_probe_simulation")
    ...
    candidate = build_qi_actuation_license_candidate(
        ...
        phase_boundary_packet=read_json(args.phase_packet),
        requested_actuation_mode=args.mode,
    )
