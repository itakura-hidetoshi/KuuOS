...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions terminal finalizer v10.8")
    ...
    result = build_qi_github_actions_terminal_finalizer(
        runtime_context=read_json(args.context),
        terminal_finalizer_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_TERMINAL_FINALIZER_READY" else 1
