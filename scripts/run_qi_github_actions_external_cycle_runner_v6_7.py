...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions external cycle runner v6.7")
    ...
    result = build_qi_github_actions_external_cycle_runner(
        runtime_context=read_json(args.context),
        external_cycle_runner_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_EXTERNAL_CYCLE_RUNNER_READY" else 1
