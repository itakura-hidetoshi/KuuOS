...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions PR live snapshot adapter v7.3")
    ...
    result = build_qi_github_actions_pr_live_snapshot_adapter(
        runtime_context=read_json(args.context),
        pr_live_snapshot_adapter_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_PR_LIVE_SNAPSHOT_ADAPTER_READY" else 1
