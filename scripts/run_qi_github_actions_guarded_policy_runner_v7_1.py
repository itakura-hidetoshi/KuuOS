...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions guarded policy runner v7.1")
    ...
    result = build_qi_github_actions_guarded_policy_runner(
        runtime_context=read_json(args.context),
        guarded_policy_runner_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_GUARDED_POLICY_RUNNER_READY" else 1
