...
def main() -> int:
    ...
    result = build_indra_qi_post_assimilation_causal_reentry_v0_7(
        runtime_context=read_json(args.context),
        reentry_plan=read_json(args.plan),
        reentry_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "INDRA_QI_POST_ASSIMILATION_CAUSAL_REENTRY_V0_7_READY" else 1
