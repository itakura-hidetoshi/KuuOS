...
def http_error_detail(error: urllib.error.HTTPError) -> str:
    try:
        ...
        raw = b""
    body = raw.decode("utf-8", errors="replace").strip() if isinstance(raw, bytes) else str(raw)
...
def validate_dispatch_payload(
    ...
    if repository != REPOSITORY or event.get("action") != EVENT_TYPE:
        return None
    ...
    if raw.get("repository") != REPOSITORY:
        return None
    workflow = str(raw.get("workflow", raw.get("workflow_name", "")))
...
def process_event(
    ...
    *,
    ...
    for item in github.list_issue_comments(identity.repository, identity.pull_request):
    ...
    created = github.create_issue_comment(
        ...
        build_comment(identity, terminal, polling_used=polling_used),
    )
    return {
...
class GitHubApi:
    def __init__(self, token: str):
        self.headers = {
            ...
            "User-Agent": "KuuOS-ChatGPT-CI-Secondary-Comment-v0.1",
        }
...
def main() -> int:
    ...
    print(
        "CHATGPT_CI_SECONDARY_COMMENT_RESULT "
