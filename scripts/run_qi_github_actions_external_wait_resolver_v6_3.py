...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions external wait resolver v6.3")
    ...
    result = build_qi_github_actions_external_wait_resolver(
        runtime_context=read_json(args.context),
        external_wait_resolver_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_EXTERNAL_WAIT_RESOLVER_READY" else 1
