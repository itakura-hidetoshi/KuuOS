...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi process tensor return loop orchestrator v3.6")
    ...
    result = build_qi_process_tensor_return_loop(
        runtime_context=read_json(args.context),
        return_loop_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_PROCESS_TENSOR_RETURN_LOOP_READY" else 1
