...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions full cycle policy runner v6.9")
    ...
    result = build_qi_github_actions_full_cycle_policy_runner(
        runtime_context=read_json(args.context),
        policy_runner_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_FULL_CYCLE_POLICY_RUNNER_READY" else 1
