...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi trend adaptive supervisor packet v4.0")
    ...
    result = build_qi_trend_adaptive_supervisor_packet(
        runtime_context=read_json(args.context),
        adaptive_packet_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_TREND_ADAPTIVE_SUPERVISOR_PACKET_READY" else 1
