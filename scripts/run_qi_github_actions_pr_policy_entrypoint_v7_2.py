...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions PR policy entrypoint v7.2")
    ...
    result = build_qi_github_actions_pr_policy_entrypoint(
        runtime_context=read_json(args.context),
        pr_policy_entrypoint_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_PR_POLICY_ENTRYPOINT_READY" else 1
