...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions PR live snapshot collector v7.4")
    ...
    result = build_qi_github_actions_pr_live_snapshot_collector(
        runtime_context=read_json(args.context),
        pr_live_snapshot_collector_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_PR_LIVE_SNAPSHOT_COLLECTOR_READY" else 1
