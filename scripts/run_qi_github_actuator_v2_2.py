...
from runtime.kuuos_runtime_daemon_qi_github_actuator_v2_2 import build_qi_github_actuator
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub actuator v2.2")
    ...
    result = build_qi_github_actuator(
        runtime_context=read_json(args.context),
        actuator_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status in {"QI_GITHUB_ACTUATOR_READY", "QI_GITHUB_ACTUATOR_MERGE_BLOCKED"} else 1
