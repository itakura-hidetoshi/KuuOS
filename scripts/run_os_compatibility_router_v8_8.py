...
from runtime.kuuos_runtime_daemon_os_compatibility_router_v8_8 import build_os_compatibility_router
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run OS compatibility router v8.8")
    ...
    result = build_os_compatibility_router(
        runtime_context=read_json(args.context),
        os_compatibility_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "OS_COMPATIBILITY_ROUTER_READY" else 1
