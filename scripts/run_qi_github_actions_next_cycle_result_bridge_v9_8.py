...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions next cycle result bridge v9.8")
    ...
    result = build_qi_github_actions_next_cycle_result_bridge(
        runtime_context=read_json(args.context),
        next_cycle_result_bridge_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_NEXT_CYCLE_RESULT_BRIDGE_READY" else 1
