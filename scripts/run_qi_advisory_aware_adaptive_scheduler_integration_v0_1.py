...
from runtime.kuuos_runtime_daemon_qi_advisory_aware_adaptive_scheduler_integration_v0_1 import run_qi_advisory_aware_adaptive_scheduler_integration
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi advisory-aware adaptive scheduler integration v0.1")
    parser.add_argument("--advisory", type=pathlib.Path, required=True)
    ...
    result = run_qi_advisory_aware_adaptive_scheduler_integration(
        advisory_packet=read_json(args.advisory),
        ...
        integration_context=read_json(args.context),
    )
    ...
    return 0 if result.integration_status == "QI_ADVISORY_AWARE_ADAPTIVE_SCHEDULER_INTEGRATION_COMPLETED" else 1
