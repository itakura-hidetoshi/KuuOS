...
def main() -> int:
    ...
    result = build_indra_qi_process_tensor_cycle_v0_5(
        runtime_context=read_json(args.context),
        cycle_plan=read_json(args.plan),
        cycle_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "INDRA_QI_PROCESS_TENSOR_CYCLE_V0_5_READY" else 1
