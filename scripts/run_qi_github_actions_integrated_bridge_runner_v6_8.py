...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions integrated bridge runner v6.8")
    ...
    result = build_qi_github_actions_integrated_bridge_runner(
        runtime_context=read_json(args.context),
        integrated_bridge_runner_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_INTEGRATED_BRIDGE_RUNNER_READY" else 1
