...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions autopilot policy action handoff v8.6")
    ...
    result = build_qi_github_actions_autopilot_policy_action_handoff(
        ...
        policy_action_handoff_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_AUTOPILOT_POLICY_ACTION_HANDOFF_READY" else 1
