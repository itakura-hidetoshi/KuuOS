...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions capability planner v5.5")
    ...
    result = build_qi_github_actions_capability_planner(
        runtime_context=read_json(args.context),
        github_actions_planner_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_CAPABILITY_PLANNER_READY" else 1
