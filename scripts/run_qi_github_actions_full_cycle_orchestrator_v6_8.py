...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions full cycle orchestrator v6.8")
    ...
    result = build_qi_github_actions_full_cycle_orchestrator(
        runtime_context=read_json(args.context),
        full_cycle_orchestrator_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_FULL_CYCLE_ORCHESTRATOR_READY" else 1
