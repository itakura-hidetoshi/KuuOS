...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi automerge executor v2.4")
    ...
    result = build_qi_automerge_executor(
        runtime_context=read_json(args.context),
        executor_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status in {"QI_AUTOMERGE_EXECUTOR_MERGED", "QI_AUTOMERGE_EXECUTOR_IDLE"} else 1
