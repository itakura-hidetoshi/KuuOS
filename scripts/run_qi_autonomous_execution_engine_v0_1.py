...
from runtime.kuuos_runtime_daemon_qi_autonomous_execution_engine_v0_1 import build_qi_autonomous_execution_engine
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build Qi autonomous execution engine v0.1")
    ...
    parser.add_argument("--cadence-release", type=pathlib.Path, required=True)
    ...
    result = build_qi_autonomous_execution_engine(
        ...
        cadence_release_packet=read_json(args.cadence_release),
        execution_context=read_json(args.context),
    )
    ...
    return 0 if result.engine_status == "QI_AUTONOMOUS_EXECUTION_ENGINE_READY" else 1
