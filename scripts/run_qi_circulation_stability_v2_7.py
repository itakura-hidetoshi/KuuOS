...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi circulation stability v2.7")
    ...
    result = build_qi_circulation_stability(
        runtime_context=read_json(args.context),
        stability_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_CIRCULATION_STABILITY_READY" else 1
