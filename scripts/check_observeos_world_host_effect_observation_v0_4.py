...
def main() -> int:
    ...
    family = tuple(sorted(formal.parent.glob("WorldHostEffectObservationReceipt*.lean")))
    ...
    for path in (formal, formal_root, aggregate, docs, manifest_path, workflow, *family):
        require(path.is_file(), f"missing file: {path}")

    import_token = "KUOS.ObserveOS.WorldHostEffectObservationReceiptV0_4"
    require_tokens(formal_root, (import_token,))
    require_tokens(aggregate, (import_token,))
    ...
    tokens = (
        ...
        "WorldHostEffectObservationBridge", "WorldHostEffectObservationReceipt",
        "observation_requires_ready_uncommitted_world_intake",
        "observation_uses_exact_act_cycle", "observation_preserves_upstream_lineage",
        ...
        "evidence_collection_and_receipt_are_single_use", "observation_events_append_strictly",
        "observation_history_appends_two_records", "ownership_is_separated",
        "observation_receipt_digest_is_exact",
    )
    for token in tokens:
        require(token in text, f"module family missing {token}")
    require_tokens(docs, (
        "host receipt used as independent evidence = false", "verification required = true",
        "atomic commit ready = false", "receipt immutable = true",
    ))
    require_tokens(ROOT / "scripts/run_kuuos_runtime_full_check_v0_52.py", ("check_observeos_v04",))
