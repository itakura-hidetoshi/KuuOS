...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions external bridge executor v6.7")
    ...
    result = build_qi_github_actions_external_bridge_executor(
        runtime_context=read_json(args.context),
        external_bridge_executor_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_EXTERNAL_BRIDGE_EXECUTOR_READY" else 1
