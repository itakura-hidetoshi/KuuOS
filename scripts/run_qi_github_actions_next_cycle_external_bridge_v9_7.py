...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions next cycle external bridge v9.7")
    ...
    result = build_qi_github_actions_next_cycle_external_bridge(
        runtime_context=read_json(args.context),
        next_cycle_external_bridge_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_NEXT_CYCLE_EXTERNAL_BRIDGE_READY" else 1
