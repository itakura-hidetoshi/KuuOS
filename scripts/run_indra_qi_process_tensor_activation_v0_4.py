...
def main() -> int:
    ...
    result = build_indra_qi_process_tensor_activation_v0_4(
        runtime_context=read_json(args.context),
        activation_plan=read_json(args.plan),
        activation_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "INDRA_QI_PROCESS_TENSOR_ACTIVATION_V0_4_READY" else 1
