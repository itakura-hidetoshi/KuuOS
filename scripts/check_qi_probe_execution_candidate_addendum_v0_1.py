...
MANIFEST = ROOT / "manifests" / "qi_probe_execution_candidate_addendum_v0_1.json"
...
def main() -> int:
    ...
    if m.get("manifest_version") != "qi_probe_execution_candidate_addendum_v0_1":
        errors.append("manifest_version_mismatch")
    if m.get("addendum_status") != "QI_PROBE_EXECUTION_CANDIDATE_ADDENDUM_READY":
        errors.append("addendum_status_mismatch")
    if m.get("extends_manifest") != "manifests/qi_process_tensor_aware_scheduler_addendum_v0_1.json":
        errors.append("extends_manifest_mismatch")
    if m.get("authority") != "none":
        errors.append("authority_not_none")
    for key in [
        "execution_candidate_only",
        "scheduler_due_required",
    ...
    if errors:
        for error in errors:
            print("ERROR:", error)
        return 1
    print("PASS: Qi probe execution candidate addendum check")
    return 0
