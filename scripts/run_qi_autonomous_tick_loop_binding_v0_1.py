...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Bind Qi autonomous tick policy receipt to daemon loop v0.1")
    ...
    result = bind_qi_autonomous_tick_policy_to_daemon_loop(
        policy_packet=read_json(args.policy),
        ...
        loop_context=read_json(args.context),
    )
    ...
    return 0 if result.loop_binding_status == "QI_AUTONOMOUS_TICK_LOOP_BINDING_COMPLETED" else 1
