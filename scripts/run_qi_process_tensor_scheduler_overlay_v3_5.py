...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi process tensor scheduler overlay v3.5")
    ...
    result = build_qi_process_tensor_scheduler_overlay(
        runtime_context=read_json(args.context),
        overlay_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_PROCESS_TENSOR_SCHEDULER_OVERLAY_READY" else 1
