...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi progress-aware integrated runner v7.9")
    ...
    result = build_qi_progress_aware_integrated_runner(
        runtime_context=read_json(args.context),
        progress_integrated_runner_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_PROGRESS_AWARE_INTEGRATED_RUNNER_READY" else 1
