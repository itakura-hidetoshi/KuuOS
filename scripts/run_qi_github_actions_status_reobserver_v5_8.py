...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions status reobserver v5.8")
    ...
    result = build_qi_github_actions_status_reobserver(
        runtime_context=read_json(args.context),
        status_reobserver_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_STATUS_REOBSERVER_READY" else 1
