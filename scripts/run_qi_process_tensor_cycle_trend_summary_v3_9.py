...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi process tensor cycle trend summary v3.9")
    ...
    result = build_qi_process_tensor_cycle_trend_summary(
        runtime_context=read_json(args.context),
        trend_summary_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_PROCESS_TENSOR_CYCLE_TREND_SUMMARY_READY" else 1
