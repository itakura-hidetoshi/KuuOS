...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi circulation scheduler packet handoff v3.4")
    ...
    result = build_qi_circulation_scheduler_packet(
        runtime_context=read_json(args.context),
        scheduler_packet_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_CIRCULATION_SCHEDULER_PACKET_READY" else 1
