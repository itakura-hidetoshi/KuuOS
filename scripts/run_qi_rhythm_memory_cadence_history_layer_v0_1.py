...
from runtime.kuuos_runtime_daemon_qi_rhythm_memory_cadence_history_layer_v0_1 import run_qi_rhythm_memory_cadence_history_layer
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi rhythm memory cadence history layer v0.1")
    ...
    parser.add_argument("--rhythm-history", type=pathlib.Path, required=True)
    ...
    result = run_qi_rhythm_memory_cadence_history_layer(
        ...
        rhythm_history=read_json(args.rhythm_history),
        event_log_path=args.event_log,
        ledger_state_path=args.ledger_state,
        rhythm_context=read_json(args.context),
    )
    ...
    return 0 if result.rhythm_layer_status == "QI_RHYTHM_MEMORY_CADENCE_HISTORY_LAYER_COMPLETED" else 1
