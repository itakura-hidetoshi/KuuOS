...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions policy safety gate v7.0")
    ...
    result = build_qi_github_actions_policy_safety_gate(
        runtime_context=read_json(args.context),
        policy_safety_gate_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_POLICY_SAFETY_GATE_READY" else 1
