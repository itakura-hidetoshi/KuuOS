...
BUILDER = ROOT / "scripts" / "build_dependent_origination_runtime_bundle_v0_1.py"
MANIFEST = ROOT / "specs" / "dependent_origination_runtime_bundle_v0_1.generated.json"
...
def main() -> int:
    ...
    errors: list[str] = []
    if manifest.get("authority_note") != "dependent_origination_runtime_integrity_surface_only":
    ...
    if errors:
        for e in errors:
            print("ERROR:", e)
        return 1
    print(f"PASS: Dependent Origination runtime bundle checked root={manifest['bundle_root_hash']}")
    return 0
