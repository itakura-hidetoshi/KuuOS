...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi scheduled closed-loop runner v3.2")
    ...
    result = build_qi_scheduled_closed_loop_runner(
        runtime_context=read_json(args.context),
        runner_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status in {"QI_SCHEDULED_CLOSED_LOOP_READY", "QI_SCHEDULED_CLOSED_LOOP_CONVERGED"} else 1
