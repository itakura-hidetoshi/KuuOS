...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi executable action batch recipe executor v4.8")
    ...
    result = build_qi_executable_action_batch_recipe_executor(
        runtime_context=read_json(args.context),
        batch_recipe_executor_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_EXECUTABLE_ACTION_BATCH_RECIPE_EXECUTOR_READY" else 1
