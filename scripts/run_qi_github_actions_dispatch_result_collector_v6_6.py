...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions dispatch result collector v6.6")
    ...
    result = build_qi_github_actions_dispatch_result_collector(
        runtime_context=read_json(args.context),
        dispatch_result_collector_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_DISPATCH_RESULT_COLLECTOR_READY" else 1
