...
from runtime.kuuos_runtime_daemon_physical_quantum_qi_corridor_stability_gate_receipt_ledger_v11_9 import build_physical_quantum_qi_corridor_stability_gate_receipt_ledger
...
PT = {
    "process_tensor_digest": "pt-digest-v11-9",
    "memory_kernel_digest": "memory-kernel-digest-v11-9",
    "history_window_digest": "history-window-digest-v11-9",
    "instrument_trace_digest": "instrument-trace-digest-v11-9",
    "non_markov_context_digest": "non-markov-context-digest-v11-9",
}
...
def ctx(root: pathlib.Path) -> dict[str, Any]:
    return {
        "physical_quantum_qi_corridor_stability_gate_receipt_ledger_enabled": True,
        "apply_physical_quantum_qi_corridor_stability_gate_receipt_ledger": True,
...
def lic(**overrides: Any) -> dict[str, Any]:
    value = {
        "license_status": "PHYSICAL_QUANTUM_QI_CORRIDOR_STABILITY_GATE_RECEIPT_LEDGER_LICENSE_READY",
        "corridor_stability_gate_packet_read_allowed": True,
...
def gate_packet(decision: str) -> dict[str, Any]:
    ...
    packet = {
        ...
        "source_digests": {
            "history_aware_candidate_corridor_receipt": "corridor-receipt-digest-v11-9",
            "history_aware_candidate_corridor": "corridor-digest-v11-9",
        },
        "boundary": dict(GATE_BOUNDARY),
        "epoch": 1,
    }
    packet["corridor_stability_gate_digest"] = f"corridor-stability-gate-digest-{decision}"
    return packet
...
def run(root: pathlib.Path, name: str, packet: dict[str, Any] | None, license_packet: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    if packet is not None:
        dump(runtime / "physical_quantum_qi_corridor_stability_gate_packet.json", packet)
    result = build_physical_quantum_qi_corridor_stability_gate_receipt_ledger(
        runtime_context=ctx(runtime),
        corridor_stability_gate_receipt_ledger_license=license_packet,
    )
    ledger = read_jsonl(runtime / "physical_quantum_qi_corridor_stability_gate_receipt_ledger.jsonl")
    return result.to_dict(), ledger


def assert_ready(label: str, out: dict[str, Any], ledger: list[dict[str, Any]]) -> None:
    assert out["status"] == "PHYSICAL_QUANTUM_QI_CORRIDOR_STABILITY_GATE_RECEIPT_LEDGER_READY", label
    ...
    assert latest["boundary"]["corridor_stability_gate_receipt_only"] is True, label
    assert latest["boundary"]["stability_gate_receipt_only"] is True, label
...
def main() -> int:
    with tempfile.TemporaryDirectory() as td:
        ...
        out, ledger = run(root, "admit", gate_packet("corridor_stability_admit"), lic())
        ...
        assert ledger[-1]["corridor_status"] == "history_aware_admissible_corridor"

        out, ledger = run(root, "hold", gate_packet("corridor_stability_hold"), lic())
        assert_ready("hold", out, ledger)
        assert out["corridor_stability_gate_decision"] == "corridor_stability_hold"
        assert out["probe_count"] == 2

        out, ledger = run(root, "block", gate_packet("corridor_stability_block"), lic())
        ...
        out, ledger = run(root, "chain", gate_packet("corridor_stability_admit"), lic())
        assert_ready("chain_1", out, ledger)
        out, ledger = run(root, "chain", gate_packet("corridor_stability_hold"), lic())
        ...
        bad_status = gate_packet("corridor_stability_hold")
        bad_status["corridor_status"] = "history_aware_admissible_corridor"
        out, ledger = run(root, "bad_status", bad_status, lic())
        assert out["status"] == "PHYSICAL_QUANTUM_QI_CORRIDOR_STABILITY_GATE_RECEIPT_LEDGER_BLOCKED"
        assert "corridor_status_gate_decision_mismatch" in out["blockers"]
        assert ledger == []

        mismatch = gate_packet("corridor_stability_admit")
        mismatch["corridor_segments"][0] = segment("admit_lane", 0, mismatch=True)
        out, ledger = run(root, "mismatch", mismatch, lic())
        assert out["status"] == "PHYSICAL_QUANTUM_QI_CORRIDOR_STABILITY_GATE_RECEIPT_LEDGER_BLOCKED"
        assert "admit_lane_segment_0_instrument_trace_digest_mismatch" in out["blockers"]
        assert ledger == []

        bad_rank = gate_packet("corridor_stability_admit")
        ...
        bad_boundary = gate_packet("corridor_stability_admit")
        bad_boundary["boundary"]["does_not_start_next_cycle"] = False
        out, ledger = run(root, "bad_boundary", bad_boundary, lic())
        assert out["status"] == "PHYSICAL_QUANTUM_QI_CORRIDOR_STABILITY_GATE_RECEIPT_LEDGER_BLOCKED"
        assert "corridor_stability_gate_boundary_does_not_start_next_cycle_missing" in out["blockers"]
        assert ledger == []

        out, ledger = run(root, "missing", None, lic())
        assert out["status"] == "PHYSICAL_QUANTUM_QI_CORRIDOR_STABILITY_GATE_RECEIPT_LEDGER_BLOCKED"
        assert "corridor_stability_gate_packet_missing_or_invalid" in out["blockers"]
        assert ledger == []

        out, ledger = run(root, "license_block", gate_packet("corridor_stability_admit"), lic(receipt_ledger_append_allowed=False))
        assert out["status"] == "PHYSICAL_QUANTUM_QI_CORRIDOR_STABILITY_GATE_RECEIPT_LEDGER_BLOCKED"
        assert "receipt_ledger_append_not_allowed" in out["blockers"]
        assert ledger == []

    print("physical_quantum_qi_corridor_stability_gate_receipt_ledger_v11_9 checks passed")
    return 0
