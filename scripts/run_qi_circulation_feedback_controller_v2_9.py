...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi circulation feedback controller v2.9")
    ...
    result = build_qi_circulation_feedback_controller(
        runtime_context=read_json(args.context),
        feedback_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_CIRCULATION_FEEDBACK_READY" else 1
