...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Write Qi probe scheduler proposal reuse v0.1")
    parser.add_argument("--scheduler-apply", type=pathlib.Path, required=True)
    ...
    result = build_qi_probe_scheduler_proposal_reuse(
        replay_applied_scheduler_state=read_json(args.scheduler_apply),
        reuse_context=read_json(args.context),
    )
    ...
    return 0 if result.reuse_status == "QI_PROBE_SCHEDULER_PROPOSAL_REUSE_READY" else 1
