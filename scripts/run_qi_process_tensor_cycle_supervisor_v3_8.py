...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi process tensor cycle supervisor v3.8")
    ...
    result = build_qi_process_tensor_cycle_supervisor(
        runtime_context=read_json(args.context),
        supervisor_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_PROCESS_TENSOR_CYCLE_SUPERVISOR_READY" else 1
