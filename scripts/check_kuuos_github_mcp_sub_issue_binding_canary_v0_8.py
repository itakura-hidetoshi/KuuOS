...
BASE_SHA = "6067a37352daae5428ee15931ea0512209403b83"
NONCE = "subissue-v08-check-001"
PARENT = 1801
CHILD = 1802
CHILD_ID = 91802
    ...
    def call_tool(self, name: str, arguments: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if name == "issue_read" and args["method"] == "get":
            if args["issue_number"] == PARENT:
                return response({
                    "number": PARENT, "title": REQUEST_TITLE, "state": "open",
                    "body": json.dumps({
        ...
        if name == "issue_write" and args["method"] == "create":
            ...
            return response({"id": str(CHILD_ID), "url": f"https://github.com/itakura-hidetoshi/KuuOS/issues/{CHILD}"})
        if name == "sub_issue_write" and args["method"] == "add":
            assert self.child is not None
            self.parent_children = [{
                "id": CHILD_ID, "number": CHILD,
                "title": self.child["title"], "state": "open",
            }]
...
def main() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        (root / "github_mcp_sub_issue_binding_canary_plan_v0_8.json").write_text(json.dumps(plan()))
        (root / "github_mcp_sub_issue_binding_canary_authority_v0_8.json").write_text(json.dumps(authority()))
        transport = MockTransport()
        result = build_github_mcp_sub_issue_binding_canary(
            runtime_context={
        ...
        assert result.parent_subissues_preflight_empty
        assert result.child_created_verified
