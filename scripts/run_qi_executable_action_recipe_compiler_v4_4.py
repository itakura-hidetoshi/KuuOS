...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi executable action recipe compiler v4.4")
    ...
    result = build_qi_executable_action_recipe_compiler(
        runtime_context=read_json(args.context),
        recipe_compiler_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_EXECUTABLE_ACTION_RECIPE_COMPILER_READY" else 1
