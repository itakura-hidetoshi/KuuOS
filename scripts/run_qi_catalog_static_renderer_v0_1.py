...
from runtime.kuuos_runtime_daemon_qi_catalog_static_renderer_v0_1 import build_qi_catalog_static_renderer
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build Qi catalog static renderer v0.1")
    ...
    rendered = build_qi_catalog_static_renderer(
    ...
    if args.write_html is not None:
        ...
        args.write_html.write_text(rendered.html, encoding="utf-8")
    if not args.quiet:
        print(json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True))
    return 0 if rendered.multi_dashboard_index_ready else 1
