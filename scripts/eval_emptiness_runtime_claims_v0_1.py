...
def claim_from_dict(data: dict[str, Any]) -> Any:
    return runtime.EmptinessClaim(**data)
...
def evaluate_file(path: pathlib.Path) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for item in data.get("claims", []):
        ...
        out = runtime.evaluate_emptiness(claim_from_dict(item.get("claim", {})))
...
def main() -> int:
    ...
    parser.add_argument("--input", default="specs/emptiness_runtime_claims_v0_1.json")
    ...
    if args.json:
        ...
        if errors:
            for err in errors:
                print("ERROR:", err)
            return 1
        print("PASS: Emptiness runtime claim evaluation completed")
    return 0
