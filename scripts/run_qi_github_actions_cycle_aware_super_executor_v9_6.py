...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions cycle-aware super executor v9.6")
    ...
    result = build_qi_github_actions_cycle_aware_super_executor(
        runtime_context=read_json(args.context),
        cycle_aware_super_executor_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_CYCLE_AWARE_SUPER_EXECUTOR_READY" else 1
