...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions reentry merge supercycle v10.3")
    ...
    result = build_qi_github_actions_reentry_merge_supercycle(
        runtime_context=read_json(args.context),
        reentry_merge_supercycle_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_REENTRY_MERGE_SUPERCYCLE_READY" else 1
