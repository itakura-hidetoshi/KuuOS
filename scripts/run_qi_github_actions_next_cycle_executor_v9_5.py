...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions next cycle executor v9.5")
    ...
    result = build_qi_github_actions_next_cycle_executor(
        ...
        next_cycle_executor_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_NEXT_CYCLE_EXECUTOR_READY" else 1
