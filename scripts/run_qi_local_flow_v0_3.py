...
from runtime.kuuos_runtime_daemon_qi_local_flow_v0_3 import build_qi_local_flow
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi local flow v0.3")
    ...
    result = build_qi_local_flow(
        runtime_context=read_json(args.context),
        flow_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.flow_status in {"QI_LOCAL_FLOW_APPLIED", "QI_LOCAL_FLOW_REPLAYED", "QI_LOCAL_FLOW_IDLE"} else 1
