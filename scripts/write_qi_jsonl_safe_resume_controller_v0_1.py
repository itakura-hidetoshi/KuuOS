...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Write Qi JSONL safe resume controller result v0.1")
    ...
    parser.add_argument("--desired-start-tick", type=int, required=True)
    parser.add_argument("--desired-tick-count", type=int, required=True)
    ...
    result = run_qi_jsonl_safe_resume_controller(
        ...
        desired_start_tick=args.desired_start_tick,
        desired_tick_count=args.desired_tick_count,
        resume_context=normalize_dict(read_json(args.context)),
    )
