...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions policy reentry adapter v9.9")
    ...
    result = build_qi_github_actions_policy_reentry_adapter(
        runtime_context=read_json(args.context),
        policy_reentry_adapter_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_POLICY_REENTRY_ADAPTER_READY" else 1
