...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi circulation trajectory adaptor v3.3")
    ...
    result = build_qi_circulation_trajectory_adaptor(
        runtime_context=read_json(args.context),
        adaptor_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_CIRCULATION_TRAJECTORY_ADAPTOR_READY" else 1
