...
BUILDER = ROOT / "scripts" / "build_dependent_origination_runtime_attestation_v0_1.py"
ATTEST = ROOT / "specs" / "dependent_origination_runtime_attestation_v0_1.generated.json"
...
def main() -> int:
    ...
    worm = read_json(WORM)
    root = chain_root()
    if att.get("authority_note") != "dependent_origination_runtime_attestation_is_integrity_summary_only":
    ...
    if errors:
        for e in errors:
            print("ERROR:", e)
        return 1
    print("PASS: Dependent Origination runtime attestation checked")
    return 0
