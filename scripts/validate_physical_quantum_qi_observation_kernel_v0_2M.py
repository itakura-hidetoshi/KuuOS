#!/usr/bin/env python3
"""Validate Physical Quantum Qi Observation Kernel packet v0.2M."""
...
DEFAULT_PACKET = ROOT / "examples" / "physical_quantum_qi_observation_kernel_packet_v0_2M.json"
...
def main() -> int:
    ...
    if errors:
        print("Physical Quantum Qi Observation Kernel v0.2M validation failed:")
        for err in errors:
            print(f"- {err}")
        print(json.dumps({"candidate": result}, indent=2, ensure_ascii=False, sort_keys=True))
        return 1

    print("Physical Quantum Qi Observation Kernel v0.2M validation passed.")
    print(json.dumps({"candidate": result}, indent=2, ensure_ascii=False, sort_keys=True))
    return 0
