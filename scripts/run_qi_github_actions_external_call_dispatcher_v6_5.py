...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions external call dispatcher v6.5")
    ...
    result = build_qi_github_actions_external_call_dispatcher(
        runtime_context=read_json(args.context),
        external_call_dispatcher_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_EXTERNAL_CALL_DISPATCHER_READY" else 1
