...
CLI = ROOT / "scripts" / "run_qi_operator_surface_baseline_established_packet_v0_1.py"

DECLARATION = {
    "declaration_status": "QI_OPERATOR_NAVIGATION_CHAIN_FINAL_DECLARATION_READY",
    "declaration_id": "qi-nav-chain-final-test-0001",
...
CTX = {
    "additive_only_future_required": True,
    "operator_surface_baseline_established_enabled": True,
...
def run_case(root: pathlib.Path, declaration: dict, ctx: dict, name: str) -> tuple[int, dict]:
    declaration_path = root / f"{name}_declaration.json"
    ctx_path = root / f"{name}_context.json"
    out = root / f"{name}_established.json"
    dump(declaration_path, declaration)
    dump(ctx_path, ctx)
    completed = subprocess.run([
        sys.executable,
        str(CLI),
        "--chain-declaration", str(declaration_path),
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        rc, good = run_case(root, DECLARATION, CTX, "good")
        if rc != 0:
            errors.append("good_returncode_mismatch")
        if good.get("established_status") != "QI_OPERATOR_SURFACE_BASELINE_ESTABLISHED_PACKET_READY":
            errors.append("good_status_mismatch")
        if good.get("operator_surface_baseline_established") is not True:
            errors.append("baseline_established_not_true")
        ...
        if not good.get("established_packet_id"):
            errors.append("established_packet_id_missing")
        ...
        missing = {**DECLARATION, "release_marker_hash": None}
        rc, bad = run_case(root, missing, CTX, "missing")
        if rc != 1:
            errors.append("missing_returncode_mismatch")
        if "release_marker_hash_missing" not in bad.get("established_blockers", []):
            errors.append("release_marker_hash_blocker_missing")
        if bad.get("operator_surface_baseline_established") is not False:
            errors.append("bad_established_not_false")

        not_additive = {**CTX, "additive_only_future_required": False}
        rc, blocked = run_case(root, DECLARATION, not_additive, "blocked")
        if rc != 1:
            errors.append("blocked_returncode_mismatch")
        if "additive_only_future_required_not_true" not in blocked.get("established_blockers", []):
    ...
    if errors:
        for error in errors:
            print("ERROR:", error)
        return 1
    print("PASS: Qi operator surface baseline established packet check")
    return 0
