...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions terminal scheduler executor v10.9")
    ...
    result = build_qi_github_actions_terminal_scheduler_executor(
        runtime_context=read_json(args.context),
        terminal_scheduler_executor_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_TERMINAL_SCHEDULER_EXECUTOR_READY" else 1
