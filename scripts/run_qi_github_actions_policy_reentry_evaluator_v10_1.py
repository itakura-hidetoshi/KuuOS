...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions policy reentry evaluator v10.1")
    ...
    result = build_qi_github_actions_policy_reentry_evaluator(
        runtime_context=read_json(args.context),
        policy_reentry_evaluator_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_POLICY_REENTRY_EVALUATOR_READY" else 1
