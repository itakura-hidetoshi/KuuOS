...
def main() -> int:
    ...
    result = build_indra_qi_process_tensor_world_assimilation_v0_6(
        runtime_context=read_json(args.context),
        assimilation_plan=read_json(args.plan),
        assimilation_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "INDRA_QI_PROCESS_TENSOR_WORLD_ASSIMILATION_V0_6_READY" else 1
