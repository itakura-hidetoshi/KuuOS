...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions external call result adapter v6.4")
    ...
    result = build_qi_github_actions_external_call_result_adapter(
        runtime_context=read_json(args.context),
        result_adapter_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_EXTERNAL_CALL_RESULT_ADAPTER_READY" else 1
