...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi loop adapt v1.0")
    ...
    result = build_qi_loop_adapt(
        runtime_context=read_json(args.context),
        pt_packet=read_json(args.pt),
        adapt_license_packet=read_json(args.license),
    )
    ...
    return 0 if result.status in {"QI_LOOP_ADAPT_APPLIED", "QI_LOOP_ADAPT_IDLE"} else 1
