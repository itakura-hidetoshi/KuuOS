...
def _make_case(
    *,
    ...
    verification_required = any(stage != STAGE_NOT_RUN for stage in stages[4:])
    ...
    return seal(
        {
            ...
            "source_commit_sha": _git_sha(seed),
            "candidate_digest": _digest(seed),
            "patch_artifact_digest": _digest(seed + 1),
            "generation_receipt_digest": _digest(seed + 2),
            "application_receipt_digest": (
                _digest(seed + 3) if application_required else None
            ),
            "verification_execution_receipt_digest": (
                _digest(seed + 4) if verification_required else None
            ),
            "independent_verification_receipt_digest": (
                _digest(seed + 5) if independent_required else None
            ),
            "provider_id": "gpt",
            "model_id": "model-v1",
...
def _successor_cases() -> list[dict[str, Any]]:
    return [
        _make_case(
            case_id="case-pass",
            seed=4,
...
def build_fixture() -> dict[str, Any]:
    baseline_bundle = _make_source_bundle(
    ...
    successor_bundle = _make_source_bundle(
        cases=_successor_cases(),
        ...
        window_end=210,
        request_epoch=220,
        evaluation_epoch=225,
    )
    baseline_result = build_codeai_generated_patch_error_baseline_replay_evaluation(
        dataset=baseline_bundle[0],
        request=baseline_bundle[1],
        policy=baseline_bundle[2],
    )
    successor_result = build_codeai_generated_patch_error_baseline_replay_evaluation(
        dataset=successor_bundle[0],
        request=successor_bundle[1],
        policy=successor_bundle[2],
    )
