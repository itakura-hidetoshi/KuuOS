...
RUNTIME_PATH = ROOT / "examples" / "dependent_origination_runtime_v0_1.py"
DEFAULT_IN = ROOT / "specs" / "dependent_origination_runtime_claims_v0_1.json"
DEFAULT_OUT = ROOT / "specs" / "dependent_origination_runtime_audit_events_v0_1.generated.jsonl"

spec = importlib.util.spec_from_file_location("dependent_origination_runtime_v0_1", RUNTIME_PATH)
if spec is None or spec.loader is None:
    raise RuntimeError("failed to load dependent origination runtime module")
...
def make_event(claim_id: str, claim_data: dict[str, Any]) -> dict[str, Any]:
    result = runtime.evaluate_dependent_origination(runtime.DependentOriginationClaim(**claim_data))
    payload = {
        "runtime": "dependent_origination_runtime_v0_1",
        "claim_id": claim_id,
        "status": result["status"],
        "reason": result["reason"],
        "principle": result["principle"],
        "self_origin_allowed": result["self_origin_allowed"],
        "independent_essence_allowed": result["independent_essence_allowed"],
