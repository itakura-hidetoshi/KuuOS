...
def main() -> int:
    ...
    result = build_indra_qi_causal_feedback_bridge_v0_3(
        runtime_context=read_json(args.context),
        feedback_plan=read_json(args.plan),
        feedback_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "INDRA_QI_CAUSAL_FEEDBACK_BRIDGE_V0_3_READY" else 1
