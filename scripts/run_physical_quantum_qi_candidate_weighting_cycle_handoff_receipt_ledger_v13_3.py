...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Physical Quantum Qi candidate weighting cycle handoff receipt ledger v13.3")
    ...
    result = build_physical_quantum_qi_candidate_weighting_cycle_handoff_receipt_ledger(
        ...
        candidate_weighting_cycle_handoff_receipt_ledger_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "PHYSICAL_QUANTUM_QI_CANDIDATE_WEIGHTING_CYCLE_HANDOFF_RECEIPT_LEDGER_READY" else 1
