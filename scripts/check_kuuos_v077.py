...
from runtime.kuuos_v077_a import verify_committed_memory_state
from runtime.kuuos_v077_d import verify_rolled_back_memory_state
...
def main() -> int:
    ...
    commit_tamper = replace(
        commit_receipt,
        selected_kernel_digest="tampered-kernel",
    )
    commit_blocked = verify_committed_memory_state(
        committed_state,
        commit_tamper,
    )
    assert commit_blocked.status == BLOCKED
    assert "commit_receipt_digest_mismatch" in commit_blocked.issues
    assert "commit_observed_payload_mismatch" in commit_blocked.issues
    ...
    rollback_blocked = verify_rolled_back_memory_state(
        restored_state,
        restored_ledger,
        rollback_tamper,
        commit_receipt,
    ...
    assert "rollback_receipt_digest_mismatch" in rollback_blocked.issues
    assert "rollback_observed_payload_mismatch" in rollback_blocked.issues
    ...
    print(json.dumps({
        ...
        "checks": [
            "exact-commit-state-and-payload-binding",
            "commit-authority-consumption",
            "exact-rollback-state-and-payload-binding",
