...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions PR info result bridge v10.2")
    ...
    result = build_qi_github_actions_pr_info_result_bridge(
        runtime_context=read_json(args.context),
        pr_info_result_bridge_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_PR_INFO_RESULT_BRIDGE_READY" else 1
