...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions unified closed loop v10.4")
    ...
    result = build_qi_github_actions_unified_closed_loop(
        runtime_context=read_json(args.context),
        unified_closed_loop_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_UNIFIED_CLOSED_LOOP_READY" else 1
