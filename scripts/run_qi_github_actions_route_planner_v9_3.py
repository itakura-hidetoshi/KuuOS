...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions route planner v9.3")
    ...
    result = build_qi_github_actions_route_planner(
        runtime_context=read_json(args.context),
        route_planner_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_ROUTE_PLANNER_READY" else 1
