...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi executable capability sequence runner v5.0")
    ...
    result = build_qi_executable_capability_sequence_runner(
        runtime_context=read_json(args.context),
        sequence_runner_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_EXECUTABLE_CAPABILITY_SEQUENCE_RUNNER_READY" else 1
