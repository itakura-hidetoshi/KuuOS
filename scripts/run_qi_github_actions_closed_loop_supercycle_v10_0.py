...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions closed-loop supercycle v10.0")
    ...
    result = build_qi_github_actions_closed_loop_supercycle(
        runtime_context=read_json(args.context),
        closed_loop_supercycle_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_CLOSED_LOOP_SUPERCYCLE_READY" else 1
