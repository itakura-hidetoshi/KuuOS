...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions direct executor v5.6")
    ...
    result = build_qi_github_actions_direct_executor(
        runtime_context=read_json(args.context),
        direct_executor_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_DIRECT_EXECUTOR_READY" else 1
