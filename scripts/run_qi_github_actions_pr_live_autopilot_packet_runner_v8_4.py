...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Qi GitHub Actions PR live autopilot packet runner v8.4")
    ...
    result = build_qi_github_actions_pr_live_autopilot_packet_runner(
        runtime_context=read_json(args.context),
        autopilot_packet_runner_license=read_json(args.license),
    )
    ...
    return 0 if result.status == "QI_GITHUB_ACTIONS_PR_LIVE_AUTOPILOT_PACKET_RUNNER_READY" else 1
