...
def run_phase_a(runtime_root, root, registry):
    ...
    assert abs(first.transported_short_weight - 0.5) < 1e-6
    assert abs(first.transported_medium_weight - 0.3) < 1e-6
    assert abs(first.transported_long_weight - 0.2) < 1e-6
    ...
    replay = build_horizon_gauge_arbitration(
        runtime_context=arbitration_context(runtime_root),
        source_packets=sources1,
        root_principles_packet=root,
        adapter_registry=registry,
        arbitration_plan=plan1,
        arbitration_license=license_packet(plan1, sources1, root, registry, current),
    )
    ...
    assert any(abs(a - b) > 1e-6 for a, b in zip(weights, (0.5, 0.3, 0.2)))
