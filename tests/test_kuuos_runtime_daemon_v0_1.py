...
class KuuOSRuntimeDaemonTests(unittest.TestCase):
    def test_daemon_runs_until_max_ticks(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(Path(result.yinyang_polarity_result_path).is_file())
            ...
            yy_result = load(Path(result.yinyang_polarity_result_path))
            ...
            self.assertFalse(yy_result["grants_execution_authority"])
    ...
    def test_daemon_stops_on_waiting(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result.yinyang_polarity_state, "YIN_REOBSERVE_REQUIRED")
