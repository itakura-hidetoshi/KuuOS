...
from runtime.kuuos_current_root_sequence_v0_68 import (
    CURRENT_ROOT_STEPS,
    SNAPSHOT_FRONTIER,
    as_markdown,
    sequence_issues,
...
class CurrentRootSequenceV068Test(unittest.TestCase):
    ...
    def test_snapshot_frontier_is_connected(self):
        self.assertEqual("kuuos_self_organization_status_snapshot_v0_68", SNAPSHOT_FRONTIER)
        targets = set(unittest_targets())
        self.assertIn("tests.test_kuuos_self_organization_status_snapshot_v0_68", targets)
        self.assertIn("tests.test_kuuos_current_root_sequence_v0_68", targets)

    def test_markdown_summary(self):
        text = as_markdown()
        self.assertIn("self-organization-status-snapshot-v0-68", text)
        self.assertIn("current-root-sequence-v0-68", text)
