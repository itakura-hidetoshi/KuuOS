...
from runtime.kuuos_current_root_sequence_v0_65 import (
    CURRENT_ROOT_STEPS,
    EXECUTION_FRONTIER,
    as_markdown,
    sequence_issues,
...
class CurrentRootSequenceV065Test(unittest.TestCase):
    ...
    def test_execution_frontier_is_connected(self):
        self.assertEqual("kuuos_self_organization_bounded_execution_v0_64", EXECUTION_FRONTIER)
        targets = set(unittest_targets())
        self.assertIn("tests.test_kuuos_self_organization_bounded_execution_v0_64", targets)
        self.assertIn("tests.test_kuuos_current_root_sequence_v0_65", targets)

    def test_markdown_summary(self):
        text = as_markdown()
        self.assertIn("self-organization-bounded-execution-v0-64", text)
        self.assertIn("current-root-sequence-v0-65", text)
