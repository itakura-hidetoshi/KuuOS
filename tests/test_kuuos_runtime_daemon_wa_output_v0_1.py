...
RAW = {
    "cycle_id": "wa-output-001",
    "next_cycle_id": "wa-output-002",
    "generated_at_utc": "2026-05-22T00:00:00+00:00",
    "dispatched_at_utc": "2026-05-22T00:00:01+00:00",
    "candidate_only": True,
...
class DaemonWaOutputTests(unittest.TestCase):
    def test_daemon_writes_wa_output_and_posture(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(Path(result.wa_function_result_path).is_file())
            ...
            wa_result = load(Path(result.wa_function_result_path))
            self.assertEqual(daemon_result["wa_function_result_path"], result.wa_function_result_path)
            self.assertEqual(daemon_result["recommended_runtime_posture"], "CONTINUE_HARMONIZED")
            self.assertEqual(wa_result["recommended_runtime_posture"], "CONTINUE_HARMONIZED")
            self.assertFalse(wa_result["grants_execution_authority"])
