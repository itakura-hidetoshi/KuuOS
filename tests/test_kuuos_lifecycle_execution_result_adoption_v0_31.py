...
from runtime.kuuos_lifecycle_governance_execution_result_adoption_v0_31 import (
    ADOPTED, DEFERRED, REJECTED, Rec, adoption_digest, make_adoption,
...
def evidence(src, **kw):
    ...
    data = dict(
        ...
        result_review_record_freshness_digest="r" * 64,
        result_review_record_fresh=True,
        adoption_receipt_digest="d" * 64,
        adoption_consistency_receipt_digest="c" * 64,
        adoption_consistency_valid=True,
        completion_scope_receipt_digest="s" * 64,
        completion_scope_valid=True,
        adoption_confirmed=True,
        deferral_reason_digest="",
        ...
        completion_review_deadline_at_epoch_seconds=t + 63,
    )
    data.update(kw)
...
def adoption(src, ev, **kw):
    ...
    data = dict(
        ...
        completion_review_route_digest=ev.completion_review_route_digest,
        completion_review_deadline_at_epoch_seconds=t + 63,
        adoption_confirmed=ev.adoption_confirmed,
        deferral_reason_digest=ev.deferral_reason_digest,
    )
    data.update(kw)
...
class ExecutionResultAdoptionV031Test(unittest.TestCase):
    def test_adopted_routes_to_completion_review(self):
        ...
        self.assertTrue(art.completion_review_required_next)
        self.assertFalse(art.repository_changed)

    def test_adopter_authority_failure_defers(self):
        src = source(); ev = evidence(src, adopter_authority_verified=False); ad = adoption(src, ev)
        art = verify_artifact(*args(src, ev, ad, policy()))
        self.assertEqual(DEFERRED, art.status)
        self.assertFalse(art.completion_review_required_next)
    ...
    def test_completion_route_swap_is_rejected(self):
        ...
        ad = refresh_adoption(adoption(src, ev), completion_review_route_digest="bad-route")
        art = verify_artifact(*args(src, ev, ad, policy()))
        self.assertEqual(REJECTED, art.status)
