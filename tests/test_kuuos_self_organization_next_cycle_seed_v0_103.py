...
from runtime.kuuos_self_organization_next_cycle_seed_v0_103 import (
...
class NextCycleSeedV0103Test(unittest.TestCase):
    def test_valid(self):
    ...
    def test_path(self):
        self.assertEqual(
            "status/self_organization_next_cycle_seed_v0_103.json",
            NEXT_CYCLE_SEED_PATH,
        )
    ...
    def test_seed_fields(self):
        ...
        self.assertEqual("self_organization_candidate_queue_seed", data["seed_scope"])
        self.assertEqual("status/self_organization_candidate_queue_v0_104.json", data["next_artifact"])
        self.assertEqual("v0.104", data["next_stage"])
