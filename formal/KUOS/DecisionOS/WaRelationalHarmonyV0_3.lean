...
structure WaEndorsementGate where
  ...
  sourcePluralIdentityPreserved : Bool
  ...
  identityRequired : sourcePluralIdentityPreserved = true
...
theorem endorsedWa_preserves_plural_selected_identity
    (gate : WaEndorsementGate) :
    gate.sourcePluralIdentityPreserved = true := by
...
theorem waPlurality_preserves_stakeholder_sections
    (boundary : WaPluralityBoundary) :
    boundary.stakeholderSectionsPreserved = true := by
  exact boundary.stakeholderRequired


theorem waPlurality_preserves_veto_appeal_history
    (boundary : WaPluralityBoundary) :
