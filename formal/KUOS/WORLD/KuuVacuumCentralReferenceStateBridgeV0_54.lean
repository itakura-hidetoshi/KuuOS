...
def vacuumRelativeEntropyLocal
    (Central : WorldKuuVacuumCentralReferenceStateBridge K)
    (region : W.Region) : ENNReal :=
  E.localRelativeEntropy region

/-- Global Araki entropy understood relative to the central vacuum state. -/
def vacuumRelativeEntropyGlobal
    (Central : WorldKuuVacuumCentralReferenceStateBridge K) : ENNReal :=
...
theorem central_reference_normalized :
  ...
  change K.vacuumState 1 = 1
  exact K.vacuumState_normalized

theorem central_reference_positive (observable : B.A) :
    0 ≤ (Central.centralReferenceState (star observable * observable)).re := by
  change 0 ≤ (K.vacuumState (star observable * observable)).re
  exact K.vacuumState_positive observable
...
theorem central_reference_gauge_invariant
  ...
  change K.vacuumState (K.gaugeAction gauge observable) =
    K.vacuumState observable
  exact K.vacuumState_gauge_invariant gauge observable
...
theorem os_form_is_central_two_point
  ...
  change K.osReflectionForm left right =
    K.vacuumState
      (star (Central.positiveTimeObservableToAlgebra left) *
        Central.positiveTimeObservableToAlgebra right)
  exact Central.osCorrelationBinding left right
...
theorem os_central_correlation_nonnegative
  ...
  rw [← os_form_is_central_two_point Central observable observable]
  exact K.osReflectionPositive observable
...
theorem modular_reference_is_central (observable : B.A) :
  ...
  change R.referenceState observable = K.vacuumState observable
  exact Central.modularReferenceIsVacuum observable
...
theorem central_reference_modular_invariant
  ...
  calc
    Central.centralReferenceState (M.modularFlow time observable) =
      ...
      (modular_reference_is_central Central
    ...
    _ = Central.centralReferenceState observable :=
      modular_reference_is_central Central observable
...
theorem excitation_inner_product_is_two_point
  ...
  change inner ℂ
      ...
      (M.representation right K.kuuVacuum) =
    K.vacuumState (star left * right)
  exact Central.excitationCorrelationBinding left right

theorem vacuum_is_identity_excitation :
    Central.excitationVector (1 : B.A) = K.kuuVacuum := by
  change M.representation (1 : B.A) K.kuuVacuum = K.kuuVacuum
  simp
...
theorem excitation_modular_covariance
  ...
  calc
    M.modularUnitary time (Central.excitationVector observable) =
      ...
      change M.modularUnitary time
          (M.representation observable K.kuuVacuum) =
        M.representation (M.modularFlow time observable)
          (M.modularUnitary time K.kuuVacuum)
      exact M.modularUnitary_implements time observable K.kuuVacuum
    _ = Central.excitationVector (M.modularFlow time observable) := by
      rw [WorldKuuVacuumOSHilbertCompletionBridge.modular_vacuum_invariant K time]
      rfl
...
theorem excitation_physical_time_covariance
  ...
  calc
    K.physicalTimeUnitary time (Central.excitationVector observable) =
      ...
      change K.physicalTimeUnitary time
          (M.representation observable K.kuuVacuum) =
        M.representation (K.physicalTimeFlow time observable)
          (K.physicalTimeUnitary time K.kuuVacuum)
      exact K.physicalTimeUnitary_implements time observable K.kuuVacuum
    _ = Central.excitationVector (K.physicalTimeFlow time observable) := by
      rw [WorldKuuVacuumOSHilbertCompletionBridge.physical_vacuum_invariant K time]
      rfl
...
theorem excited_state_normalized
  ...
  have hweight : K.vacuumState (star creator * creator) ≠ 0 := by
    exact nonzeroWeight
  change
    K.vacuumState (star creator * 1 * creator) /
        K.vacuumState (star creator * creator) = 1
  simp [hweight]
...
theorem vacuum_relative_entropy_data_processing
  ...
  change E.localRelativeEntropy smaller ≤ E.localRelativeEntropy larger
  exact E.localDataProcessing included
...
theorem central_reference_exactly_recovered (observable : B.A) :
  ...
  calc
    ...
    _ = R.referenceState observable :=
      WorldPetzRecoverySufficiencyBridge.referenceState_exactly_recovered
        P observable
    _ = Central.centralReferenceState observable :=
      modular_reference_is_central Central observable
...
theorem vacuum_recovery_entropy_equality :
  ...
  change P.coarseRelativeEntropy = E.globalRelativeEntropy
  exact P.entropyEquality
