...
/-!
Kū–Indra WORLD vacuum central-reference-state bridge v0.50.

The completed-Hilbert-space vacuum from v0.49 is promoted from an isolated
analytic layer to the common reference state tying together reflection
positivity, vacuum correlations, modular theory, relative entropy, Petz
recovery, and excitation vectors.

"Central" means a common organizing reference. It does not claim that the
vacuum vector or its state belongs to the algebraic center.
-/
...
structure WorldKuuVacuumCentralReferenceStateBridge
  ...
  relativeEntropyUsesVacuumReferenceProof :
    relativeEntropyUsesVacuumReferenceClaim
  petzRecoveryUsesVacuumReferenceClaim : Prop
  petzRecoveryUsesVacuumReferenceProof :
    petzRecoveryUsesVacuumReferenceClaim
  excitationSectorGeneratedFromVacuumClaim : Prop
  excitationSectorGeneratedFromVacuumProof :
    excitationSectorGeneratedFromVacuumClaim
  ...
  noRuntimeVacuumCorrelationComputation :
    runtimeComputesVacuumCorrelations = false
  noRuntimeVacuumRelativeEntropyComputation :
    runtimeComputesRelativeEntropyFromVacuum = false
  noRuntimeVacuumRecoveryConstruction :
    runtimeConstructsPetzRecoveryFromVacuum = false
  noRuntimeExcitationVectorConstruction :
    runtimeBuildsExcitationHilbertVectors = false
  ...
  vacuumNotClaimedAlgebraicallyCentralProof :
    vacuumNotClaimedAlgebraicallyCentral
  ...
  relativeEntropyNotOntologicalDistanceProof :
    relativeEntropyNotOntologicalDistance
...
variable (U : WorldKuuVacuumCentralReferenceStateBridge K)

/-- The v0.49 vacuum as the common Hilbert-space reference vector. -/
def centralVacuum : M.H := K.kuuVacuum

/-- Excitation vector generated from the common vacuum reference. -/
def excitationVector (observable : B.A) : M.H :=
  M.representation observable U.centralVacuum

/-- Vacuum one-point function. -/
def vacuumExpectation (observable : B.A) : ℂ :=
  K.vacuumState observable

/-- Vacuum two-point correlation anchored at the same reference state. -/
def vacuumTwoPointCorrelation (left right : B.A) : ℂ :=
  K.vacuumState (left * right)

/-- Relative entropy read through the vacuum-reference state pair. -/
def localVacuumRelativeEntropy (region : W.Region) : ENNReal :=
  E.localRelativeEntropy region

/-- Global relative entropy read through the vacuum-reference state pair. -/
def globalVacuumRelativeEntropy : ENNReal :=
  E.globalRelativeEntropy

/-- The recovered channel already supplied by the v0.35 Petz bridge. -/
noncomputable def vacuumRecoveredChannel : B.A →ₗ[ℂ] B.A :=
  P.recoveredChannel
...
theorem reflection_positive_anchor
...
theorem recovered_channel_unital :
    U.vacuumRecoveredChannel 1 = 1 := by
...
theorem unified_reference_receipts :
    U.vacuumCorrelationClaim ∧
    U.reflectionToVacuumClaim ∧
  ...
  ⟨U.vacuumCorrelationProof,
    U.reflectionToVacuumProof,
...
theorem runtime_grants_no_central_reference_authority :
    ...
    U.runtimeExecutesModularFlow = false ∧
    U.runtimeUpdatesWorld = false :=
  ⟨U.noRuntimeVacuumCorrelationComputation,
    ...
    U.noRuntimeModularExecution,
    U.noRuntimeWorldUpdate⟩

theorem central_reference_boundary_preserved :
    U.centralMeansOrganizingReference ∧
    U.vacuumNotClaimedAlgebraicallyCentral ∧
    U.vacuumNotTruthAuthority ∧
    U.excitationNotTruthAuthority ∧
    U.recoveryNotWorldOverwrite ∧
    U.relativeEntropyNotOntologicalDistance ∧
    U.modularTimeNotPhysicalTime ∧
    U.worldNotIdentifiedWithVacuum ∧
    U.multiWorldNoncollapsePreserved ∧
    U.twoTruthsGapPreserved :=
  ⟨U.centralMeansOrganizingReferenceProof,
    U.vacuumNotClaimedAlgebraicallyCentralProof,
    U.vacuumNotTruthAuthorityProof,
    U.excitationNotTruthAuthorityProof,
    U.recoveryNotWorldOverwriteProof,
    U.relativeEntropyNotOntologicalDistanceProof,
    U.modularTimeNotPhysicalTimeProof,
    U.worldNotIdentifiedWithVacuumProof,
    U.multiWorldNoncollapseProof,
    U.twoTruthsGapProof⟩
