...
`FreeMonoid Event` is definitionally a finite list of events, with multiplication
implemented by list concatenation, and `SingleObj` turns that monoid into a
one-object category.  Composition is reversed multiplication in `SingleObj`,
exactly matching the KuuOS convention that the left history acts after the
right history:
...
def toFreeHistoryFunctor
  ...
  obj := fun _ => State
  map := fun word x => H.eval (FreeMonoid.toList word) x
  map_id := by
    intro X
    funext x
