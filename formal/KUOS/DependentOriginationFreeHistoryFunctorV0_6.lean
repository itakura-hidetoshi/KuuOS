...
`FreeMonoid Event` is mathlib's synonym of `List Event`, with concatenation as
multiplication.  `SingleObj` turns that free monoid into a one-object category.
Composition is reversed multiplication in `SingleObj`, exactly matching the
KuuOS convention that the left history acts after the right history:
