...
Universe and argument bookkeeping is part of this interface.  The source
category uses universes `u` and `v`; the target `Cat.{vH, uH}` uses `uH` and
`vH`.  Therefore `RawHigherContextualSystem` is instantiated with universe
application syntax `.{u, v, uH, vH}`.  These levels are not term-level named
arguments.  By contrast, `W` is genuine localization data and is passed with
`(W := W)` only to declarations whose types actually depend on it.  This
separation prevents elaboration failures from being mistaken for failures of
the mathematical universal-property statements themselves.
...
theorem higherWeakLocalizationExistence_of_universalPrinciple
    (hU : HigherWeakLocalizationUniversalPrinciple.{u, v, uH, vH} (W := W)) :
