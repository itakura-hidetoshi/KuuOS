...
subsystem versionは独立しています。ObserveOS v0.7、VerifyOS v0.13、PlanOS v1.23、DecisionOS v0.6、MemoryOS v1.00、self-organization v0.113を、一つの連続version番号として扱いません。
...
最新の機能統合はPR #1281、VerifyOS v0.13 Sequential Epistemic Observation Verification Handoffです。機能frontierの基準commitは`a2423f9d9da06eeab9c560cd3475e48bfc670f35`です。
...
| VerifyOS | v0.13 independent verification handoff | 統合済み、専用CI、継続検証 |
...
ObserveOS v0.7とVerifyOS v0.13はこのprofile rootへ暗黙に追加せず、各専用workflow、Evidence Cycle累積runner、versioned formal aggregateで独立検証します。
...
次の変更時には専用CI、Evidence Cycle累積runner、repository formal baselineを再検証します。
...
- verification completed / debt stateの遷移変更
- current-effectまたはauthority-negative predicateの変更

VerifyOS v0.13の成功を、独立再計算の完了、因果確証、経験的真理、WORLD write authority、policy activation、execution permissionとして扱いません。
...
README、ROADMAP、structured ObserveOS／VerifyOS indexを、self-organization v0.113、ObserveOS v0.7、VerifyOS v0.13、PlanOS v1.23、DecisionOS v0.6、MemoryOS v1.00へ同期します。
...
### VerifyOS independent evidence execution

VerifyOS v0.13はhandoff準備まで統合済みです。次段階で独立検証を実行する場合は、handoff requestとは別receiptとして、少なくとも次を構成します。

- preserved evidence snapshotからの独立再計算
- confidence sequence、conformal、ADWIN artifactの再現または不一致記録
- minimum reproduction attemptsとactual attemptsのexact accounting
- preregistered acceptance criteriaに対する結果
- verifierとreviewerの責任lineage
- completed / inconclusive / contradicted / additional-evidence-requiredの分離
- verification resultとtruth authority、WORLD mutation、policy activation、executionの分離

この候補だけでVerifyOS v0.14または既存v0.11 verification-performing lineの置換を自動確定しません。
