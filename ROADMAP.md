...
subsystem versionは独立しています。ObserveOS v0.7、VerifyOS v0.14、PlanOS v1.23、DecisionOS v0.6、MemoryOS v1.00、self-organization v0.113を、一つの連続version番号として扱いません。
...
最新の機能統合はPR #1283、**VerifyOS v0.14 Independent Evidence Verification v0.1**です。機能frontierの基準commitは`461f37e02f2439bd6fd4f2c21e72885d6d944df6`です。
...
次の変更時にはv0.13とv0.14の専用CI、Evidence Cycle累積runner、repository formal baselineを再検証します。
...
- bounded windowまたはreplay closureの変更
- verification completed / debt / reobservation stateの遷移変更
- current-effectまたはauthority-negative predicateの変更

VerifyOS v0.14の成功を、経験的真理、因果確証、WORLD adoption、WORLD write authority、policy activation、execution permissionとして扱いません。
...
README、ROADMAP、structured ObserveOS／VerifyOS indexを、self-organization v0.113、ObserveOS v0.7、VerifyOS v0.14、PlanOS v1.23、DecisionOS v0.6、MemoryOS v1.00へ同期します。
...
### VerifyOS outcome disposition handoff

VerifyOS v0.14は独立検証結果の記録まで統合済みです。次段階で結果を他OSへ渡す場合は、verification receiptとは別の**read-only disposition request**として構成します。

少なくとも次を必要とします。

- canonical v0.14 receiptとoutcomeのexact binding
- `passed / failed / indeterminate`の非混同
- passedからadopt candidateを作る場合の独立authority source
- failedからreject candidateを作る場合のevidence preservationとappeal/review route
- indeterminateからdefer / reobserve candidateを作る場合のopen debt preservation
- source WORLD revision、history、lineage、responsibilityの維持
- replay closureとbounded validity window
- disposition candidateとWORLD mutation、policy activation、executionの分離
...
disposition request != WORLD mutation
```

既存の旧VerifyOS/WORLD adoption系列へ暗黙接続せず、現在のv0.13-v0.14 receipt contractに対する独立adapterとして設計します。

### Reobservation request separation

v0.14の`indeterminate`は`reobservation_required = true`を記録しますが、ObserveOSを自動起動しません。
...
verification resultを学習へ渡す場合は、future-only deltaとして設計します。
...
passedだけをreinforcementへ直結せず、failed、indeterminate、counterevidence、review dissentを保持します。
