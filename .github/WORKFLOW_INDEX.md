...
| `world-four-great-phase-dynamics-v0-59.yml` | Stable WORLD v0.59 Four-Great validation entry point with scoped automatic triggers |

## Integrity guard

`scripts/check_workflow_consolidation_integrity.py` verifies that canonical workflows and retained stable entry points exist, superseded files stay deleted, failure propagation remains enabled, and the migrated validation markers remain present.
...
- a stable externally referenced Actions URL with independently visible validation status
...
6. Delete superseded workflow files immediately after their validation paths are migrated, unless a stable externally referenced entry point is required.
...
9. Give retained stable entry points explicit, documented path filters. Duplicate execution is permitted only when it preserves an independently referenced validation surface.
