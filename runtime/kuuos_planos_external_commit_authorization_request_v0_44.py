...
VERSION = "kuuos_planos_external_commit_authorization_request_v0_44"
SOURCE_VERSION = "kuuos_planos_execution_receipt_v0_43"

EXTERNAL_COMMIT_AUTHORIZATION_REQUEST_BOUNDARY = {
    "request_owned_by_plan_os": True,
    "source_execution_receipt_preserved": True,
    ...
    "external_commit_authorization_request_only": True,
    "external_commit_authorization_requested": True,
    "external_commit_authorization_granted": False,
...
REQUIRED_SOURCE_BOUNDARY_TRUE = (
    ...
    "selected_candidate_bound_to_execution_grant",
...
class ExternalCommitAuthorizationRequestRecord:
    selected_candidate_id: str
    selected_candidate_digest: str
    source_execution_receipt_digest: str
    source_execution_authorization_grant_digest: str
    external_commit_authorization_request_digest: str
    request_scope: str
    ...
    external_commit_authorization_granted: bool = False
...
@dataclass(frozen=True)
class ExternalCommitAuthorizationRequestReceipt:
    version: str
    source_version: str
    status: str
    source_execution_receipt_digest: str
    selected_candidate_id: str
    selected_candidate_digest: str
    external_commit_authorization_request: dict[str, Any] | None
    blockers: list[str]
...
def _source_execution_record(receipt: Mapping[str, Any]) -> Mapping[str, Any]:
    return _m(receipt.get("execution_receipt"))


def _source_blockers(receipt: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    if receipt.get("version") != SOURCE_VERSION:
        blockers.append("source_execution_receipt_version_invalid")
    if receipt.get("status") != "PLANOS_EXECUTION_RECEIPT_READY":
        blockers.append("source_execution_receipt_not_ready")
    ...
    record = _source_execution_record(receipt)
    if not record:
...
def build_external_commit_authorization_request(
    *,
    execution_receipt: Mapping[str, Any],
) -> ExternalCommitAuthorizationRequestReceipt:
    ...
    record = _source_execution_record(source)
    if not selected_id:
    ...
    if selected_id and selected_digest and not blockers:
        request_obj = ExternalCommitAuthorizationRequestRecord(
            selected_candidate_id=selected_id,
            selected_candidate_digest=selected_digest,
            source_execution_receipt_digest=str(source.get("receipt_digest", "")),
            source_execution_authorization_grant_digest=str(record.get("source_execution_authorization_grant_digest", "")),
            external_commit_authorization_request_digest=_digest(
                {
                    "source_execution_receipt_digest": source.get("receipt_digest"),
                    "source_execution_authorization_grant_digest": record.get("source_execution_authorization_grant_digest"),
                    "selected_candidate_id": selected_id,
                    "selected_candidate_digest": selected_digest,
                    "scope": "external_commit_authorization_request_only",
                }
            ),
            request_scope="external_commit_authorization_request_only",
        )
        request = request_obj.to_dict()

    status = "PLANOS_EXTERNAL_COMMIT_AUTHORIZATION_REQUEST_READY" if not blockers else "PLANOS_EXTERNAL_COMMIT_AUTHORIZATION_REQUEST_BLOCKED"
    partial = {
        "version": VERSION,
        ...
        "selected_candidate_id": selected_id,
        "selected_candidate_digest": selected_digest,
        "external_commit_authorization_request": request,
        "blockers": list(blockers),
        "boundary": dict(EXTERNAL_COMMIT_AUTHORIZATION_REQUEST_BOUNDARY),
    }
    return ExternalCommitAuthorizationRequestReceipt(receipt_digest=_digest(partial), **partial)


def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print("usage: kuuos_planos_external_commit_authorization_request_v0_44.py EXECUTION_RECEIPT.json", file=sys.stderr)
        return 2
    with open(argv[1], "r", encoding="utf-8") as handle:
        payload = json.load(handle)
    receipt = build_external_commit_authorization_request(execution_receipt=payload)
