...
from runtime import kuuos_current_root_sequence_v0_85 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_86"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
REVIEW_DECISION_FRONTIER = "kuuos_self_organization_review_decision_v0_86"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "review-decision-v0-86",
        "unittest",
        "tests.test_kuuos_self_organization_review_decision_v0_86",
        True,
        "Review decision is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-86",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_86",
        True,
        "Current root sequence v0.86 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if REVIEW_DECISION_FRONTIER != "kuuos_self_organization_review_decision_v0_86":
        issues.append("review_decision_frontier")
    return tuple(issues)
