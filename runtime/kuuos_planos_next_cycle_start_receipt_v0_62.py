...
VERSION = "kuuos_planos_next_cycle_start_receipt_v0_62"
SOURCE_VERSION = "kuuos_planos_next_cycle_admission_grant_v0_61"

NEXT_CYCLE_START_RECEIPT_BOUNDARY = {
    "receipt_owned_by_plan_os": True,
    "source_next_cycle_admission_grant_preserved": True,
    "selected_candidate_bound_to_next_cycle_admission_grant": True,
    ...
    "next_cycle_start_receipt_only": True,
    "next_cycle_admission_requested": True,
    "next_cycle_admission_granted": True,
    "next_cycle_started": True,
    "next_cycle_cycle_closed": False,
}

REQUIRED_SOURCE_BOUNDARY_TRUE = (
    "grant_owned_by_plan_os",
    "source_next_cycle_admission_request_preserved",
    "selected_candidate_bound_to_next_cycle_admission_request",
...
class NextCycleStartRecord:
    ...
    next_cycle_start_digest: str
    start_scope: str = "next_cycle_start_receipt_only"
    ...
    next_cycle_cycle_closed: bool = False
...
class NextCycleStartReceipt:
    ...
    selected_candidate_id: str
    selected_candidate_digest: str
    next_cycle_start_receipt: dict[str, Any] | None
    blockers: list[str]
...
def _source_grant_record(receipt: Mapping[str, Any]) -> Mapping[str, Any]:
    return _m(receipt.get("next_cycle_admission_grant"))


def _source_blockers(receipt: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    if receipt.get("version") != SOURCE_VERSION:
        blockers.append("source_next_cycle_admission_grant_version_invalid")
    if receipt.get("status") != "PLANOS_NEXT_CYCLE_ADMISSION_GRANT_READY":
        blockers.append("source_next_cycle_admission_grant_not_ready")
    ...
    if not receipt.get("receipt_digest"):
        blockers.append("source_next_cycle_admission_grant_digest_missing")
    record = _source_grant_record(receipt)
    if not record:
        blockers.append("source_next_cycle_admission_grant_record_missing")
    if record:
        for field in (
            "memory_overwrite_preserved",
            "memory_overwrite_closeout_preserved",
...
def build_next_cycle_start_receipt(
    *,
    next_cycle_admission_grant: Mapping[str, Any],
) -> NextCycleStartReceipt:
    source = _m(next_cycle_admission_grant)
    ...
    if record:
        if selected_id and str(record.get("selected_candidate_id", "")) != selected_id:
            blockers.append("selected_candidate_id_next_cycle_admission_grant_mismatch")
        if selected_digest and str(record.get("selected_candidate_digest", "")) != selected_digest:
            blockers.append("selected_candidate_digest_next_cycle_admission_grant_mismatch")

    start_receipt = None
    if selected_id and selected_digest and not blockers:
        start_obj = NextCycleStartRecord(
            selected_candidate_id=selected_id,
            selected_candidate_digest=selected_digest,
            source_next_cycle_admission_grant_digest=str(source.get("receipt_digest", "")),
            source_next_cycle_admission_request_digest=str(
                record.get("source_next_cycle_admission_request_digest", "")
            ),
            source_blocker_release_closeout_receipt_digest=str(
            ...
            next_cycle_start_digest=sha({
                "source_next_cycle_admission_grant_digest": source.get("receipt_digest"),
                "source_next_cycle_admission_request_digest": record.get(
                    "source_next_cycle_admission_request_digest"
                ),
                ...
                "selected_candidate_id": selected_id,
                "selected_candidate_digest": selected_digest,
                "scope": "next_cycle_start_receipt_only",
            }),
        )
    ...
    status = (
        ...
        if not blockers
        else "PLANOS_NEXT_CYCLE_START_RECEIPT_BLOCKED"
    )
    partial = {
        "version": VERSION,
        ...
        "selected_candidate_id": selected_id,
        "selected_candidate_digest": selected_digest,
        "next_cycle_start_receipt": start_receipt,
        "blockers": blockers,
        "boundary": dict(NEXT_CYCLE_START_RECEIPT_BOUNDARY),
    }
    return NextCycleStartReceipt(receipt_digest=sha(partial), **partial)


def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print(
            "usage: kuuos_planos_next_cycle_start_receipt_v0_62.py NEXT_CYCLE_ADMISSION_GRANT.json",
            file=sys.stderr,
        )
        return 2
    with open(argv[1], "r", encoding="utf-8") as handle:
        payload = json.load(handle)
    receipt = build_next_cycle_start_receipt(next_cycle_admission_grant=payload)
