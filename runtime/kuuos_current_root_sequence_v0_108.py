...
from runtime import kuuos_current_root_sequence_v0_107 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_108"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
BOUNDED_CHANGE_PLAN_FRONTIER = "kuuos_self_organization_bounded_change_plan_v0_108"
SELF_CHECK_TARGET = "runtime.kuuos_current_root_sequence_v0_108:run_self_check"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "bounded-change-plan-v0-108",
        "unittest",
        "tests.test_kuuos_self_organization_bounded_change_plan_v0_108",
        True,
        "Bounded change plan v0.108 is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-108",
        "callable",
        SELF_CHECK_TARGET,
        True,
        "Current root sequence v0.108 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if BOUNDED_CHANGE_PLAN_FRONTIER != "kuuos_self_organization_bounded_change_plan_v0_108":
