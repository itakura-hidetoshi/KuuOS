...
from runtime import kuuos_current_root_sequence_v0_98 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_99"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
SELECTION_POLICY_FRONTIER = "kuuos_self_organization_selection_policy_v0_99"
SELF_CHECK_TARGET = "runtime.kuuos_current_root_sequence_v0_99:run_self_check"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "selection-policy-v0-99",
        "unittest",
        "tests.test_kuuos_self_organization_selection_policy_v0_99",
        True,
        "Selection policy v0.99 is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-99",
        "callable",
        SELF_CHECK_TARGET,
        True,
        "Current root sequence v0.99 self-check is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if SELECTION_POLICY_FRONTIER != "kuuos_self_organization_selection_policy_v0_99":
