...
VERSION = "kuuos_codeai_reobservation_bounded_continuation_proposal_gate_v0_1"
SCHEMA_VERSION = "v0.1"
PROFILE_VERSION = "CodeAI Reobservation-Bounded Continuation Proposal Gate v0.1"
PREDECESSOR_PROFILE_VERSION = "CodeAI Environment Capsule and Livelock-Efficiency Gate v0.1"
STATUS_READY = "ready"
STATUS_BLOCKED = "blocked"
MODE_PROPOSAL_GATE_ONLY = "reobservation_bounded_continuation_proposal_gate_only"
DISPOSITION_COMPLETED = "reobservation_bounded_continuation_proposal_gate_completed"
...
DECISION_ADMIT = "bounded_continuation_proposal_admitted"
DECISION_HOLD = "bounded_continuation_proposal_held"

REQUEST_DIGEST_FIELD = "codeai_reobservation_bounded_continuation_request_digest"
POLICY_DIGEST_FIELD = "codeai_reobservation_bounded_continuation_policy_digest"
PROPOSAL_DIGEST_FIELD = "codeai_bounded_continuation_proposal_digest"
PACK_DIGEST_FIELD = "codeai_reobservation_bounded_continuation_proposal_gate_pack_digest"
RECEIPT_DIGEST_FIELD = "codeai_reobservation_bounded_continuation_proposal_gate_receipt_digest"
...
BINDING_FIELDS = (
    ...
    "predecessor_gate_pack_digest",
    "predecessor_gate_receipt_digest",
    ...
    "observation_return_contract_digest",
...
class CodeAIReobservationBoundedContinuationProposalGateResult:
    status: str
    issues: tuple[str, ...]
...
def _validate_binding(value: Mapping[str, Any], prefix: str) -> list[str]:
    ...
    specialist_id = value.get("selected_specialist_id")
    if not isinstance(specialist_id, str) or IDENTIFIER.fullmatch(specialist_id) is None:
        issues.append(prefix + "_specialist_id_invalid")
...
def validate_request(request: Mapping[str, Any]) -> list[str]:
    ...
    if not positive_int(request["requested_continuation_round"]):
        issues.append("request_continuation_round_invalid")
...
def validate_policy(policy: Mapping[str, Any]) -> list[str]:
    required = {
        ...
        *{"expected_" + field for field in BINDING_FIELDS},
        "evaluation_epoch",
        "maximum_request_age",
        "maximum_proposal_age",
        "expected_continuation_round",
        "maximum_action_count",
        "allowed_action_kinds",
        *{"maximum_total_" + name for name in RESOURCE_NAMES},
        *{"maximum_proposal_" + name for name in RESOURCE_NAMES},
    ...
    for resource in RESOURCE_NAMES:
        total_field = "maximum_total_" + resource
        proposal_field = "maximum_proposal_" + resource
        if not nonnegative_int(policy[total_field]):
            issues.append("policy_integer_invalid:" + total_field)
        if not nonnegative_int(policy[proposal_field]):
            issues.append("policy_integer_invalid:" + proposal_field)
    for field in (
        "maximum_total_steps",
        "maximum_total_tool_calls",
        "maximum_total_model_calls",
        "maximum_total_token_units",
        "maximum_total_wall_clock_ms",
        "maximum_proposal_steps",
        "maximum_proposal_tool_calls",
        "maximum_proposal_model_calls",
        "maximum_proposal_token_units",
        "maximum_proposal_wall_clock_ms",
    ):
        if not positive_int(policy[field]):
            issues.append("policy_positive_integer_invalid:" + field)
    if not unique_strings(policy["allowed_action_kinds"], nonempty=True):
        issues.append("policy_allowed_action_kinds_invalid")
    elif any(item not in ACTION_KINDS for item in policy["allowed_action_kinds"]):
        issues.append("policy_action_kind_invalid")
    for field in (
        "require_exact_binding",
        "require_predecessor_admitted",
        "require_predecessor_hint_only",
