...
from runtime import kuuos_current_root_sequence_v0_77 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_78"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
README_SURFACE_EXPOSURE_FRONTIER = "kuuos_readme_surface_exposure_v0_78"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "readme-surface-exposure-v0-78",
        "unittest",
        "tests.test_kuuos_readme_surface_status_v0_78",
        True,
        "README surface exposure is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-78",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_78",
        True,
        "Current root sequence v0.78 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if README_SURFACE_EXPOSURE_FRONTIER != "kuuos_readme_surface_exposure_v0_78":
        issues.append("readme_surface_exposure_frontier")
    return tuple(issues)
