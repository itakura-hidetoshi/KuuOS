...
from runtime.kuuos_belief_os_phase_v0_1 import apply_phase_payload
from runtime.kuuos_belief_os_state_v0_1 import (
    build_belief_event,
...
def _result(
    *,
    ...
    value = {
        ...
        "status": status,
        "belief_event_digest": event_id,
        "predecessor_belief_state_digest": predecessor,
        "result_belief_state_digest": state["belief_state_digest"],
        "state": deepcopy(dict(state)),
        "errors": errors,
        "belief_apply_result_digest": "",
    }
    value["belief_apply_result_digest"] = apply_result_digest(value)
    return value


def apply_belief_event(
    state: Mapping[str, Any], event: Mapping[str, Any]
) -> dict[str, Any]:
    state_errors = validate_belief_state(state)
    if state_errors:
        raise ValueError("invalid_state:" + ";".join(state_errors))
    event_id = str(event.get("belief_event_digest", ""))
    predecessor = str(state["belief_state_digest"])
    ...
    next_state["predecessor_belief_state_digest"] = predecessor
    ...
    if target == "commit":
        next_state["belief_version"] += 1
        next_state["completed_revisions"] += 1
        next_state["revision_summaries"] = list(next_state["revision_summaries"]) + [
            {
                "belief_version": next_state["belief_version"],
                "route": next_state["route"],
                "credal_state": deepcopy(next_state["credal_state"]),
                "two_truths": deepcopy(next_state["two_truths"]),
                "qi_process_tensor_digest": next_state["qi_process"][
        ...
        next_state["latest_committed_belief_digest"] = sha(
            {
                "belief_id": next_state["belief_id"],
                "belief_version": next_state["belief_version"],
                "claim_digest": next_state["claim_digest"],
                "credal_state": next_state["credal_state"],
                "two_truths": next_state["two_truths"],
                "qi_process": next_state["qi_process"],
                "route": next_state["route"],
                "commit_event_digest": event_id,
            }
        )

    next_state["belief_state_digest"] = ""
    next_state["belief_state_digest"] = state_digest(next_state)
    next_errors = validate_belief_state(next_state)
    if next_errors:
...
def build_replan_activation_receipt(
    *,
    state: Mapping[str, Any],
    ...
    if errors:
        raise ValueError("invalid_state:" + ";".join(errors))
    if state.get("current_phase") != "commit":
        raise ValueError("belief_not_committed")
    if state.get("pending_replan_activation") is not True:
        raise ValueError("belief_not_pending_activation")
    ...
    receipt = {
        ...
        "committed_belief_digest": state["latest_committed_belief_digest"],
    ...
    receipt["belief_activation_receipt_digest"] = activation_receipt_digest(receipt)
    return receipt
