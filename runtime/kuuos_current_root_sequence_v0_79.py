...
from runtime import kuuos_current_root_sequence_v0_78 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_79"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
SELF_ORGANIZATION_CANDIDATE_QUEUE_FRONTIER = "kuuos_self_organization_candidate_queue_v0_79"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "self-organization-candidate-queue-v0-79",
        "unittest",
        "tests.test_kuuos_self_organization_candidate_queue_v0_79",
        True,
        "Self-organization candidate queue is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-79",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_79",
        True,
        "Current root sequence v0.79 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if SELF_ORGANIZATION_CANDIDATE_QUEUE_FRONTIER != "kuuos_self_organization_candidate_queue_v0_79":
        issues.append("self_organization_candidate_queue_frontier")
    return tuple(issues)
