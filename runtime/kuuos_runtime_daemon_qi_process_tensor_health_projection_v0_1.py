...
def compile_qi_process_tensor_health_projection(
    *,
    ...
    token = _as_bool(invocation_boundary.get("single_tick_invocation_token") or daemon_result.get("single_tick_invocation_token"))
    ...
    if not process_visible:
        ...
        health = "REOBSERVE_REQUIRED"
        action = "reobserve"
        reason = "reentry_plan_requires_reobserve"
        next_receipt = "daemon_qi_process_tensor_reentry_plan_v0_1.json"
        ...
        reason = "reentry_plan_requires_compaction"
        next_receipt = "daemon_qi_process_tensor_reentry_plan_v0_1.json"
        ...
        action = "no_action"
        reason = "manual_runner_already_invoked_one_bounded_tick"
        next_receipt = "daemon_qi_process_tensor_bounded_tick_executor_receipt_v0_1.json"
    elif token and license_decision == "BOUNDED_TICK_LICENSE_GRANTED":
        phase = "QI_SINGLE_TICK_TOKEN_READY"
        health = "HEALTHY_REENTRY_READY"
        action = "invoke_manual_runner"
        reason = "single_tick_invocation_token_ready"
        next_receipt = "daemon_qi_process_tensor_bounded_tick_executor_receipt_v0_1.json"
    elif license_decision == "NO_BOUNDED_TICK_LICENSE":
        phase = "QI_REENTRY_LICENSE_DENIED"
        health = "EXECUTOR_DENIED"
        action = "hold"
        reason = str(license_gate.get("denied_reason") or daemon_result.get("reentry_license_denied_reason") or "bounded_tick_license_denied")
        next_receipt = "daemon_qi_process_tensor_reentry_license_gate_v0_1.json"
    else:
        phase = "QI_PROCESS_TENSOR_REENTRY_UNRESOLVED"
        health = "REOBSERVE_REQUIRED"
        action = "reobserve"
        reason = "reentry_surface_incomplete_or_unresolved"
        next_receipt = None
    ...
    return KuuOSQiProcessTensorHealthProjection(
        ...
        projection_status="QI_PROCESS_TENSOR_HEALTH_PROJECTED",
