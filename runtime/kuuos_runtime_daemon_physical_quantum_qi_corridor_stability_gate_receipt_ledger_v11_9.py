...
GATE_DECISIONS = {
    "corridor_stability_admit",
    "corridor_stability_hold",
    "corridor_stability_block",
}
EXPECTED_STATUS = {
...
class PhysicalQuantumQiCorridorStabilityGateReceiptLedgerResult:
    version: str
    status: str
    packet_id: str
    runtime_root: str
...
def _validate_gate_packet(packet: Mapping[str, Any], blockers: list[str], warnings: list[str]) -> tuple[dict[str, Any], dict[str, int], str]:
    if not packet:
        blockers.append("corridor_stability_gate_packet_missing_or_invalid")
        return {}, {"admitted": 0, "probe": 0, "blocked": 0}, "corridor_stability_block"
    if packet.get("physical_quantum_qi_corridor_stability_gate_considered") is not True:
        blockers.append("corridor_stability_gate_considered_not_true")
    boundary = _m(packet.get("boundary"))
    for name in REQUIRED_GATE_BOUNDARY_FLAGS:
        if boundary.get(name) is not True:
            blockers.append(f"corridor_stability_gate_boundary_{name}_missing")
    decision = str(packet.get("corridor_stability_gate_decision", "corridor_stability_block"))
    if decision not in GATE_DECISIONS:
        blockers.append("corridor_stability_gate_decision_invalid")
        decision = "corridor_stability_block"
    corridor_status = str(packet.get("corridor_status", "history_aware_blocked_corridor"))
    if corridor_status != EXPECTED_STATUS[decision]:
        blockers.append("corridor_status_gate_decision_mismatch")
    ...
    if decision == "corridor_stability_admit" and seen["admit_lane"] <= 0:
        blockers.append("admit_stability_without_admit_lane")
    if decision == "corridor_stability_hold" and (seen["admit_lane"] > 0 or seen["probe_lane"] <= 0):
        blockers.append("hold_stability_requires_probe_lane_only")
    if decision == "corridor_stability_block" and (seen["admit_lane"] > 0 or seen["probe_lane"] > 0):
        blockers.append("block_stability_with_admit_or_probe_lane")
    if not packet.get("corridor_stability_gate_digest"):
        warnings.append("corridor_stability_gate_digest_missing")
    payload = {
        ...
        "source_corridor_stability_gate_digest": str(packet.get("corridor_stability_gate_digest", _sha(dict(packet)))),
...
def build_physical_quantum_qi_corridor_stability_gate_receipt_ledger(
    *,
    runtime_context: Mapping[str, Any],
    corridor_stability_gate_receipt_ledger_license: Mapping[str, Any],
) -> PhysicalQuantumQiCorridorStabilityGateReceiptLedgerResult:
    ctx = _m(runtime_context)
    lic = _m(corridor_stability_gate_receipt_ledger_license)
    blockers: list[str] = []
    warnings: list[str] = []
    root = _root(ctx.get("runtime_root"), blockers)
    packet_path = root / "physical_quantum_qi_corridor_stability_gate_packet.json"
    ledger_path = root / "physical_quantum_qi_corridor_stability_gate_receipt_ledger.jsonl"
    summary_path = root / "physical_quantum_qi_corridor_stability_gate_receipt_summary.json"
    receipt_path = root / "physical_quantum_qi_corridor_stability_gate_receipt_ledger_receipt.json"
    audit_path = root / "physical_quantum_qi_corridor_stability_gate_receipt_ledger_audit.jsonl"

    if ctx.get("physical_quantum_qi_corridor_stability_gate_receipt_ledger_enabled") is not True:
        blockers.append("physical_quantum_qi_corridor_stability_gate_receipt_ledger_enabled_not_true")
    if ctx.get("apply_physical_quantum_qi_corridor_stability_gate_receipt_ledger") is not True:
        blockers.append("apply_physical_quantum_qi_corridor_stability_gate_receipt_ledger_not_true")
    if lic.get("license_status") != "PHYSICAL_QUANTUM_QI_CORRIDOR_STABILITY_GATE_RECEIPT_LEDGER_LICENSE_READY":
        blockers.append("physical_quantum_qi_corridor_stability_gate_receipt_ledger_license_not_ready")
    for name in [
        "corridor_stability_gate_packet_read_allowed",
    ...
    if not blockers:
        ...
        summary = {
            "version": "physical_quantum_qi_corridor_stability_gate_receipt_summary_v11_9",
            "corridor_stability_gate_decision": decision,
            "corridor_status": record["corridor_status"],
            "counts": dict(counts),
            "lane_counts": dict(record["lane_counts"]),
            "latest_record_digest": record["record_digest"],
            "source_corridor_stability_gate_digest": record["source_corridor_stability_gate_digest"],
    ...
    status = "PHYSICAL_QUANTUM_QI_CORRIDOR_STABILITY_GATE_RECEIPT_LEDGER_READY" if not blockers else "PHYSICAL_QUANTUM_QI_CORRIDOR_STABILITY_GATE_RECEIPT_LEDGER_BLOCKED"
    packet_id = "physical-quantum-qi-corridor-stability-gate-receipt-" + _sha({"record": record, "blockers": blockers})[:16]
    receipt = {
        "version": "kuuos_runtime_daemon_physical_quantum_qi_corridor_stability_gate_receipt_ledger_v11_9",
        "status": status,
        "packet_id": packet_id,
        "corridor_stability_gate_decision": decision,
    ...
    return PhysicalQuantumQiCorridorStabilityGateReceiptLedgerResult(
        "kuuos_runtime_daemon_physical_quantum_qi_corridor_stability_gate_receipt_ledger_v11_9",
        status,
        packet_id,
        str(root),
        decision,
