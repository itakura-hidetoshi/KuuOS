...
VERSION = "kuuos_planos_execution_receipt_v0_43"
SOURCE_VERSION = "kuuos_planos_execution_authorization_grant_v0_42"

EXECUTION_RECEIPT_BOUNDARY = {
    "receipt_owned_by_plan_os": True,
    "source_execution_authorization_grant_preserved": True,
    "selected_candidate_bound_to_execution_grant": True,
...
REQUIRED_SOURCE_BOUNDARY_TRUE = (
    ...
    "source_execution_authorization_request_preserved",
    "selected_candidate_bound_to_execution_request",
    ...
    "execution_authorization_request_preserved",
    "execution_authorization_grant_only",
...
def _source_grant_record(receipt: Mapping[str, Any]) -> Mapping[str, Any]:
    return _m(receipt.get("execution_authorization_grant"))
...
def _source_blockers(receipt: Mapping[str, Any]) -> list[str]:
    ...
    if receipt.get("status") != "PLANOS_EXECUTION_AUTHORIZATION_GRANT_READY":
    ...
    if not receipt.get("receipt_digest"):
        blockers.append("source_execution_authorization_grant_digest_missing")
    ...
    if record:
        ...
        if _truthy(record.get("execution_authorization_granted")) is not True:
            blockers.append("source_record_execution_authorization_granted_missing")
        for closed in (
            "execution_ready",
    ...
    *,
    ...
    if record:
        if selected_id and str(record.get("selected_candidate_id", "")) != selected_id:
            blockers.append("selected_candidate_id_execution_grant_mismatch")
        if selected_digest and str(record.get("selected_candidate_digest", "")) != selected_digest:
            blockers.append("selected_candidate_digest_execution_grant_mismatch")

    receipt_record = None
    if selected_id and selected_digest and not blockers:
        receipt_obj = ExecutionReceiptRecord(
            selected_candidate_id=selected_id,
            selected_candidate_digest=selected_digest,
            source_execution_authorization_grant_digest=str(source.get("receipt_digest", "")),
            source_execution_authorization_request_digest=str(record.get("source_execution_authorization_request_digest", "")),
            execution_receipt_digest=_digest(
                {
                    "source_execution_authorization_grant_digest": source.get("receipt_digest"),
                    "source_execution_authorization_request_digest": record.get("source_execution_authorization_request_digest"),
                    "selected_candidate_id": selected_id,
                    "selected_candidate_digest": selected_digest,
                    "scope": "execution_receipt_only",
                }
            ),
            receipt_scope="execution_receipt_only",
        )
        receipt_record = receipt_obj.to_dict()

    status = "PLANOS_EXECUTION_RECEIPT_READY" if not blockers else "PLANOS_EXECUTION_RECEIPT_BLOCKED"
    partial = {
        "version": VERSION,
    ...
    return ExecutionReceipt(receipt_digest=_digest(partial), **partial)


def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print("usage: kuuos_planos_execution_receipt_v0_43.py EXECUTION_GRANT.json", file=sys.stderr)
        return 2
    with open(argv[1], "r", encoding="utf-8") as handle:
        payload = json.load(handle)
    receipt = build_execution_receipt(execution_grant=payload)
