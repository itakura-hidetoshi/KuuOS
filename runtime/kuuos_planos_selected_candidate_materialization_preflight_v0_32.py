...
VERSION = "kuuos_planos_selected_candidate_materialization_preflight_v0_32"
SOURCE_VERSION = "kuuos_planos_selected_candidate_synthesis_receipt_v0_31"

NON_AUTHORITY_BOUNDARY = {
    "preflight_owned_by_plan_os": True,
    "source_synthesis_receipt_preserved": True,
    "selected_candidate_bound_to_synthesis_receipt": True,
...
class SelectedCandidateMaterializationPreflightRecord:
    selected_candidate_id: str
    selected_candidate_digest: str
    source_synthesis_receipt_digest: str
...
@dataclass(frozen=True)
class SelectedCandidateMaterializationPreflightReceipt:
    version: str
    source_version: str
    status: str
    source_synthesis_receipt_digest: str
    selected_candidate_id: str
    selected_candidate_digest: str
    materialization_preflight: dict[str, Any] | None
    blockers: list[str]
...
def _source_blockers(receipt: Mapping[str, Any]) -> list[str]:
    ...
    if receipt.get("version") != SOURCE_VERSION:
        blockers.append("source_synthesis_receipt_version_invalid")
    if receipt.get("status") != "PLANOS_SELECTED_CANDIDATE_SYNTHESIS_RECEIPT_READY":
        blockers.append("source_synthesis_receipt_not_ready")
    boundary = _m(receipt.get("boundary"))
    for required in (
        "receipt_owned_by_plan_os",
        "source_synthesis_request_preserved",
        "selected_candidate_bound_to_request",
        "synthesis_receipt_only",
    ):
...
def build_selected_candidate_materialization_preflight_receipt(
    *,
    synthesis_receipt: Mapping[str, Any],
) -> SelectedCandidateMaterializationPreflightReceipt:
    source = _m(synthesis_receipt)
    ...
    if record:
        if selected_id and str(record.get("selected_candidate_id", "")) != selected_id:
            blockers.append("selected_candidate_id_synthesis_receipt_mismatch")
        if selected_digest and str(record.get("selected_candidate_digest", "")) != selected_digest:
            blockers.append("selected_candidate_digest_synthesis_receipt_mismatch")

    preflight = None
    if selected_id and selected_digest and not blockers:
        preflight_obj = SelectedCandidateMaterializationPreflightRecord(
            selected_candidate_id=selected_id,
            selected_candidate_digest=selected_digest,
            source_synthesis_receipt_digest=str(source.get("receipt_digest", "")),
            preflight_digest=_digest(
                {
                    "source_synthesis_receipt_digest": source.get("receipt_digest"),
                    "selected_candidate_id": selected_id,
                    "selected_candidate_digest": selected_digest,
                    "scope": "selected_candidate_materialization_preflight_only",
                }
            ),
            preflight_scope="selected_candidate_materialization_preflight_only",
            materialization_authorization_granted=False,
            materialization_executed=False,
            activation_authorization_granted=False,
    ...
    status = "PLANOS_SELECTED_CANDIDATE_MATERIALIZATION_PREFLIGHT_READY" if not blockers else "PLANOS_SELECTED_CANDIDATE_MATERIALIZATION_PREFLIGHT_BLOCKED"
    partial = {
        "version": VERSION,
    ...
    return SelectedCandidateMaterializationPreflightReceipt(receipt_digest=_digest(partial), **partial)


def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print("usage: kuuos_planos_selected_candidate_materialization_preflight_v0_32.py SYNTHESIS_RECEIPT.json", file=sys.stderr)
        return 2
    with open(argv[1], "r", encoding="utf-8") as handle:
        payload = json.load(handle)
    receipt = build_selected_candidate_materialization_preflight_receipt(synthesis_receipt=payload)
