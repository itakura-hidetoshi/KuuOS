...
VERSION = "kuuos_codeai_subtask_level_version_bound_memory_v0_1"
SCHEMA_VERSION = "v0.1"
PROFILE_VERSION = "CodeAI Subtask-Level Version-Bound Memory v0.1"
STATUS_READY = "ready"
STATUS_BLOCKED = "blocked"
MODE_MEMORY_ONLY = "subtask_level_version_bound_memory_only"
DISPOSITION_COMPLETED = "subtask_level_version_bound_memory_completed"

SUBTASK_KINDS = ("localize", "diagnose", "edit", "verify")
OUTCOME_VERIFIED_USEFUL = "verified_useful"
OUTCOME_VERIFIED_NOT_USEFUL = "verified_not_useful"
OUTCOME_INCONCLUSIVE = "inconclusive"
OUTCOMES = (OUTCOME_VERIFIED_USEFUL, OUTCOME_VERIFIED_NOT_USEFUL, OUTCOME_INCONCLUSIVE)

RECOMMENDATION_AVAILABLE = "exact_subtask_version_bound_memory_available"
RECOMMENDATION_NONE = "no_exact_subtask_version_bound_memory"

REQUEST_DIGEST_FIELD = "codeai_subtask_level_version_bound_memory_request_digest"
POLICY_DIGEST_FIELD = "codeai_subtask_level_version_bound_memory_policy_digest"
ENTRY_DIGEST_FIELD = "codeai_subtask_level_version_bound_memory_entry_digest"
CORPUS_DIGEST_FIELD = "codeai_subtask_level_version_bound_memory_corpus_digest"
PACK_DIGEST_FIELD = "codeai_subtask_level_version_bound_memory_pack_digest"
RECEIPT_DIGEST_FIELD = "codeai_subtask_level_version_bound_memory_receipt_digest"
...
VERSION_BINDING_FIELDS = (
    ...
    "source_commit_sha",
    "source_tree_digest",
    "temporal_corpus_digest",
    "context_pack_digest",
    "verifier_ensemble_digest",
    "subtask_kind",
...
class CodeAISubtaskLevelVersionBoundMemoryResult:
    status: str
    issues: tuple[str, ...]
...
def _validate_binding(value: Mapping[str, Any], prefix: str) -> list[str]:
    ...
    for field in (
        "source_tree_digest", "temporal_corpus_digest", "context_pack_digest",
        "verifier_ensemble_digest", "subtask_contract_digest", "predecessor_output_digest",
        "dependency_slice_digest", "toolchain_digest", "environment_digest", "memory_policy_digest",
    ):
...
def validate_request(request: Mapping[str, Any]) -> list[str]:
    required = {
        ...
        *VERSION_BINDING_FIELDS, "request_created_epoch", "unresolved_questions",
        "claims_selection_authority", "claims_repair_authority",
        "claims_execution_authority", "claims_git_authority", REQUEST_DIGEST_FIELD,
    }
    issues: list[str] = []
...
def validate_policy(policy: Mapping[str, Any]) -> list[str]:
    required = {
        ...
        *{"expected_" + field for field in VERSION_BINDING_FIELDS},
        "evaluation_epoch", "maximum_request_age", "maximum_entry_age",
        "maximum_corpus_entries", "maximum_matched_entries",
        "allowed_outcomes", "required_subtask_kinds",
        "require_exact_version_binding", "require_subtask_alignment",
        "require_dependency_alignment", "require_verifier_grounding",
        "require_holdout_protection", "require_not_superseded",
        "allow_memory_hint", "allow_repository_mutation",
        "allow_candidate_selection", "allow_repair_execution",
        "allow_execution_authority", "allow_git_authority", POLICY_DIGEST_FIELD,
    }
    issues: list[str] = []
    ...
    expected = {field: policy["expected_" + field] for field in VERSION_BINDING_FIELDS}
    issues.extend(_validate_binding(expected, "policy_binding"))
    for field in ("evaluation_epoch", "maximum_request_age", "maximum_entry_age", "maximum_corpus_entries", "maximum_matched_entries"):
        if not positive_int(policy[field]):
            issues.append("policy_positive_integer_invalid:" + field)
    if not unique_strings(policy["allowed_outcomes"], nonempty=True) or any(x not in OUTCOMES for x in policy["allowed_outcomes"]):
        issues.append("policy_outcomes_invalid")
    if not unique_strings(policy["required_subtask_kinds"], nonempty=True) or set(policy["required_subtask_kinds"]) != set(SUBTASK_KINDS):
        issues.append("policy_subtask_coverage_invalid")
    for field in (
        "require_exact_version_binding", "require_subtask_alignment",
        "require_dependency_alignment", "require_verifier_grounding",
        "require_holdout_protection", "require_not_superseded",
...
__all__ = [name for name in globals() if name.isupper()] + [
    "CodeAISubtaskLevelVersionBoundMemoryResult", "canonical_digest", "canonical_json",
    "digest_without", "digest_ok", "mapping", "nonnegative_int", "positive_int",
    "seal", "unique_strings", "validate_policy", "validate_request",
]
