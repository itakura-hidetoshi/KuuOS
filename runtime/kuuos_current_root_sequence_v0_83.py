...
from runtime import kuuos_current_root_sequence_v0_82 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_83"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
EXECUTION_PLAN_FRONTIER = "kuuos_self_organization_execution_plan_v0_83"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "execution-plan-v0-83",
        "unittest",
        "tests.test_kuuos_self_organization_execution_plan_v0_83",
        True,
        "Execution plan is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-83",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_83",
        True,
        "Current root sequence v0.83 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if EXECUTION_PLAN_FRONTIER != "kuuos_self_organization_execution_plan_v0_83":
        issues.append("execution_plan_frontier")
    return tuple(issues)
