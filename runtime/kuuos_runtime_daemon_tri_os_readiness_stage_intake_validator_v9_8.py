...
STAGE_MODES = {"stage_candidate", "stage_reobserve_request", "stage_repair_quarantine"}
DECISIONS = {"accept", "hold", "block"}
EXPECTED_FILES = {
    "MemoryOS": "memory_os_readiness_stage_packet.json",
    "PlanOS": "plan_os_readiness_stage_packet.json",
    "RunGovernance": "run_governance_readiness_stage_packet.json",
}


@dataclass(frozen=True)
class TriOSReadinessStageIntakeValidatorResult:
    version: str
    status: str
    packet_id: str
    runtime_root: str
    memory_stage_decision: str
    plan_stage_decision: str
    run_governance_stage_decision: str
    stage_intake_packet_path: str
    receipt_path: str
    audit_path: str
    stage_intake_packet_written: bool
    blockers: list[str]
    warnings: list[str]
...
def _common(packet: Mapping[str, Any], os_name: str) -> tuple[list[str], list[str]]:
    ...
    warnings: list[str] = []
    if not packet:
        blockers.append(f"{os_name.lower()}_stage_packet_missing")
        return blockers, warnings
    if packet.get("target_os") != os_name:
        blockers.append(f"{os_name.lower()}_target_os_mismatch")
    if str(packet.get("stage_mode", "")) not in STAGE_MODES:
        blockers.append(f"{os_name.lower()}_stage_mode_invalid")
    boundary = _m(packet.get("boundary"))
    if boundary.get("stage_packet_only") is not True:
        blockers.append(f"{os_name.lower()}_stage_packet_only_boundary_missing")
    if boundary.get("does_not_authorize_execution") is not True:
        blockers.append(f"{os_name.lower()}_execution_boundary_missing")
    if packet.get("readiness") == "repair_required" and boundary.get("repair_required_blocks_ready_state") is not True:
        blockers.append(f"{os_name.lower()}_repair_ready_block_boundary_missing")
    if not packet.get("next_stage_action"):
        warnings.append(f"{os_name.lower()}_next_stage_action_missing")
    return blockers, warnings
...
def _validate_memory(packet: Mapping[str, Any]) -> tuple[str, list[str], list[str]]:
    ...
    boundary = _m(packet.get("boundary"))
    payload = _m(packet.get("stage_payload"))
    if packet:
        ...
        if payload.get("allow_overwrite") is True:
            blockers.append("memory_stage_overwrite_attempt")
...
def _validate_run(packet: Mapping[str, Any]) -> tuple[str, list[str], list[str]]:
    ...
    if packet:
        ...
        if payload.get("run_allowed") is True:
            blockers.append("run_governance_stage_run_attempt")
...
def _packet(memory: Mapping[str, Any], plan: Mapping[str, Any], run: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_digests": {
            "memory_stage": _sha(dict(memory)),
            "plan_stage": _sha(dict(plan)),
            "run_governance_stage": _sha(dict(run)),
        },
        "boundary": {
            "stage_intake_validation_only": True,
...
def build_tri_os_readiness_stage_intake_validator(*, runtime_context: Mapping[str, Any], stage_intake_license: Mapping[str, Any]) -> TriOSReadinessStageIntakeValidatorResult:
    ctx = _m(runtime_context)
    lic = _m(stage_intake_license)
    blockers: list[str] = []
    warnings: list[str] = []
    ...
    packet_path = root / "tri_os_readiness_stage_intake_decision_packet.json"
    receipt_path = root / "tri_os_readiness_stage_intake_validator_receipt.json"
    audit_path = root / "tri_os_readiness_stage_intake_validator_audit.jsonl"

    if ctx.get("tri_os_readiness_stage_intake_validator_enabled") is not True:
        blockers.append("tri_os_readiness_stage_intake_validator_enabled_not_true")
    if ctx.get("apply_tri_os_readiness_stage_intake_validator") is not True:
        blockers.append("apply_tri_os_readiness_stage_intake_validator_not_true")
    if lic.get("license_status") != "TRI_OS_READINESS_STAGE_INTAKE_VALIDATOR_LICENSE_READY":
        blockers.append("tri_os_readiness_stage_intake_validator_license_not_ready")
    for name in ["stage_packets_read_allowed", "stage_intake_packet_write_allowed", "receipt_write_allowed", "audit_append_allowed"]:
    ...
    if not blockers:
        ...
        if packet.get("boundary", {}).get("fail_closed_on_boundary_loss") is not True:
            blockers.append("stage_intake_fail_closed_boundary_missing")
        else:
    ...
    status = "TRI_OS_READINESS_STAGE_INTAKE_VALIDATOR_READY" if not blockers else "TRI_OS_READINESS_STAGE_INTAKE_VALIDATOR_BLOCKED"
    packet_id = "tri-os-readiness-stage-intake-" + _sha({"packet": packet, "blockers": blockers})[:16]
    receipt = {
        "version": "kuuos_runtime_daemon_tri_os_readiness_stage_intake_validator_v9_8",
        "status": status,
        "packet_id": packet_id,
        "memory_stage_decision": mem_dec,
        "plan_stage_decision": plan_dec,
        "run_governance_stage_decision": run_dec,
        "stage_intake_packet_written": written,
        "stage_intake_packet_digest": _sha(packet),
        "blockers": blockers,
        "warnings": warnings,
        "epoch": int(time.time()),
    }
    ...
    return TriOSReadinessStageIntakeValidatorResult(
        "kuuos_runtime_daemon_tri_os_readiness_stage_intake_validator_v9_8",
        status,
        packet_id,
        str(root),
        mem_dec,
