...
VERSION = "kuuos_planos_blocker_release_authorization_request_v0_56"
SOURCE_VERSION = "kuuos_planos_truth_authority_closeout_receipt_v0_55"

BLOCKER_RELEASE_AUTHORIZATION_REQUEST_BOUNDARY = {
    "request_owned_by_plan_os": True,
    "source_truth_authority_closeout_preserved": True,
    "selected_candidate_bound_to_truth_authority_closeout": True,
    ...
    "blocker_release_authorization_request_only": True,
    "blocker_release_authorization_requested": True,
    "blocker_release_authorization_granted": False,
    "blocker_release_granted": False,
}

REQUIRED_SOURCE_BOUNDARY_TRUE = (
    "closeout_owned_by_plan_os",
    "source_truth_authority_receipt_preserved",
    "selected_candidate_bound_to_truth_authority_receipt",
...
class BlockerReleaseAuthorizationRequestRecord:
    ...
    blocker_release_authorization_request_digest: str
    request_scope: str = "blocker_release_authorization_request_only"
    ...
    blocker_release_authorization_granted: bool = False
...
@dataclass(frozen=True)
class BlockerReleaseAuthorizationRequestReceipt:
    version: str
    source_version: str
    status: str
    source_truth_authority_closeout_receipt_digest: str
    selected_candidate_id: str
    selected_candidate_digest: str
    blocker_release_authorization_request: dict[str, Any] | None
    blockers: list[str]
...
def _source_closeout_record(receipt: Mapping[str, Any]) -> Mapping[str, Any]:
    return _m(receipt.get("truth_authority_closeout_receipt"))


def _source_blockers(receipt: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    if receipt.get("version") != SOURCE_VERSION:
        blockers.append("source_truth_authority_closeout_version_invalid")
    if receipt.get("status") != "PLANOS_TRUTH_AUTHORITY_CLOSEOUT_RECEIPT_READY":
        blockers.append("source_truth_authority_closeout_not_ready")
    ...
    if not receipt.get("receipt_digest"):
        blockers.append("source_truth_authority_closeout_digest_missing")
    record = _source_closeout_record(receipt)
    if not record:
        blockers.append("source_truth_authority_closeout_record_missing")
    if record:
        for field in (
            "memory_overwrite_preserved",
            "memory_overwrite_closeout_preserved",
...
def build_blocker_release_authorization_request(
    *,
    truth_authority_closeout: Mapping[str, Any],
) -> BlockerReleaseAuthorizationRequestReceipt:
    source = _m(truth_authority_closeout)
    ...
    if record:
        if selected_id and str(record.get("selected_candidate_id", "")) != selected_id:
            blockers.append("selected_candidate_id_truth_authority_closeout_mismatch")
        if selected_digest and str(record.get("selected_candidate_digest", "")) != selected_digest:
            blockers.append("selected_candidate_digest_truth_authority_closeout_mismatch")

    request = None
    if selected_id and selected_digest and not blockers:
        request_obj = BlockerReleaseAuthorizationRequestRecord(
            selected_candidate_id=selected_id,
            selected_candidate_digest=selected_digest,
            source_truth_authority_closeout_receipt_digest=str(source.get("receipt_digest", "")),
            ...
            blocker_release_authorization_request_digest=sha({
                "source_truth_authority_closeout_receipt_digest": source.get("receipt_digest"),
                ...
                "selected_candidate_id": selected_id,
                "selected_candidate_digest": selected_digest,
                "scope": "blocker_release_authorization_request_only",
            }),
        )
    ...
    status = (
        ...
        if not blockers
        else "PLANOS_BLOCKER_RELEASE_AUTHORIZATION_REQUEST_BLOCKED"
    )
    partial = {
        "version": VERSION,
        ...
        "selected_candidate_id": selected_id,
        "selected_candidate_digest": selected_digest,
        "blocker_release_authorization_request": request,
        "blockers": blockers,
        "boundary": dict(BLOCKER_RELEASE_AUTHORIZATION_REQUEST_BOUNDARY),
    }
    return BlockerReleaseAuthorizationRequestReceipt(receipt_digest=sha(partial), **partial)


def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print(
            "usage: kuuos_planos_blocker_release_authorization_request_v0_56.py TRUTH_AUTHORITY_CLOSEOUT.json",
            file=sys.stderr,
        )
        return 2
    with open(argv[1], "r", encoding="utf-8") as handle:
        payload = json.load(handle)
    receipt = build_blocker_release_authorization_request(truth_authority_closeout=payload)
