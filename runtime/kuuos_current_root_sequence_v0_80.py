...
from runtime import kuuos_current_root_sequence_v0_79 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_80"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
CANDIDATE_RECEIPT_FRONTIER = "kuuos_self_organization_candidate_receipt_v0_80"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "candidate-receipt-v0-80",
        "unittest",
        "tests.test_kuuos_self_organization_candidate_receipt_v0_80",
        True,
        "Candidate receipt is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-80",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_80",
        True,
        "Current root sequence v0.80 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if CANDIDATE_RECEIPT_FRONTIER != "kuuos_self_organization_candidate_receipt_v0_80":
