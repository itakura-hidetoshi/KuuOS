...
VERSION = "kuuos_planos_materialization_authorization_request_v0_33"
SOURCE_VERSION = "kuuos_planos_selected_candidate_materialization_preflight_v0_32"

NON_AUTHORITY_BOUNDARY = {
    "request_owned_by_plan_os": True,
    "source_materialization_preflight_preserved": True,
    "selected_candidate_bound_to_preflight": True,
    "materialization_authorization_request_only": True,
...
class MaterializationAuthorizationRequestRecord:
...
class MaterializationAuthorizationRequestReceipt:
    ...
    selected_candidate_id: str
    selected_candidate_digest: str
    materialization_authorization_request: dict[str, Any] | None
    blockers: list[str]
...
def _source_preflight_record(receipt: Mapping[str, Any]) -> Mapping[str, Any]:
    return _m(receipt.get("materialization_preflight"))


def _source_blockers(receipt: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    if receipt.get("version") != SOURCE_VERSION:
        blockers.append("source_materialization_preflight_version_invalid")
    if receipt.get("status") != "PLANOS_SELECTED_CANDIDATE_MATERIALIZATION_PREFLIGHT_READY":
        blockers.append("source_materialization_preflight_not_ready")
    boundary = _m(receipt.get("boundary"))
    for required in (
        "preflight_owned_by_plan_os",
        "source_synthesis_receipt_preserved",
        "selected_candidate_bound_to_synthesis_receipt",
    ...
    if not receipt.get("receipt_digest"):
        blockers.append("source_preflight_digest_missing")
    record = _source_preflight_record(receipt)
    if not record:
        blockers.append("source_preflight_record_missing")
    for closed in (
        "materialization_authorization_granted",
        "materialization_executed",
        "activation_authorization_granted",
...
def build_materialization_authorization_request_receipt(
    *,
    materialization_preflight_receipt: Mapping[str, Any],
) -> MaterializationAuthorizationRequestReceipt:
    source = _m(materialization_preflight_receipt)
    ...
    record = _source_preflight_record(source)
    if not selected_id:
    ...
    if record:
        if selected_id and str(record.get("selected_candidate_id", "")) != selected_id:
            blockers.append("selected_candidate_id_preflight_mismatch")
        if selected_digest and str(record.get("selected_candidate_digest", "")) != selected_digest:
            blockers.append("selected_candidate_digest_preflight_mismatch")

    request = None
    if selected_id and selected_digest and not blockers:
        request_obj = MaterializationAuthorizationRequestRecord(
            selected_candidate_id=selected_id,
            selected_candidate_digest=selected_digest,
            source_preflight_digest=str(source.get("receipt_digest", "")),
            authorization_request_digest=_digest(
                {
                    "source_preflight_digest": source.get("receipt_digest"),
                    "selected_candidate_id": selected_id,
                    "selected_candidate_digest": selected_digest,
                    "scope": "materialization_authorization_request_only",
                }
            ),
            request_scope="materialization_authorization_request_only",
            materialization_authorization_granted=False,
            materialization_executed=False,
            activation_authorization_granted=False,
    ...
    status = "PLANOS_MATERIALIZATION_AUTHORIZATION_REQUEST_READY" if not blockers else "PLANOS_MATERIALIZATION_AUTHORIZATION_REQUEST_BLOCKED"
    partial = {
        "version": VERSION,
        ...
        "selected_candidate_id": selected_id,
        "selected_candidate_digest": selected_digest,
        "materialization_authorization_request": request,
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print("usage: kuuos_planos_materialization_authorization_request_v0_33.py MATERIALIZATION_PREFLIGHT.json", file=sys.stderr)
        return 2
    with open(argv[1], "r", encoding="utf-8") as handle:
        payload = json.load(handle)
    receipt = build_materialization_authorization_request_receipt(materialization_preflight_receipt=payload)
