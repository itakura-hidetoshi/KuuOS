...
from runtime import kuuos_current_root_sequence_v0_70 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_71"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
CURRENT_STATUS_RESOLVER_FRONTIER = "kuuos_current_status_resolver_v0_71"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "current-status-resolver-v0-71",
        "unittest",
        "tests.test_kuuos_current_status_resolver_v0_71",
        True,
        "Stable current status resolver must follow current pointer to index and snapshot.",
    ),
    CurrentRootStep(
        "current-status-cli-v0-71",
        "unittest",
        "tests.test_kuuos_current_status_cli_v0_71",
        True,
        "Stable current status CLI must expose the current resolver.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-71",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_71",
        True,
        "This layer connects the stable current status resolver into the current runtime root.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if CURRENT_STATUS_RESOLVER_FRONTIER != "kuuos_current_status_resolver_v0_71":
        issues.append("current_status_resolver_frontier")
    return tuple(issues)
