...
from runtime import kuuos_current_root_sequence_v0_67 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_68"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
SNAPSHOT_FRONTIER = "kuuos_self_organization_status_snapshot_v0_68"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "self-organization-status-snapshot-v0-68",
        "unittest",
        "tests.test_kuuos_self_organization_status_snapshot_v0_68",
        True,
        "Published status snapshot must match the live self-organization status.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-68",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_68",
        True,
        "This layer connects the status snapshot into the current runtime root.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if SNAPSHOT_FRONTIER != "kuuos_self_organization_status_snapshot_v0_68":
        issues.append("snapshot_frontier")
    return tuple(issues)
