...
from runtime import kuuos_current_root_sequence_v0_103 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_104"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
CANDIDATE_QUEUE_FRONTIER = "kuuos_self_organization_candidate_queue_v0_104"
SELF_CHECK_TARGET = "runtime.kuuos_current_root_sequence_v0_104:run_self_check"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "candidate-queue-v0-104",
        "unittest",
        "tests.test_kuuos_self_organization_candidate_queue_v0_104",
        True,
        "Candidate queue v0.104 is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-104",
        "callable",
        SELF_CHECK_TARGET,
        True,
        "Current root sequence v0.104 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if CANDIDATE_QUEUE_FRONTIER != "kuuos_self_organization_candidate_queue_v0_104":
