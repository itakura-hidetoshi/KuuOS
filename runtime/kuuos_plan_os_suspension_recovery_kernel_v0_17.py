    ...
    *,
    ...
    if route == REVALIDATE:
        ...
        for kind in affected:
            if _renewal_eligible(
                dict(policies[kind]), int(suspension_tick["tick_at_ms"])
            ):
...
def validate_suspension_recovery_handoff(
    ...
    try:
        ...
        require_int(
            handoff.get("mission_cycle_index"),
            "mission_cycle_index",
            minimum=1,
        )
        require_int(
        ...
        if route == REVALIDATE:
            ...
            if (
                candidates
                or escalations
                or handoff.get("rerotation_required") is not False
            ):
            ...
            if handoff.get("revalidation_observation_digest") != "":
                errors.append(
                    "recovery_renewal_revalidation_digest_forbidden"
                )
            ...
            if candidates or escalations:
                errors.append(
                    "recovery_rerotation_renewal_payload_forbidden"
                )
            if handoff.get("revalidation_observation_digest") != "":
                errors.append(
                    "recovery_rerotation_revalidation_digest_forbidden"
                )
        else:
