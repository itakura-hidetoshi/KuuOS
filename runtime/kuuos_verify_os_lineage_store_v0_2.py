...
class VerifyLineageStoreError(RuntimeError):
    pass


def build_initial_verify_lineage_store_state(
    *, store_id: str, now_ms: int
    ...
    state["verify_lineage_store_state_digest"] = store_state_digest(state)
    return state


def validate_verify_lineage_store_state(
    state: Mapping[str, Any]
) -> list[str]:
    errors: list[str] = []
    try:
        if state.get("version") != STORE_STATE_VERSION:
            errors.append("verify_lineage_store_state_version_invalid")
        ...
        if state.get("verify_lineage_store_state_digest") != store_state_digest(
            state
        ):
            errors.append("verify_lineage_store_state_digest_invalid")
        ...
        completed_digests = [
            str(item.get("handoff_receipt_digest", ""))
            for item in completed
        ]
        if len(issued_keys) != len(set(issued_keys)):
            errors.append("verify_lineage_store_single_use_key_duplicate")
        if len(issued_digests) != len(set(issued_digests)):
            errors.append("verify_lineage_store_handoff_duplicate")
        if len(completed_digests) != len(set(completed_digests)):
            errors.append("verify_lineage_store_completion_duplicate")
        if not set(completed_digests).issubset(set(issued_digests)):
            errors.append("verify_lineage_store_completion_without_handoff")
        if len(processed) != len(set(processed)):
            errors.append("verify_lineage_store_processed_duplicate")
        if int(state.get("commit_count", -1)) != len(processed):
            errors.append("verify_lineage_store_commit_count_mismatch")
    except (TypeError, ValueError) as exc:
        errors.append(str(exc))
    return errors


class VerifyLineageStore:
    def __init__(self, root: str | Path) -> None:
        self.root = Path(root)
        self.genesis_path = self.root / "verify-lineage-genesis.json"
        self.ledger_path = self.root / "verify-lineage-ledger.jsonl"
        self.snapshot_path = self.root / "verify-lineage-snapshot.json"
        self.lock_path = self.root / ".verify-os-v02-lineage.lock"
    ...
    def _read_json(path: Path) -> dict[str, Any]:
        try:
            ...
            raise VerifyLineageStoreError(
                f"verify_lineage_json_read_failed:{path.name}"
            ) from exc
        if not isinstance(value, dict):
            raise VerifyLineageStoreError(
                f"verify_lineage_json_object_required:{path.name}"
            )
        return value

    def initialize(self, initial_state: Mapping[str, Any]) -> dict[str, Any]:
        errors = validate_verify_lineage_store_state(initial_state)
        if errors:
            raise VerifyLineageStoreError(
                "initial_verify_lineage_store_state_invalid:"
                + ";".join(errors)
            )
        with self._locked():
            if self.genesis_path.exists() or self.ledger_path.exists():
                raise VerifyLineageStoreError(
                    "verify_lineage_store_already_initialized"
                )
        ...
        try:
            with self.ledger_path.open("r", encoding="utf-8") as handle:
                for line_number, raw in enumerate(handle, start=1):
                    ...
                    if not line:
                        raise VerifyLineageStoreError(
                            f"verify_lineage_ledger_blank_line:{line_number}"
                        )
                    item = json.loads(line)
                    if not isinstance(item, dict):
                        raise VerifyLineageStoreError(
                            f"verify_lineage_ledger_object_required:{line_number}"
                        )
                    commits.append(item)
        except (OSError, json.JSONDecodeError) as exc:
            raise VerifyLineageStoreError(
                "verify_lineage_ledger_read_failed"
            ) from exc
        return commits
    ...
    def _apply_receipt(
        ...
        if stage not in STAGES:
            raise VerifyLineageStoreError("verify_lineage_stage_invalid")
        next_state = deepcopy(dict(state))
        if stage == STAGE_HANDOFF:
            errors = validate_handoff_receipt(receipt)
            if errors:
                raise VerifyLineageStoreError(
                    "verify_lineage_handoff_invalid:" + ";".join(errors)
                )
            digest = str(receipt["verify_lineage_handoff_receipt_digest"])
            ...
            if key in {
                ...
                raise VerifyLineageStoreError(
                    "verify_lineage_handoff_already_issued"
                )
            next_state["issued_handoffs"] = list(
                next_state["issued_handoffs"]
            ) + [
                {
                    "single_use_key": key,
                    "handoff_receipt_digest": digest,
                    "observe_completion_receipt_digest": receipt[
                        "observe_lineage_completion_receipt_digest"
                    ],
                    "verification_criterion_digest": receipt[
            ...
            if errors:
                raise VerifyLineageStoreError(
                    "verify_lineage_completion_invalid:" + ";".join(errors)
                )
            digest = str(receipt["verify_lineage_completion_receipt_digest"])
            if digest in set(next_state["processed_receipt_digests"]):
                return "REPLAYED", next_state
            handoff_digest = str(
                receipt["verify_lineage_handoff_receipt_digest"]
            )
            issued = {
            ...
            if handoff_digest not in issued:
                raise VerifyLineageStoreError(
                    "verify_lineage_completion_handoff_not_issued"
                )
            completed = {
                str(item["handoff_receipt_digest"])
            ...
            if handoff_digest in completed:
                raise VerifyLineageStoreError(
                    "verify_lineage_completion_already_committed"
                )
            next_state["completed_handoffs"] = list(
            ...
            ) + [
                {
                    ...
                    "verify_state_digest": receipt[
                        "committed_verify_state_digest"
                    ],
                    "route": receipt["route"],
                    "verdict": receipt["verdict"],
                    "learning_required": receipt["learning_required"],
                }
            ]
        ...
        next_state["verify_lineage_store_state_digest"] = ""
        next_state["verify_lineage_store_state_digest"] = store_state_digest(
            next_state
        )
        errors = validate_verify_lineage_store_state(next_state)
        if errors:
            raise VerifyLineageStoreError(
                "next_verify_lineage_store_state_invalid:"
                + ";".join(errors)
            )
    ...
    def _recover_unlocked(
        ...
        if not self.genesis_path.exists() or not self.ledger_path.exists():
            raise VerifyLineageStoreError(
                "verify_lineage_store_not_initialized"
            )
        state = self._read_json(self.genesis_path)
        errors = validate_verify_lineage_store_state(state)
        if errors:
            raise VerifyLineageStoreError(
                "verify_lineage_genesis_invalid:" + ";".join(errors)
            )
        ...
        for index, commit in enumerate(commits, start=1):
            if commit.get("version") != STORE_COMMIT_VERSION:
                raise VerifyLineageStoreError(
                    f"verify_lineage_commit_version_invalid:{index}"
                )
            if commit.get(
                "verify_lineage_store_commit_digest"
            ) != store_commit_digest(commit):
                raise VerifyLineageStoreError(
                    f"verify_lineage_commit_digest_invalid:{index}"
                )
            if commit.get("predecessor_commit_digest") != previous_commit_digest:
                raise VerifyLineageStoreError(
                    f"verify_lineage_commit_chain_broken:{index}"
                )
            if commit.get("predecessor_state_digest") != state.get(
                "verify_lineage_store_state_digest"
            ):
                raise VerifyLineageStoreError(
                    f"verify_lineage_state_chain_broken:{index}"
                )
            ...
            if status != "COMMITTED":
                raise VerifyLineageStoreError(
                    f"verify_lineage_recovery_replay_unexpected:{index}"
                )
            if commit.get("result_state_digest") != state.get(
                "verify_lineage_store_state_digest"
            ):
                raise VerifyLineageStoreError(
                    f"verify_lineage_result_digest_mismatch:{index}"
                )
    ...
    def recover(self, *, require_snapshot_match: bool = True) -> dict[str, Any]:
        with self._locked():
            ...
            if require_snapshot_match:
                ...
                if snapshot.get(
                    "verify_lineage_store_state_digest"
                ) != state.get("verify_lineage_store_state_digest"):
                    raise VerifyLineageStoreError(
                        "verify_lineage_snapshot_ledger_mismatch"
                    )
    ...
    def commit(
        ...
        with self._locked():
            ...
            commit["verify_lineage_store_commit_digest"] = store_commit_digest(
                commit
            )
            with self.ledger_path.open("a", encoding="utf-8") as handle:
