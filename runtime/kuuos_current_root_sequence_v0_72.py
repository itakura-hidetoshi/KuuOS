...
from runtime import kuuos_current_root_sequence_v0_71 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_72"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
RESOLVED_STATUS_ARTIFACT_FRONTIER = "kuuos_current_resolved_status_artifact_v0_72"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "current-resolved-status-artifact-v0-72",
        "unittest",
        "tests.test_kuuos_current_resolved_status_artifact_v0_72",
        True,
        "Committed resolved status artifact must match the stable resolver output.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-72",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_72",
        True,
        "This layer connects the resolved current status artifact into the current runtime root.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if RESOLVED_STATUS_ARTIFACT_FRONTIER != "kuuos_current_resolved_status_artifact_v0_72":
        issues.append("resolved_status_artifact_frontier")
    return tuple(issues)
