...
from runtime import kuuos_current_root_sequence_v0_65 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_66"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
PUBLIC_STATUS_FRONTIER = "kuuos_readme_public_status_v0_66"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "readme-public-status-v0-66",
        "unittest",
        "tests.test_kuuos_readme_public_status_v0_66",
        True,
        "README public status must reflect the active self-organization current root frontier.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-66",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_66",
        True,
        "This layer connects README public status synchronization into the current runtime root.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if PUBLIC_STATUS_FRONTIER != "kuuos_readme_public_status_v0_66":
        issues.append("public_status_frontier")
    return tuple(issues)
