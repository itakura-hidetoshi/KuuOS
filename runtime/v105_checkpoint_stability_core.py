...
def construct_repository_checkpoint_stability_certificate(
    ...
    *,
    ...
    receipt_confirmed = bool(
        receipt_valid
        and creation_receipt.status
        == "REPOSITORY_CHECKPOINT_CREATION_RECEIPT_CONFIRMED"
    )
    ...
    sequence_ordered = bool(
        initial_sequence
        < delayed_observation.sequence_number
    ...
    observations_fresh = bool(
        all(
            0
            <= evaluated_at_epoch_seconds - value
    ...
    stability_confirmed = bool(
        failure_kind == FAILURE_NONE and immutable_policy_enforced
    )
    checks = {
