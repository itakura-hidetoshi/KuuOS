...
VERSION = "kuuos_planos_next_cycle_admission_request_v0_60"
SOURCE_VERSION = "kuuos_planos_blocker_release_closeout_receipt_v0_59"

NEXT_CYCLE_ADMISSION_REQUEST_BOUNDARY = {
    "request_owned_by_plan_os": True,
    "source_blocker_release_closeout_preserved": True,
    "selected_candidate_bound_to_blocker_release_closeout": True,
    ...
    "next_cycle_admission_request_only": True,
    "next_cycle_admission_requested": True,
    "next_cycle_admission_granted": False,
    "next_cycle_started": False,
}

REQUIRED_SOURCE_BOUNDARY_TRUE = (
    "closeout_owned_by_plan_os",
    "source_blocker_release_receipt_preserved",
    "selected_candidate_bound_to_blocker_release_receipt",
...
class NextCycleAdmissionRequestRecord:
    ...
    next_cycle_admission_request_digest: str
    request_scope: str = "next_cycle_admission_request_only"
    ...
    next_cycle_admission_granted: bool = False
...
@dataclass(frozen=True)
class NextCycleAdmissionRequestReceipt:
    version: str
    source_version: str
    status: str
    source_blocker_release_closeout_receipt_digest: str
    selected_candidate_id: str
    selected_candidate_digest: str
    next_cycle_admission_request: dict[str, Any] | None
    blockers: list[str]
...
def _source_record(receipt: Mapping[str, Any]) -> Mapping[str, Any]:
    return _m(receipt.get("blocker_release_closeout_receipt"))


def _source_blockers(receipt: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    if receipt.get("version") != SOURCE_VERSION:
        blockers.append("source_blocker_release_closeout_version_invalid")
    if receipt.get("status") != "PLANOS_BLOCKER_RELEASE_CLOSEOUT_RECEIPT_READY":
        blockers.append("source_blocker_release_closeout_not_ready")
    ...
    if not receipt.get("receipt_digest"):
        blockers.append("source_blocker_release_closeout_digest_missing")
    record = _source_record(receipt)
    if not record:
        blockers.append("source_blocker_release_closeout_record_missing")
    if record:
        for field in (
            "memory_overwrite_preserved",
            "memory_overwrite_closeout_preserved",
        ...
        if _truthy(record.get("next_cycle_admission_ready")):
            blockers.append("source_record_next_cycle_admission_ready_promoted")
    return blockers


def build_next_cycle_admission_request(
    *,
    blocker_release_closeout: Mapping[str, Any],
) -> NextCycleAdmissionRequestReceipt:
    source = _m(blocker_release_closeout)
    ...
    if record:
        if selected_id and str(record.get("selected_candidate_id", "")) != selected_id:
            blockers.append("selected_candidate_id_blocker_release_closeout_mismatch")
        if selected_digest and str(record.get("selected_candidate_digest", "")) != selected_digest:
            blockers.append("selected_candidate_digest_blocker_release_closeout_mismatch")

    request = None
    if selected_id and selected_digest and not blockers:
        request_obj = NextCycleAdmissionRequestRecord(
            selected_candidate_id=selected_id,
            selected_candidate_digest=selected_digest,
            source_blocker_release_closeout_receipt_digest=str(source.get("receipt_digest", "")),
            ...
            next_cycle_admission_request_digest=sha({
                "source_blocker_release_closeout_receipt_digest": source.get("receipt_digest"),
                ...
                "selected_candidate_id": selected_id,
                "selected_candidate_digest": selected_digest,
                "scope": "next_cycle_admission_request_only",
            }),
        )
    ...
    status = (
        ...
        if not blockers
        else "PLANOS_NEXT_CYCLE_ADMISSION_REQUEST_BLOCKED"
    )
    partial = {
        "version": VERSION,
        ...
        "selected_candidate_id": selected_id,
        "selected_candidate_digest": selected_digest,
        "next_cycle_admission_request": request,
        "blockers": blockers,
        "boundary": dict(NEXT_CYCLE_ADMISSION_REQUEST_BOUNDARY),
    }
    return NextCycleAdmissionRequestReceipt(receipt_digest=sha(partial), **partial)


def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print(
            "usage: kuuos_planos_next_cycle_admission_request_v0_60.py BLOCKER_RELEASE_CLOSEOUT.json",
            file=sys.stderr,
        )
        return 2
    with open(argv[1], "r", encoding="utf-8") as handle:
        payload = json.load(handle)
    receipt = build_next_cycle_admission_request(blocker_release_closeout=payload)
