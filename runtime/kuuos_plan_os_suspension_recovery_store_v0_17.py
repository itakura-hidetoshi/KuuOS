...
class SuspensionRecoveryStore:
    ...
    def _recover(self) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any] | None]:
        ...
        if len(commits) > 1:
            raise SuspensionRecoveryStoreError("recovery_multiple_commits_forbidden")
        if not commits:
        ...
        if commit.get("suspension_recovery_store_commit_digest") != store_commit_digest(
            commit
        ):
        ...
        if handoff.get("source_materialization_receipt_digest") != materialization.get(
    ...
    def commit(
        ...
        with self._locked():
            genesis, commits, existing = self._recover()
            handoff_id = str(handoff.get("suspension_recovery_handoff_digest", ""))
            ...
            if handoff.get("source_tick_digest") != genesis["suspension_tick"].get(
                "lease_monitor_tick_digest"
            ):
                return {
                    "status": "REJECTED",
            ...
            commit["suspension_recovery_store_commit_digest"] = store_commit_digest(
                commit
            )
    ...
    def recover(self, *, require_snapshot_match: bool = True) -> dict[str, Any]:
        with self._locked():
            ...
            if require_snapshot_match and handoff is not None:
                ...
                if snapshot.get("suspension_recovery_handoff_digest") != handoff.get(
