...
from runtime import kuuos_current_root_sequence_v0_66 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_67"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
STATUS_FRONTIER = "kuuos_self_organization_status_v0_67"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "self-organization-status-v0-67",
        "unittest",
        "tests.test_kuuos_self_organization_status_v0_67",
        True,
        "Machine-readable active self-organization status must be checked from the current root.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-67",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_67",
        True,
        "This layer connects the self-organization status CLI into the current runtime root.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if STATUS_FRONTIER != "kuuos_self_organization_status_v0_67":
        issues.append("status_frontier")
    return tuple(issues)
