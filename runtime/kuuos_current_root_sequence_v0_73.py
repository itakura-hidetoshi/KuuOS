...
from runtime import kuuos_current_root_sequence_v0_72 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_73"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
CURRENT_STATUS_MANIFEST_FRONTIER = "kuuos_current_status_manifest_v0_73"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "current-status-manifest-v0-73",
        "unittest",
        "tests.test_kuuos_current_status_manifest_v0_73",
        True,
        "Committed current status manifest must point to the verified status surface.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-73",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_73",
        True,
        "This layer connects the current status manifest into the current runtime root.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if CURRENT_STATUS_MANIFEST_FRONTIER != "kuuos_current_status_manifest_v0_73":
        issues.append("current_status_manifest_frontier")
    return tuple(issues)
