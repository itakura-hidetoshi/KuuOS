...
from runtime import kuuos_current_root_sequence_v0_88 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_89"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
NEXT_CYCLE_SEED_FRONTIER = "kuuos_self_organization_next_cycle_seed_v0_89"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "next-cycle-seed-v0-89",
        "unittest",
        "tests.test_kuuos_self_organization_next_cycle_seed_v0_89",
        True,
        "Next cycle seed is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-89",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_89",
        True,
        "Current root sequence v0.89 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if NEXT_CYCLE_SEED_FRONTIER != "kuuos_self_organization_next_cycle_seed_v0_89":
