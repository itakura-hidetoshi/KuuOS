...
from runtime import kuuos_current_root_sequence_v0_113 as current_root
from runtime import kuuos_self_organization_candidate_receipt_v0_112 as receipt
from runtime import kuuos_self_organization_selection_policy_v0_113 as policy

VERSION = "kuuos_self_organization_selected_next_action_v0_114"
DEPENDS_ON = policy.VERSION
CURRENT_ROOT_SEQUENCE = current_root.VERSION
SELECTED_NEXT_ACTION_PATH = "status/self_organization_selected_next_action_v0_114.json"
SELECTION_SCHEMA_VERSION = "v0.114"
...
CANDIDATE_NEXT_STAGES: dict[str, str] = {
    "candidate-receipt-v0-112": "v0.112",
    "selection-policy-v0-113": "v0.113",
    "selected-next-action-v0-114": "v0.114",
    "bounded-change-plan-v0-115": "v0.115",
}

CANDIDATE_SCOPES: dict[str, str] = {
    "candidate-receipt-v0-112": "candidate_receipt_only",
    "selection-policy-v0-113": "selection_policy_only",
    "selected-next-action-v0-114": "selection_only",
    "bounded-change-plan-v0-115": "bounded_change_plan_only",
}

REQUIRED_SELECTED_NEXT_ACTION_KEYS: tuple[str, ...] = (
    "authority_boundary",
    "candidate_receipt",
...
def expected_selected_candidate_id() -> str:
    ...
    for candidate_id in receipt.expected_received_candidate_ids():
...
def expected_selected_next_action() -> dict[str, Any]:
    ...
    return {
        ...
        "next_artifact": "status/self_organization_bounded_change_plan_v0_115.json",
        "next_stage": "v0.115",
...
def selected_next_action_issues() -> tuple[str, ...]:
    ...
    missing = set(REQUIRED_SELECTED_NEXT_ACTION_KEYS).difference(selected)
    if missing:
        issues.append("missing_selected_next_action_keys:" + ",".join(sorted(missing)))
    extra = set(selected).difference(REQUIRED_SELECTED_NEXT_ACTION_KEYS)
    if extra:
    ...
    if selected.get("selected_candidate_id") not in receipt.expected_received_candidate_ids():
        issues.append("selected_candidate_missing_from_receipt")
    if selected.get("selected_candidate_id") != "bounded-change-plan-v0-115":
    ...
    if selected.get("selected_next_stage") != "v0.115":
        issues.append("selected_next_stage")
    if selected.get("next_artifact") != "status/self_organization_bounded_change_plan_v0_115.json":
        issues.append("next_artifact")
    if selected.get("next_stage") != "v0.115":
    ...
    if policy_payload.get("next_stage") != "v0.114":
