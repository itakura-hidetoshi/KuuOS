...
def repository_checkpoint_stability_policy_issues(
    ...
    if not policy.policy_id:
        issues.append("checkpoint_stability_policy_id_missing")
    if (
        not policy.authorized_observer_ids
        or policy.authorized_observer_ids
        != canonical_strings(policy.authorized_observer_ids)
    ...
    if policy.checkpoint_namespace_prefix != CHECKPOINT_PREFIX:
        issues.append("checkpoint_stability_namespace_invalid")
    if not all(
        (
