...
class SelfModificationStoreError(ValueError):
    pass
...
class SelfModificationStore:
    def __init__(self, root: str | Path) -> None:
        self.root = Path(root)
        self.initial_path = self.root / "self-modification-initial.json"
        self.snapshot_path = self.root / "self-modification-snapshot.json"
        self.ledger_path = self.root / "self-modification-ledger.jsonl"
    ...
    def _read_json(path: Path) -> dict[str, Any]:
        try:
            ...
            raise SelfModificationStoreError(
                f"self_modification_json_invalid:{path.name}"
            ) from exc
        if not isinstance(value, dict):
            raise SelfModificationStoreError(
                f"self_modification_json_root_invalid:{path.name}"
            )
        return value
    ...
    def initialize(self, initial_state: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if errors:
            raise SelfModificationStoreError(
                "self_modification_initial_state_invalid:" + ";".join(errors)
            )
        ...
        if any(
            ...
            raise SelfModificationStoreError(
                "self_modification_store_already_initialized"
            )
    ...
    def recover(self, *, require_snapshot_match: bool = True) -> dict[str, Any]:
        if not self.initial_path.exists() or not self.ledger_path.exists():
            raise SelfModificationStoreError(
                "self_modification_store_not_initialized"
            )
        ...
        for line_number, line in enumerate(
            ...
            try:
                ...
                raise SelfModificationStoreError(
                    f"self_modification_ledger_json_invalid:{line_number}"
                ) from exc
            if not isinstance(event, dict):
                raise SelfModificationStoreError(
                    f"self_modification_ledger_event_invalid:{line_number}"
                )
            result = apply_event(state, event)
            if result.get("status") != "APPLIED":
                raise SelfModificationStoreError(
                    f"self_modification_ledger_replay_rejected:{line_number}:"
        ...
        if self.snapshot_path.exists():
            ...
            if require_snapshot_match and snapshot != state:
                raise SelfModificationStoreError(
                    "self_modification_snapshot_ledger_mismatch"
                )
        elif require_snapshot_match:
            raise SelfModificationStoreError("self_modification_snapshot_missing")
        return deepcopy(state)
...
__all__ = ["SelfModificationStore", "SelfModificationStoreError"]
