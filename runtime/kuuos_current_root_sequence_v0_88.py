...
from runtime import kuuos_current_root_sequence_v0_87 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_88"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
COMPLETION_RECEIPT_FRONTIER = "kuuos_self_organization_completion_receipt_v0_88"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "completion-receipt-v0-88",
        "unittest",
        "tests.test_kuuos_self_organization_completion_receipt_v0_88",
        True,
        "Completion receipt is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-88",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_88",
        True,
        "Current root sequence v0.88 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if COMPLETION_RECEIPT_FRONTIER != "kuuos_self_organization_completion_receipt_v0_88":
