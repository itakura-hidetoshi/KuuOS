    ...
    *,
    ...
    for index, item in enumerate(values):
        ...
        if not ok:
            blockers.append(reason)
            continue
        for field in ("scenario_id", "homotopy_class_id", "coherence_block_id"):
            if not _nonempty_text(item[field]):
                blockers.append(f"{field}_invalid_{history_id}")
        weight_numerator = item["weight_numerator"]
        if not _positive_nat(weight_numerator):
            blockers.append(f"history_weight_invalid_{history_id}")
            weight_numerator = 0
        phase_mod4 = item["phase_mod4"]
        if phase_mod4 not in (0, 1, 2, 3):
        ...
        if not _nonempty_text(item["source_homotopy_witness_digest"]):
            blockers.append(f"history_homotopy_witness_digest_missing_{history_id}")
        output.append(
            {
...
def normalize_claims(value: Any):
    fields = {
        ...
        "z4_phase_surface_used",
        "gaussian_integer_arithmetic_exact",
        "homotopy_classes_retained",
        "coherence_blocks_retained",
        "decoherence_mask_applied_without_pruning",
        "argmin_performed",
        ...
        "source_v121_certificate_mutated",
...
def compute_gaussian_homotopy_decoherence_input_digest(**payload) -> str:
