...
class ActLineageStoreError(RuntimeError):
    pass


def build_initial_act_lineage_store_state(
    *, store_id: str, now_ms: int
    ...
    state["act_lineage_store_state_digest"] = store_state_digest(state)
    return state


def validate_act_lineage_store_state(state: Mapping[str, Any]) -> list[str]:
    errors: list[str] = []
    try:
        if state.get("version") != STORE_STATE_VERSION:
            errors.append("act_lineage_store_state_version_invalid")
        ...
        if state.get("act_lineage_store_state_digest") != store_state_digest(state):
            errors.append("act_lineage_store_state_digest_invalid")
        ...
        issued_digests = [str(item.get("handoff_receipt_digest", "")) for item in issued]
        completed_digests = [
            str(item.get("handoff_receipt_digest", "")) for item in completed
        ]
        if len(issued_keys) != len(set(issued_keys)):
            errors.append("act_lineage_store_single_use_key_duplicate")
        if len(issued_digests) != len(set(issued_digests)):
            errors.append("act_lineage_store_handoff_duplicate")
        if len(completed_digests) != len(set(completed_digests)):
            errors.append("act_lineage_store_completion_duplicate")
        if not set(completed_digests).issubset(set(issued_digests)):
            errors.append("act_lineage_store_completion_without_handoff")
        if len(processed) != len(set(processed)):
            errors.append("act_lineage_store_processed_duplicate")
        if int(state.get("commit_count", -1)) != len(processed):
            errors.append("act_lineage_store_commit_count_mismatch")
    except (TypeError, ValueError) as exc:
        errors.append(str(exc))
    return errors


class ActLineageStore:
    def __init__(self, root: str | Path) -> None:
        self.root = Path(root)
        self.genesis_path = self.root / "act-lineage-genesis.json"
        self.ledger_path = self.root / "act-lineage-ledger.jsonl"
        self.snapshot_path = self.root / "act-lineage-snapshot.json"
        self.lock_path = self.root / ".act-os-v02-lineage.lock"
    ...
    def _read_json(path: Path) -> dict[str, Any]:
        try:
            ...
            raise ActLineageStoreError(
                f"act_lineage_json_read_failed:{path.name}"
            ) from exc
        if not isinstance(value, dict):
            raise ActLineageStoreError(
                f"act_lineage_json_object_required:{path.name}"
            )
        return value

    def initialize(self, initial_state: Mapping[str, Any]) -> dict[str, Any]:
        errors = validate_act_lineage_store_state(initial_state)
        if errors:
            raise ActLineageStoreError(
                "initial_act_lineage_store_state_invalid:" + ";".join(errors)
            )
        with self._locked():
            if self.genesis_path.exists() or self.ledger_path.exists():
                raise ActLineageStoreError("act_lineage_store_already_initialized")
        ...
        try:
            with self.ledger_path.open("r", encoding="utf-8") as handle:
                for line_number, raw in enumerate(handle, start=1):
                    ...
                    if not line:
                        raise ActLineageStoreError(
                            f"act_lineage_ledger_blank_line:{line_number}"
                        )
                    item = json.loads(line)
                    if not isinstance(item, dict):
                        raise ActLineageStoreError(
                            f"act_lineage_ledger_object_required:{line_number}"
                        )
                    commits.append(item)
        except (OSError, json.JSONDecodeError) as exc:
            raise ActLineageStoreError("act_lineage_ledger_read_failed") from exc
        return commits

    @staticmethod
    def _apply_receipt(
        state: Mapping[str, Any], stage: str, receipt: Mapping[str, Any], now_ms: int
    ) -> tuple[str, dict[str, Any]]:
        if stage not in STAGES:
            raise ActLineageStoreError("act_lineage_stage_invalid")
        next_state = deepcopy(dict(state))
        if stage == STAGE_HANDOFF:
            errors = validate_handoff_receipt(receipt)
            if errors:
                raise ActLineageStoreError(
                    "act_lineage_handoff_invalid:" + ";".join(errors)
                )
            digest = str(receipt["act_lineage_handoff_receipt_digest"])
            ...
            if key in {
                ...
                raise ActLineageStoreError("act_lineage_handoff_already_issued")
            next_state["issued_handoffs"] = list(next_state["issued_handoffs"]) + [
                {
                    "single_use_key": key,
                    "handoff_receipt_digest": digest,
                    "next_cycle_compiler_receipt_digest": receipt[
                        "next_cycle_compiler_receipt_digest"
                    ],
                    "selected_step_digest": receipt["selected_step_digest"],
            ...
            if errors:
                raise ActLineageStoreError(
                    "act_lineage_completion_invalid:" + ";".join(errors)
                )
            digest = str(receipt["act_lineage_completion_receipt_digest"])
            if digest in set(next_state["processed_receipt_digests"]):
                return "REPLAYED", next_state
            handoff_digest = str(receipt["act_lineage_handoff_receipt_digest"])
            issued = {
            ...
            if handoff_digest not in issued:
                raise ActLineageStoreError(
                    "act_lineage_completion_handoff_not_issued"
                )
            completed = {
                str(item["handoff_receipt_digest"])
            ...
            if handoff_digest in completed:
                raise ActLineageStoreError(
                    "act_lineage_completion_already_committed"
                )
            next_state["completed_handoffs"] = list(
            ...
            ) + [
                {
                    ...
                    "host_invocation_digest": receipt["host_invocation_digest"],
                }
            ]
        ...
        next_state["act_lineage_store_state_digest"] = ""
        next_state["act_lineage_store_state_digest"] = store_state_digest(next_state)
        errors = validate_act_lineage_store_state(next_state)
        if errors:
            raise ActLineageStoreError(
                "next_act_lineage_store_state_invalid:" + ";".join(errors)
            )
    ...
    def _recover_unlocked(self) -> tuple[dict[str, Any], list[dict[str, Any]]]:
        if not self.genesis_path.exists() or not self.ledger_path.exists():
            raise ActLineageStoreError("act_lineage_store_not_initialized")
        state = self._read_json(self.genesis_path)
        errors = validate_act_lineage_store_state(state)
        if errors:
            raise ActLineageStoreError(
                "act_lineage_genesis_invalid:" + ";".join(errors)
            )
        ...
        for index, commit in enumerate(commits, start=1):
            if commit.get("version") != STORE_COMMIT_VERSION:
                raise ActLineageStoreError(
                    f"act_lineage_commit_version_invalid:{index}"
                )
            if commit.get("act_lineage_store_commit_digest") != store_commit_digest(
                commit
            ):
                raise ActLineageStoreError(
                    f"act_lineage_commit_digest_invalid:{index}"
                )
            if commit.get("predecessor_commit_digest") != previous_commit_digest:
                raise ActLineageStoreError(
                    f"act_lineage_commit_chain_broken:{index}"
                )
            if commit.get("predecessor_state_digest") != state.get(
                "act_lineage_store_state_digest"
            ):
                raise ActLineageStoreError(
                    f"act_lineage_state_chain_broken:{index}"
                )
            ...
            if status != "COMMITTED":
                raise ActLineageStoreError(
                    f"act_lineage_recovery_replay_unexpected:{index}"
                )
            if commit.get("result_state_digest") != state.get(
                "act_lineage_store_state_digest"
            ):
                raise ActLineageStoreError(
                    f"act_lineage_result_digest_mismatch:{index}"
                )
            previous_commit_digest = commit["act_lineage_store_commit_digest"]
    ...
    def recover(self, *, require_snapshot_match: bool = True) -> dict[str, Any]:
        with self._locked():
            ...
            if require_snapshot_match:
                ...
                if snapshot.get("act_lineage_store_state_digest") != state.get(
                    "act_lineage_store_state_digest"
                ):
                    raise ActLineageStoreError(
                        "act_lineage_snapshot_ledger_mismatch"
                    )
        ...
        with self._locked():
            ...
            commit = {
                ...
                "predecessor_commit_digest": commits[-1][
                    "act_lineage_store_commit_digest"
                ]
                if commits
                else "",
            ...
            commit["act_lineage_store_commit_digest"] = store_commit_digest(commit)
            with self.ledger_path.open("a", encoding="utf-8") as handle:
                handle.write(canonical_json(commit) + "\n")
                handle.flush()
                os.fsync(handle.fileno())
            self._write_atomic(self.snapshot_path, next_state)
            return {"status": "COMMITTED", "state": deepcopy(next_state)}
