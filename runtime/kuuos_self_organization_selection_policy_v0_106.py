...
from runtime import kuuos_current_root_sequence_v0_105 as current_root
from runtime import kuuos_self_organization_candidate_receipt_v0_105 as receipt

VERSION = "kuuos_self_organization_selection_policy_v0_106"
DEPENDS_ON = receipt.VERSION
CURRENT_ROOT_SEQUENCE = current_root.VERSION
SELECTION_POLICY_PATH = "status/self_organization_selection_policy_v0_106.json"
POLICY_SCHEMA_VERSION = "v0.106"
...
def expected_ranking_rules() -> list[str]:
    return [
        "candidate_must_exist_in_v0_105_receipt",
        "candidate_must_preserve_pr_path",
        "candidate_must_require_governance_gate",
        "candidate_must_have_single_next_stage_scope",
        "candidate_must_preserve_phase_checkpoint",
    ]
...
def expected_selection_policy() -> dict[str, Any]:
    return {
        ...
        "candidate_receipt_runtime": "runtime/kuuos_self_organization_candidate_receipt_v0_105.py",
        "current_root_check": "runtime/kuuos_current_check.py",
        "current_root_sequence": CURRENT_ROOT_SEQUENCE,
        "effect_authorized": False,
        "next_artifact": "status/self_organization_selected_next_action_v0_107.json",
        "next_stage": "v0.107",
        "policy_frontier": VERSION,
        "policy_mode": "policy_only",
        "policy_schema_version": POLICY_SCHEMA_VERSION,
        "ranking_rules": expected_ranking_rules(),
...
def selection_policy_issues() -> tuple[str, ...]:
    ...
    if policy.get("ranking_rules") != expected_ranking_rules():
        issues.append("ranking_rules")
    if policy.get("tie_breaker") != "lowest_expected_next_stage":
        issues.append("tie_breaker")
    if policy.get("next_artifact") != "status/self_organization_selected_next_action_v0_107.json":
        issues.append("next_artifact")
    if policy.get("next_stage") != "v0.107":
    ...
    if receipt_payload.get("next_stage") != "v0.106":
