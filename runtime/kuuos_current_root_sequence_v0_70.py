...
from runtime import kuuos_current_root_sequence_v0_69 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_70"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
CURRENT_STATUS_POINTER_FRONTIER = "kuuos_current_status_pointer_v0_70"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "current-status-pointer-v0-70",
        "unittest",
        "tests.test_kuuos_current_status_pointer_v0_70",
        True,
        "Stable current status pointer must point to the verified status index.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-70",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_70",
        True,
        "This layer connects the stable current status pointer into the current runtime root.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if CURRENT_STATUS_POINTER_FRONTIER != "kuuos_current_status_pointer_v0_70":
        issues.append("current_status_pointer_frontier")
    return tuple(issues)
