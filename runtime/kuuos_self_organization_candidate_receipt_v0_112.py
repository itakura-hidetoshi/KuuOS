...
from runtime import kuuos_current_root_sequence_v0_111 as current_root
from runtime import kuuos_self_organization_candidate_queue_v0_111 as queue

VERSION = "kuuos_self_organization_candidate_receipt_v0_112"
DEPENDS_ON = queue.VERSION
CURRENT_ROOT_SEQUENCE = current_root.VERSION
CANDIDATE_RECEIPT_PATH = "status/self_organization_candidate_receipt_v0_112.json"
CANDIDATE_RECEIPT_SCHEMA_VERSION = "v0.112"
...
def expected_received_candidate_ids() -> list[str]:
    return queue.expected_candidate_ids()


def expected_candidate_receipt() -> dict[str, Any]:
    return {
        "candidate_receipt_frontier": VERSION,
        "candidate_receipt_schema_version": CANDIDATE_RECEIPT_SCHEMA_VERSION,
        "current_root_check": "runtime/kuuos_current_check.py",
        "current_root_sequence": CURRENT_ROOT_SEQUENCE,
        "next_artifact": "status/self_organization_selection_policy_v0_113.json",
        "next_stage": "v0.113",
        "queue": queue.CANDIDATE_QUEUE_PATH,
        "queue_frontier": queue.VERSION,
        "receipt_mode": "candidate_receipt_only",
        "receipt_scope": "self_organization_next_cycle",
        "received_candidate_count": len(expected_received_candidate_ids()),
        "received_candidate_ids": expected_received_candidate_ids(),
        "source_runtime": "runtime/kuuos_self_organization_candidate_queue_v0_111.py",
    }
...
def candidate_receipt_issues() -> tuple[str, ...]:
    ...
    if receipt.get("received_candidate_count") != len(receipt.get("received_candidate_ids", [])):
        issues.append("received_candidate_count")
    if receipt.get("received_candidate_ids") != expected_received_candidate_ids():
        issues.append("received_candidate_ids")
    if receipt.get("receipt_mode") != "candidate_receipt_only":
        issues.append("receipt_mode")
    if receipt.get("receipt_scope") != "self_organization_next_cycle":
        issues.append("receipt_scope")
    if receipt.get("next_artifact") != "status/self_organization_selection_policy_v0_113.json":
        issues.append("next_artifact")
    if receipt.get("next_stage") != "v0.113":
    ...
    if source.get("next_stage") != "v0.112":
        issues.append("queue_next_stage")
    if source.get("queue_mode") != "candidate_queue_only":
