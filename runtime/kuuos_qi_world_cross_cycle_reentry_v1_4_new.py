    ...
    state = apply_belief(
        ...
        "contextualize",
        {
            "context": {
                ...
                "world_model_id": previous_receipt[
...
def _build_next_wa(root: Path, plural: Mapping[str, Any]) -> dict[str, Any]:
...
def _build_next_plan(root: Path, wa: Mapping[str, Any]) -> dict[str, Any]:
...
def _validate_next_artifacts(
    ...
    if not isinstance(activation, Mapping):
        ...
        if activation.get(
            "belief_activation_receipt_digest"
        ) != belief_activation_digest(activation):
            errors.append("BeliefActivation_digest_invalid")
    return errors


def _expected_process_lineage_digest(
...
def _build_cross_cycle_qi_receipt(
    ...
    stages = [
        ...
        (
            ...
            next_artifacts["BeliefOS"]["belief_state_digest"],
        ),
        (
            "process_action",
            "NEXT_DECISION_COMMITTED",
            next_artifacts["DecisionOS"]["decision_state_digest"],
        ),
        (
            "process_action",
            "NEXT_WA_COMMITTED",
            next_artifacts["DecisionOSWa"]["wa_state_digest"],
        ),
        (
            "process_action",
            "NEXT_PLAN_COMMITTED",
            next_artifacts["PlanOS"]["plan_state_digest"],
        ),
    ]
    history: list[dict[str, Any]] = []
    predecessor = previous["native_full_cycle_receipt_digest"]
    for index, (field, kind, digest) in enumerate(stages):
        history.append(
            {
                field: kind,
                "native_artifact_digest": digest,
                "source_state_digest": predecessor,
...
def build_cross_cycle_reentry_receipt(root: Path) -> dict[str, Any]:
    ...
    plan = _build_next_plan(root / "next-plan", wa)
    ...
    receipt = {
        ...
        "cross_cycle_process_lineage_digest": _expected_process_lineage_digest(
...
def validate_cross_cycle_reentry_receipt(
    ...
    try:
        ...
        if not required_evidence.issubset(
            set(belief.get("evidence_digests", []))
        ):
        ...
        if plan.get("current_phase") != "commit":
            errors.append("cross_cycle_plan_not_committed")
        ...
        expected_lineage = _expected_process_lineage_digest(
            previous, next_artifacts
        )
        if receipt.get("cross_cycle_process_lineage_digest") != expected_lineage:
            errors.append("cross_cycle_process_lineage_digest_mismatch")
        ...
        for field in (
            ...
            if qi_receipt.get(field) is not False:
                errors.append(f"cross_cycle_qi_{field}_invalid")

        world = dict(receipt.get("cross_cycle_world_projection", {}))
        if world.get("world_projection_digest") != _digest_without(
            world, "world_projection_digest"
        ):
            errors.append("cross_cycle_world_projection_invalid")
        ...
        if receipt.get("next_act_not_started") is not True:
            errors.append("cross_cycle_next_act_boundary_invalid")
