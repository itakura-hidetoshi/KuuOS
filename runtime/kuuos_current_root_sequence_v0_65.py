...
from runtime import kuuos_current_root_sequence_v0_41 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_65"
...
CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "self-organization-bounded-execution-v0-64",
        "unittest",
        "tests.test_kuuos_self_organization_bounded_execution_v0_64",
        True,
        "Active self-organization state must be checked from the current root.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-65",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_65",
        True,
        "This layer connects the bounded execution frontier into the current runtime root.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if EXECUTION_FRONTIER != "kuuos_self_organization_bounded_execution_v0_64":
        issues.append("execution_frontier")
    return tuple(issues)
