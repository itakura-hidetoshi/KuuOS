...
from runtime import kuuos_current_root_sequence_v0_100 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_101"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
BOUNDED_CHANGE_PLAN_FRONTIER = "kuuos_self_organization_bounded_change_plan_v0_101"
SELF_CHECK_TARGET = "runtime.kuuos_current_root_sequence_v0_101:run_self_check"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "bounded-change-plan-v0-101",
        "unittest",
        "tests.test_kuuos_self_organization_bounded_change_plan_v0_101",
        True,
        "Bounded change plan v0.101 is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-101",
        "callable",
        SELF_CHECK_TARGET,
        True,
        "Current root sequence v0.101 self-check is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if BOUNDED_CHANGE_PLAN_FRONTIER != "kuuos_self_organization_bounded_change_plan_v0_101":
        issues.append("bounded_change_plan_frontier")
    return tuple(issues)
