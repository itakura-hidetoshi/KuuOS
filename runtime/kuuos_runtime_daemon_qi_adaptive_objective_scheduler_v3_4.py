...
def _apply_adaptation(base_packet: Mapping[str, Any], scheduled: Mapping[str, Any], base_objective: str) -> tuple[dict[str, Any], str, int, float]:
    ...
    if objective != base_objective:
        default_cycles, default_threshold = _schedule_defaults(objective)
        base_cycles = default_cycles
    ...
    out = dict(scheduled)
    out["objective_class"] = objective
    out["max_cycles"] = adapted_cycles
    out["convergence_threshold"] = adapted_threshold
    out["adaptive_objective_hint"] = hint
    out["adaptive_cycle_delta"] = cycle_delta
    out["adaptive_threshold_delta"] = threshold_delta
    out["adaptive_scheduler_applied"] = True
...
def build_qi_adaptive_objective_scheduler(*, runtime_context: Mapping[str, Any], adaptive_license_packet: Mapping[str, Any]) -> QiAdaptiveObjectiveSchedulerResult:
    ...
    if not blockers:
        ...
        base = build_qi_circulation_objective_scheduler(runtime_context={"qi_circulation_scheduler_enabled": True, "apply_circulation_scheduler": True, "runtime_root": str(root)}, scheduler_license_packet=_scheduler_license())
        ...
        if base_payload.get("status") != "QI_CIRCULATION_SCHEDULER_READY":
            ...
            adapted = True
    if blockers:
        status = "QI_ADAPTIVE_SCHEDULER_BLOCKED"
    else:
        status = "QI_ADAPTIVE_SCHEDULER_READY"
