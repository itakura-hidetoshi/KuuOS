...
from runtime import kuuos_current_root_sequence_v0_93 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_94"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
BOUNDED_CHANGE_PLAN_FRONTIER = "kuuos_self_organization_bounded_change_plan_v0_94"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "bounded-change-plan-v0-94",
        "unittest",
        "tests.test_kuuos_self_organization_bounded_change_plan_v0_94",
        True,
        "Bounded change plan v0.94 is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-94",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_94",
        True,
        "Current root sequence v0.94 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if BOUNDED_CHANGE_PLAN_FRONTIER != "kuuos_self_organization_bounded_change_plan_v0_94":
