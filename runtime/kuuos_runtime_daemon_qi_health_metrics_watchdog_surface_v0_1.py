...
def _prom(name: str, value: int | float) -> str:
    return f'{name}{{daemon="qi_jsonl",surface="health_metrics_watchdog"}} {value}'
...
def build_qi_health_metrics_watchdog_surface(
    *,
    ...
    for request_key in ["request_probe_execution", "request_world_update", "request_memory_write", "request_control_packet_mutation"]:
        if ctx.get(request_key) is True:
            blockers.append(request_key)
    ...
    if status.get("resume_status") not in ("QI_JSONL_SAFE_RESUME_CONTROLLER_COMPLETED", None):
        blockers.append("daemon_resume_status_not_completed")
    for key in ["probe_execution_performed", "world_update_performed", "memory_overwrite_performed", "control_packet_mutation_performed"]:
        if status.get(key) is not False:
            blockers.append(f"daemon_{key}_not_false")
    ...
    cursor_monotone = cursor_position >= event_count if event_count else cursor_position >= 0
    ...
    if heartbeat_count < int(ctx.get("min_heartbeat_count", 0) or 0):
        warnings.append("heartbeat_count_below_threshold")
    if pressure == "high":
        warnings.append("process_tensor_pressure_high")

    health_ok = not blockers
    watchdog_ok = health_ok and "heartbeat_count_below_threshold" not in warnings
    prometheus = "\n".join([
        _prom("kuos_qi_daemon_health_ok", 1 if health_ok else 0),
        _prom("kuos_qi_daemon_watchdog_ok", 1 if watchdog_ok else 0),
        _prom("kuos_qi_daemon_heartbeat_count", heartbeat_count),
        _prom("kuos_qi_daemon_event_log_lines", event_count),
        _prom("kuos_qi_daemon_replay_cursor_position", cursor_position),
        _prom("kuos_qi_daemon_token_ledger_count", token_count),
        _prom("kuos_qi_process_tensor_pressure", _pressure_value(str(pressure) if pressure else None)),
    ]) + "\n"
    ...
    return QiHealthMetricsWatchdogSurface(
        ...
        daemon_resume_status=str(status.get("resume_status")) if status.get("resume_status") else None,
