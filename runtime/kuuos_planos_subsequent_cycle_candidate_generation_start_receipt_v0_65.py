...
VERSION = "kuuos_planos_subsequent_cycle_candidate_generation_start_receipt_v0_65"
SOURCE_VERSION = "kuuos_planos_subsequent_cycle_replan_request_v0_64"

SUBSEQUENT_CYCLE_CANDIDATE_GENERATION_START_RECEIPT_BOUNDARY = {
    "receipt_owned_by_plan_os": True,
    "source_subsequent_cycle_replan_request_preserved": True,
    "selected_candidate_bound_to_replan_request": True,
    ...
    "subsequent_cycle_candidate_generation_start_receipt_only": True,
    "subsequent_cycle_candidate_generation_started": True,
    "subsequent_cycle_candidate_set_materialized": False,
...
REQUIRED_SOURCE_BOUNDARY_TRUE = (
    ...
    "source_next_cycle_closeout_receipt_preserved",
    "selected_candidate_bound_to_next_cycle_closeout",
...
class SubsequentCycleCandidateGenerationStartRecord:
    ...
    subsequent_cycle_candidate_generation_start_digest: str
    start_scope: str = "subsequent_cycle_candidate_generation_start_receipt_only"
    ...
    subsequent_cycle_candidate_set_materialized: bool = False
...
@dataclass(frozen=True)
class SubsequentCycleCandidateGenerationStartReceipt:
    version: str
    source_version: str
    status: str
    source_subsequent_cycle_replan_request_digest: str
    selected_candidate_id: str
    selected_candidate_digest: str
    subsequent_cycle_candidate_generation_start_receipt: dict[str, Any] | None
    blockers: list[str]
...
def _source_replan_record(receipt: Mapping[str, Any]) -> Mapping[str, Any]:
    return _m(receipt.get("subsequent_cycle_replan_request"))


def _source_blockers(receipt: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    if receipt.get("version") != SOURCE_VERSION:
        blockers.append("source_subsequent_cycle_replan_request_version_invalid")
    if receipt.get("status") != "PLANOS_SUBSEQUENT_CYCLE_REPLAN_REQUEST_READY":
        blockers.append("source_subsequent_cycle_replan_request_not_ready")
    ...
    if not receipt.get("receipt_digest"):
        blockers.append("source_subsequent_cycle_replan_request_digest_missing")
    record = _source_replan_record(receipt)
    if not record:
        blockers.append("source_subsequent_cycle_replan_request_record_missing")
    if record:
        for field in (
            "memory_overwrite_preserved",
            "memory_overwrite_closeout_preserved",
...
def build_subsequent_cycle_candidate_generation_start_receipt(
    *,
    subsequent_cycle_replan_request: Mapping[str, Any],
) -> SubsequentCycleCandidateGenerationStartReceipt:
    source = _m(subsequent_cycle_replan_request)
    ...
    record = _source_replan_record(source)
    if not selected_id:
    ...
    if record:
        if selected_id and str(record.get("selected_candidate_id", "")) != selected_id:
            blockers.append("selected_candidate_id_subsequent_cycle_replan_request_mismatch")
        if selected_digest and str(record.get("selected_candidate_digest", "")) != selected_digest:
            blockers.append("selected_candidate_digest_subsequent_cycle_replan_request_mismatch")

    start_receipt = None
    if selected_id and selected_digest and not blockers:
        start_obj = SubsequentCycleCandidateGenerationStartRecord(
            selected_candidate_id=selected_id,
            selected_candidate_digest=selected_digest,
            source_subsequent_cycle_replan_request_digest=str(source.get("receipt_digest", "")),
            source_next_cycle_closeout_receipt_digest=str(record.get("source_next_cycle_closeout_receipt_digest", "")),
            ...
            subsequent_cycle_candidate_generation_start_digest=sha({
                "source_subsequent_cycle_replan_request_digest": source.get("receipt_digest"),
                "source_next_cycle_closeout_receipt_digest": record.get("source_next_cycle_closeout_receipt_digest"),
                ...
                "selected_candidate_id": selected_id,
                "selected_candidate_digest": selected_digest,
                "scope": "subsequent_cycle_candidate_generation_start_receipt_only",
            }),
        )
        start_receipt = start_obj.to_dict()

    status = (
        "PLANOS_SUBSEQUENT_CYCLE_CANDIDATE_GENERATION_START_RECEIPT_READY"
        if not blockers
        else "PLANOS_SUBSEQUENT_CYCLE_CANDIDATE_GENERATION_START_RECEIPT_BLOCKED"
    )
    partial = {
        "version": VERSION,
        ...
        "selected_candidate_id": selected_id,
        "selected_candidate_digest": selected_digest,
        "subsequent_cycle_candidate_generation_start_receipt": start_receipt,
        "blockers": blockers,
        "boundary": dict(SUBSEQUENT_CYCLE_CANDIDATE_GENERATION_START_RECEIPT_BOUNDARY),
    }
    return SubsequentCycleCandidateGenerationStartReceipt(receipt_digest=sha(partial), **partial)


def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print(
            "usage: kuuos_planos_subsequent_cycle_candidate_generation_start_receipt_v0_65.py SUBSEQUENT_CYCLE_REPLAN_REQUEST.json",
            file=sys.stderr,
        )
        return 2
    ...
    receipt = build_subsequent_cycle_candidate_generation_start_receipt(
        subsequent_cycle_replan_request=payload
    )
