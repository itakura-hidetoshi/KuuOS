...
def _unique_ids(items: list[Any], field: str, prefix: str, blockers: list[str]) -> set[str]:
    identifiers: list[str] = []
    for index, item in enumerate(items):
        identifier = str(_m(item).get(field, ""))
        if not identifier:
...
def _validate_local_patches(patches: list[Any], blockers: list[str]) -> set[str]:
    if len(patches) < 2:
        blockers.append("local_world_patches_requires_at_least_two")
    patch_ids = _unique_ids(patches, "patch_id", "local_patch", blockers)
    for index, raw in enumerate(patches):
        ...
        for field in (
            ...
            if not _nonempty_string(patch.get(field)):
                blockers.append(f"local_patch_{index}_{field}_missing")
        algebra = _m(patch.get("operator_algebra"))
        roles = {str(value) for value in _list(algebra.get("operator_roles"))}
        ...
        witnesses = _list(algebra.get("commutator_witnesses"))
        if not witnesses:
        ...
        for witness_index, raw_witness in enumerate(witnesses):
            ...
            if not _nonempty_string(witness.get("left_operator")) or not _nonempty_string(
                witness.get("right_operator")
            ):
...
def _validate_connections(
    ...
    connection_ids = _unique_ids(connections, "connection_id", "indra_connection", blockers)
    ...
    for index, raw in enumerate(connections):
        ...
        for field in ("connection_form_digest", "parallel_transport_digest"):
            if not _nonempty_string(connection.get(field)):
...
def _validate_qi_flows(
    ...
    _unique_ids(flows, "flow_id", "qi_flow", blockers)
    for index, raw in enumerate(flows):
        ...
        elif endpoints.get(connection_id) != (source, target):
        ...
        if not _nonempty_string(flow.get("flow_observable_digest")):
            blockers.append(f"qi_flow_{index}_flow_observable_digest_missing")
        context = _m(flow.get("process_tensor_context"))
...
def _validate_holonomy(cycles: list[Any], connection_ids: set[str], blockers: list[str]) -> None:
    ...
    _unique_ids(cycles, "cycle_id", "holonomy_cycle", blockers)
    for index, raw in enumerate(cycles):
        cycle = _m(raw)
        path = [str(value) for value in _list(cycle.get("connection_ids"))]
        if len(path) < 2:
        ...
        if not _nonempty_string(cycle.get("holonomy_digest")):
            blockers.append(f"holonomy_cycle_{index}_digest_missing")
        if not _nonempty_string(cycle.get("transport_residue_digest")):
...
def _validate_strings(strings: list[Any], patch_ids: set[str], blockers: list[str]) -> None:
    ...
    _unique_ids(strings, "string_id", "ku_string", blockers)
    for index, raw in enumerate(strings):
        string = _m(raw)
        if str(string.get("source_patch", "")) not in patch_ids:
            blockers.append(f"ku_string_{index}_source_patch_unknown")
        if str(string.get("target_patch", "")) not in patch_ids:
            blockers.append(f"ku_string_{index}_target_patch_unknown")
        if string.get("correspondence_kind") not in {
        ...
        if string.get("relation_substance_claim") is not False:
            blockers.append(f"ku_string_{index}_relation_substance_claim_not_false")
        if not _nonempty_string(string.get("correspondence_digest")):
...
def _validate_branes(branes: list[Any], patch_ids: set[str], blockers: list[str]) -> None:
    ...
    _unique_ids(branes, "surface_id", "m_brane_surface", blockers)
    kinds: set[str] = set()
...
def _validate_mandala(value: Mapping[str, Any], patch_ids: set[str], blockers: list[str]) -> None:
    included = {str(item) for item in _list(value.get("included_patch_ids"))}
    if included != patch_ids:
        blockers.append("mandala_inclusion_patch_set_mismatch")
    if value.get("multi_world_noncollapse") is not True:
        blockers.append("mandala_multi_world_noncollapse_not_true")
    if value.get("single_ontology_forced") is not False:
        blockers.append("mandala_single_ontology_forced_not_false")
    if value.get("contradiction_visibility_preserved") is not True:
        blockers.append("mandala_contradiction_visibility_not_preserved")


def _validate_boundaries(model: Mapping[str, Any], blockers: list[str]) -> None:
...
def build_indra_qi_world_model_v0_1(
    *,
    ...
    if ctx.get("indra_qi_world_model_enabled") is not True:
        blockers.append("indra_qi_world_model_enabled_not_true")
    if ctx.get("build_indra_qi_world_model") is not True:
    ...
    patches = _list(model.get("local_world_patches"))
    connections = _list(model.get("indra_connections"))
    flows = _list(model.get("qi_flow_channels"))
    cycles = _list(model.get("holonomy_cycles"))
    strings = _list(model.get("ku_string_correspondences"))
    branes = _list(model.get("extended_m_brane_surfaces"))

    patch_ids = _validate_local_patches(patches, blockers)
    connection_ids, endpoints = _validate_connections(connections, patch_ids, blockers)
    _validate_qi_flows(flows, connection_ids, endpoints, blockers)
    ...
    _validate_mandala(_m(model.get("mandala_inclusion")), patch_ids, blockers)
    _validate_boundaries(model, blockers)
    ...
    status = "INDRA_QI_WORLD_MODEL_READY" if build_status.endswith("ready") else "INDRA_QI_WORLD_MODEL_BLOCKED"
    receipt = {
        "version": "kuuos_runtime_daemon_indra_qi_world_model_v0_1",
        "status": status,
        "packet_id": "indra-qi-world-model-" + _sha(
            {"world_model_id": world_model_id, "state_digest": state_digest, "blockers": blockers}
        )[:16],
        "world_model_id": world_model_id,
