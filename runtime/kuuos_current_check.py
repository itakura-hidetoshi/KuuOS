...
_PLANOS_STEP_SPECS: tuple[tuple[str, str, str], ...] = (
...
PLANOS_ACTIVE_FRONTIER_STEPS: tuple[CurrentRootStep, ...] = tuple(
    CurrentRootStep(step_id, "script", target, True, description)
    for step_id, target, description in _PLANOS_STEP_SPECS
)
...
def _run_script(target: str) -> int:
    ...
    return subprocess.run([sys.executable, str(script)], cwd=ROOT, env=env, check=False).returncode
...
def run_current(profile: str = "all") -> int:
    ...
    for ordinal, step in enumerate(steps, start=1):
        print(f"\n[{ordinal}/{len(steps)}] {step.step_id}", flush=True)
        try:
            status = _run_step(step.runner, step.target)
        except Exception as exc:
            status = 1
            print(f"current_root_exception:{step.step_id}:{type(exc).__name__}:{exc}", file=sys.stderr)
        if status == 0:
    ...
    print(
        ...
        f"({CURRENT_REPOSITORY_FRONTIER}; {CURRENT_PLANOS_FRONTIER}; {CURRENT_DECISIONOS_FRONTIER})"
    )
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--profile", choices=("all", "repository", "planos", "decisionos"), default="all")
