...
from runtime import kuuos_current_root_sequence_v0_95 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_96"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
NEXT_CYCLE_SEED_FRONTIER = "kuuos_self_organization_next_cycle_seed_v0_96"
SELF_CHECK_TARGET = "runtime.kuuos_current_root_sequence_v0_96:run_self_check"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "next-cycle-seed-v0-96",
        "unittest",
        "tests.test_kuuos_self_organization_next_cycle_seed_v0_96",
        True,
        "Next cycle seed v0.96 is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-96",
        "callable",
        SELF_CHECK_TARGET,
        True,
        "Current root sequence v0.96 self-check is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if NEXT_CYCLE_SEED_FRONTIER != "kuuos_self_organization_next_cycle_seed_v0_96":
        issues.append("next_cycle_seed_frontier")
    return tuple(issues)
