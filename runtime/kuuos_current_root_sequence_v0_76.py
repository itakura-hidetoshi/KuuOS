...
from runtime import kuuos_current_root_sequence_v0_75 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_76"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
CURRENT_STATUS_SURFACE_INDEX_FRONTIER = "kuuos_current_status_surface_index_v0_76"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "current-status-surface-index-v0-76",
        "unittest",
        "tests.test_kuuos_current_status_surface_index_v0_76",
        True,
        "Surface index is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-76",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_76",
        True,
        "Current root sequence v0.76 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if CURRENT_STATUS_SURFACE_INDEX_FRONTIER != "kuuos_current_status_surface_index_v0_76":
        issues.append("current_status_surface_index_frontier")
    return tuple(issues)
