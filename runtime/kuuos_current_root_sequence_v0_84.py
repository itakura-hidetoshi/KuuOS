...
from runtime import kuuos_current_root_sequence_v0_83 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_84"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
NEXT_REQUEST_FRONTIER = "kuuos_self_organization_next_request_v0_84"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "next-request-v0-84",
        "unittest",
        "tests.test_kuuos_self_organization_next_request_v0_84",
        True,
        "Next request is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-84",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_84",
        True,
        "Current root sequence v0.84 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if NEXT_REQUEST_FRONTIER != "kuuos_self_organization_next_request_v0_84":
        issues.append("next_request_frontier")
    return tuple(issues)
