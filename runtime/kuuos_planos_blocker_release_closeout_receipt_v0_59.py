...
VERSION = "kuuos_planos_blocker_release_closeout_receipt_v0_59"
SOURCE_VERSION = "kuuos_planos_blocker_release_receipt_v0_58"

BLOCKER_RELEASE_CLOSEOUT_BOUNDARY = {
    "closeout_owned_by_plan_os": True,
    "source_blocker_release_receipt_preserved": True,
    "selected_candidate_bound_to_blocker_release_receipt": True,
    ...
    "blocker_release_closeout_receipt_only": True,
    "blocker_release_cycle_closed": True,
    "next_cycle_admission_requested": False,
}

REQUIRED_SOURCE_BOUNDARY_TRUE = (
    "receipt_owned_by_plan_os",
    "source_blocker_release_authorization_grant_preserved",
    "selected_candidate_bound_to_blocker_release_grant",
...
class BlockerReleaseCloseoutRecord:
    ...
    blocker_release_closeout_digest: str
    closeout_scope: str = "blocker_release_closeout_receipt_only"
    ...
    blocker_release_authorization_grant_preserved: bool = True
    blocker_release_preserved: bool = True
    blocker_release_cycle_closed: bool = True
    next_cycle_admission_ready: bool = False
...
@dataclass(frozen=True)
class BlockerReleaseCloseoutReceipt:
    version: str
    source_version: str
    status: str
    source_blocker_release_receipt_digest: str
    selected_candidate_id: str
    selected_candidate_digest: str
    blocker_release_closeout_receipt: dict[str, Any] | None
    blockers: list[str]
...
def _source_record(receipt: Mapping[str, Any]) -> Mapping[str, Any]:
    return _m(receipt.get("blocker_release_receipt"))


def _source_blockers(receipt: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    if receipt.get("version") != SOURCE_VERSION:
        blockers.append("source_blocker_release_receipt_version_invalid")
    if receipt.get("status") != "PLANOS_BLOCKER_RELEASE_RECEIPT_READY":
        blockers.append("source_blocker_release_receipt_not_ready")
    ...
    if not receipt.get("receipt_digest"):
        blockers.append("source_blocker_release_receipt_digest_missing")
    record = _source_record(receipt)
    if not record:
        blockers.append("source_blocker_release_receipt_record_missing")
    if record:
        for field in (
            "memory_overwrite_preserved",
            "memory_overwrite_closeout_preserved",
        ...
        if _truthy(record.get("blocker_release_cycle_closed")):
            blockers.append("source_record_blocker_release_cycle_closed_promoted")
    return blockers


def build_blocker_release_closeout_receipt(
    *,
    blocker_release_receipt: Mapping[str, Any],
) -> BlockerReleaseCloseoutReceipt:
    source = _m(blocker_release_receipt)
    ...
    if record:
        if selected_id and str(record.get("selected_candidate_id", "")) != selected_id:
            blockers.append("selected_candidate_id_blocker_release_receipt_mismatch")
        if selected_digest and str(record.get("selected_candidate_digest", "")) != selected_digest:
            blockers.append("selected_candidate_digest_blocker_release_receipt_mismatch")

    closeout_record = None
    if selected_id and selected_digest and not blockers:
        closeout = BlockerReleaseCloseoutRecord(
            selected_candidate_id=selected_id,
            selected_candidate_digest=selected_digest,
            source_blocker_release_receipt_digest=str(source.get("receipt_digest", "")),
            source_blocker_release_authorization_grant_digest=str(
            ...
            blocker_release_closeout_digest=sha({
                "source_blocker_release_receipt_digest": source.get("receipt_digest"),
                ...
                "selected_candidate_id": selected_id,
                "selected_candidate_digest": selected_digest,
                "scope": "blocker_release_closeout_receipt_only",
            }),
        )
        closeout_record = closeout.to_dict()

    status = (
        "PLANOS_BLOCKER_RELEASE_CLOSEOUT_RECEIPT_READY"
        if not blockers
        else "PLANOS_BLOCKER_RELEASE_CLOSEOUT_RECEIPT_BLOCKED"
    )
    partial = {
        "version": VERSION,
        ...
        "selected_candidate_id": selected_id,
        "selected_candidate_digest": selected_digest,
        "blocker_release_closeout_receipt": closeout_record,
        "blockers": blockers,
        "boundary": dict(BLOCKER_RELEASE_CLOSEOUT_BOUNDARY),
    }
    return BlockerReleaseCloseoutReceipt(receipt_digest=sha(partial), **partial)


def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print(
            "usage: kuuos_planos_blocker_release_closeout_receipt_v0_59.py BLOCKER_RELEASE_RECEIPT.json",
            file=sys.stderr,
        )
        return 2
    with open(argv[1], "r", encoding="utf-8") as handle:
        payload = json.load(handle)
    receipt = build_blocker_release_closeout_receipt(blocker_release_receipt=payload)
