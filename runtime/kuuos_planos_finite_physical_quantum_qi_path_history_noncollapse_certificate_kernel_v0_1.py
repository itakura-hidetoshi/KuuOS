...
SCHEMA_VERSION = (
    "kuuos.planos.finite-physical-quantum-qi-path-history-"
    "noncollapse-certificate.v0.1"
)
...
def issue_finite_physical_quantum_qi_path_history_noncollapse_certificate(
    payload,
):
    top_fields = {
        "schema_version",
        "source_v120_certificate_digest",
    ...
    if (
        not _positive_nat(qi_dimension)
        or qi_dimension > MAXIMUM_QI_DIMENSION
    ):
    ...
    if compiled_histories:
        roots = {
            item["root_state_id"] for item in compiled_histories
        }
        if len(roots) != 1:
    ...
    input_digest = compute_path_history_input_digest(
        source_v120_certificate_digest=payload[
            "source_v120_certificate_digest"
        ],
