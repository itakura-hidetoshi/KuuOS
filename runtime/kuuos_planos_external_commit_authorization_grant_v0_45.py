...
VERSION = "kuuos_planos_external_commit_authorization_grant_v0_45"
SOURCE_VERSION = "kuuos_planos_external_commit_authorization_request_v0_44"

EXTERNAL_COMMIT_AUTHORIZATION_GRANT_BOUNDARY = {
    "grant_owned_by_plan_os": True,
    "source_external_commit_authorization_request_preserved": True,
    "selected_candidate_bound_to_external_commit_request": True,
    ...
    "external_commit_authorization_grant_only": True,
...
REQUIRED_SOURCE_BOUNDARY_TRUE = (
    ...
    "source_execution_receipt_preserved",
    ...
    "execution_granted",
    "external_commit_authorization_request_only",
...
@dataclass(frozen=True)
class ExternalCommitAuthorizationGrantReceipt:
    version: str
    source_version: str
    status: str
    source_external_commit_authorization_request_digest: str
    selected_candidate_id: str
    selected_candidate_digest: str
    external_commit_authorization_grant: dict[str, Any] | None
    blockers: list[str]
...
def _source_request_record(receipt: Mapping[str, Any]) -> Mapping[str, Any]:
    return _m(receipt.get("external_commit_authorization_request"))


def _source_blockers(receipt: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    if receipt.get("version") != SOURCE_VERSION:
        blockers.append("source_external_commit_authorization_request_version_invalid")
    if receipt.get("status") != "PLANOS_EXTERNAL_COMMIT_AUTHORIZATION_REQUEST_READY":
        blockers.append("source_external_commit_authorization_request_not_ready")
    ...
    if not receipt.get("receipt_digest"):
        blockers.append("source_external_commit_authorization_request_digest_missing")
    record = _source_request_record(receipt)
    if not record:
        blockers.append("source_external_commit_authorization_request_record_missing")
    if record:
        for required in (
            "execution_authorization_requested",
            "execution_authorization_granted",
        ...
        if _truthy(record.get("external_commit_authorization_granted")):
            blockers.append("source_record_external_commit_authorization_granted_promoted")
        for closed in (
...
def build_external_commit_authorization_grant(
    *,
    external_commit_request: Mapping[str, Any],
) -> ExternalCommitAuthorizationGrantReceipt:
    source = _m(external_commit_request)
    ...
    if record:
        if selected_id and str(record.get("selected_candidate_id", "")) != selected_id:
            blockers.append("selected_candidate_id_external_commit_request_mismatch")
        if selected_digest and str(record.get("selected_candidate_digest", "")) != selected_digest:
            blockers.append("selected_candidate_digest_external_commit_request_mismatch")

    grant = None
    if selected_id and selected_digest and not blockers:
        grant_obj = ExternalCommitAuthorizationGrantRecord(
            selected_candidate_id=selected_id,
            selected_candidate_digest=selected_digest,
            source_external_commit_authorization_request_digest=str(source.get("receipt_digest", "")),
            source_execution_receipt_digest=str(record.get("source_execution_receipt_digest", "")),
            external_commit_authorization_grant_digest=_digest(
                {
                    "source_external_commit_authorization_request_digest": source.get("receipt_digest"),
                    "source_execution_receipt_digest": record.get("source_execution_receipt_digest"),
                    "selected_candidate_id": selected_id,
                    "selected_candidate_digest": selected_digest,
                    "scope": "external_commit_authorization_grant_only",
                }
            ),
            grant_scope="external_commit_authorization_grant_only",
        )
        grant = grant_obj.to_dict()

    status = "PLANOS_EXTERNAL_COMMIT_AUTHORIZATION_GRANT_READY" if not blockers else "PLANOS_EXTERNAL_COMMIT_AUTHORIZATION_GRANT_BLOCKED"
    partial = {
        "version": VERSION,
        ...
        "selected_candidate_id": selected_id,
        "selected_candidate_digest": selected_digest,
        "external_commit_authorization_grant": grant,
        "blockers": list(blockers),
        "boundary": dict(EXTERNAL_COMMIT_AUTHORIZATION_GRANT_BOUNDARY),
    }
    return ExternalCommitAuthorizationGrantReceipt(receipt_digest=_digest(partial), **partial)


def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print("usage: kuuos_planos_external_commit_authorization_grant_v0_45.py EXTERNAL_COMMIT_REQUEST.json", file=sys.stderr)
        return 2
    with open(argv[1], "r", encoding="utf-8") as handle:
        payload = json.load(handle)
    receipt = build_external_commit_authorization_grant(external_commit_request=payload)
