...
from runtime import kuuos_current_root_sequence_v0_89 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_90"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
CANDIDATE_QUEUE_FRONTIER = "kuuos_self_organization_candidate_queue_v0_90"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "candidate-queue-v0-90",
        "unittest",
        "tests.test_kuuos_self_organization_candidate_queue_v0_90",
        True,
        "Candidate queue is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-90",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_90",
        True,
        "Current root sequence v0.90 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if CANDIDATE_QUEUE_FRONTIER != "kuuos_self_organization_candidate_queue_v0_90":
