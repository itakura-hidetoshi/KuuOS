...
from runtime import kuuos_current_root_sequence_v0_92 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_93"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
SELECTED_NEXT_ACTION_FRONTIER = "kuuos_self_organization_selected_next_action_v0_93"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "selected-next-action-v0-93",
        "unittest",
        "tests.test_kuuos_self_organization_selected_next_action_v0_93",
        True,
        "Selected next action v0.93 is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-93",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_93",
        True,
        "Current root sequence v0.93 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if SELECTED_NEXT_ACTION_FRONTIER != "kuuos_self_organization_selected_next_action_v0_93":
