...
from runtime import kuuos_current_root_sequence_v0_90 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_91"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
CANDIDATE_RECEIPT_FRONTIER = "kuuos_self_organization_candidate_receipt_v0_91"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "candidate-receipt-v0-91",
        "unittest",
        "tests.test_kuuos_self_organization_candidate_receipt_v0_91",
        True,
        "Candidate receipt is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-91",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_91",
        True,
        "Current root sequence v0.91 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if CANDIDATE_RECEIPT_FRONTIER != "kuuos_self_organization_candidate_receipt_v0_91":
