...
from runtime.kuuos_decision_os_wa_state_v0_2 import (
...
def _normalize_profile(
    ...
    if option_id not in set(state["source_admissible_option_ids"]):
        raise ValueError("wa_profile_option_not_source_admissible")
...
def _source_targets(state: Mapping[str, Any]) -> list[str]:
    route = state["source_route"]
    if route == "SELECT_CANDIDATE":
        selected = str(state["source_selected_option_id"])
        return [selected] if selected else []
    if route == "EXPERIMENT_RECOMMENDED":
        return list(state["source_recommended_option_ids"])
    return []
...
def _decide_gate(state: Mapping[str, Any]) -> tuple[str, list[str], list[str]]:
    ...
    if source_route in direct_map:
        return direct_map[source_route], [], []

    targets = _source_targets(state)
    if not targets:
        return "QUARANTINE", [], []
    ...
    if not set(targets).issubset(set(records)):
        return "QUARANTINE", [], targets

    confirmed = [
        option_id
        for option_id in targets
        if false_records[option_id]["confirmed_false_harmony"]
    ]
    if confirmed:
        return "ESCALATE", [], confirmed
    suspected = [
        option_id
        for option_id in targets
        if false_records[option_id]["suspected_false_harmony"]
    ]
    if suspected:
        return "REOBSERVE", [], suspected

    minimum_wa = float(state["thresholds"]["minimum_wa_floor"])
    minimum_dimension = float(state["thresholds"]["minimum_dimension_floor"])
    failed = [
        option_id
        for option_id in targets
        if float(records[option_id]["wa_interval"]["lower"]) < minimum_wa
        or float(records[option_id]["weakest_interval"]["lower"])
        < minimum_dimension
    ]
    if failed:
        return "REPAIR", [], failed
    return "ENDORSE", targets, []
...
def _apply_phase_payload(
    ...
    if target_phase == "profile":
        ...
        if set(profile_ids) != set(state["source_admissible_option_ids"]):
            raise ValueError("wa_all_source_admissible_options_must_be_profiled")
    ...
    if target_phase == "plurality_check":
        ...
        source_ids = set(state["source_admissible_option_ids"])
        selected = str(state["source_selected_option_id"])
        recommended = set(state["source_recommended_option_ids"])
        retained = set(state["source_retained_alternative_ids"])
        plurality = {
            "all_admissible_profiled": profile_ids == source_ids,
            ...
            "recommended_identities_preserved": recommended.issubset(profile_ids),
            "silent_substitution_detected": False,
        }
        state["plurality"] = plurality
    ...
    if target_phase == "gate":
        ...
        if not all(
            plurality[name] is True
            for name in (
                "all_admissible_profiled",
                "retained_alternatives_preserved",
                "selected_identity_preserved",
                "recommended_identities_preserved",
            )
        ...
        state["wa_basis_digest"] = sha(
            {
                ...
                "source_committed_decision_digest": state[
                ...
                "source_recommended_option_ids": state[
                    "source_recommended_option_ids"
                ],
        ...
        next_state["latest_committed_wa_digest"] = sha(
            {
                ...
                "source_committed_decision_digest": next_state[
                    "source_committed_decision_digest"
                ],
