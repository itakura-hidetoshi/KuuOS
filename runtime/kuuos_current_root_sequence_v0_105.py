...
from runtime import kuuos_current_root_sequence_v0_104 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_105"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
CANDIDATE_RECEIPT_FRONTIER = "kuuos_self_organization_candidate_receipt_v0_105"
SELF_CHECK_TARGET = "runtime.kuuos_current_root_sequence_v0_105:run_self_check"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "candidate-receipt-v0-105",
        "unittest",
        "tests.test_kuuos_self_organization_candidate_receipt_v0_105",
        True,
        "Candidate receipt v0.105 is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-105",
        "callable",
        SELF_CHECK_TARGET,
        True,
        "Current root sequence v0.105 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if CANDIDATE_RECEIPT_FRONTIER != "kuuos_self_organization_candidate_receipt_v0_105":
