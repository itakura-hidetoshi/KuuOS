...
VERSION = "kuuos_planos_truth_authority_authorization_grant_v0_53"
SOURCE_VERSION = "kuuos_planos_truth_authority_authorization_request_v0_52"

TRUTH_AUTHORITY_AUTHORIZATION_GRANT_BOUNDARY = {
    "grant_owned_by_plan_os": True,
    "source_truth_authority_authorization_request_preserved": True,
    "selected_candidate_bound_to_truth_authority_request": True,
    "memory_overwrite_preserved": True,
    "memory_overwrite_closeout_preserved": True,
    "cycle_closed_preserved": True,
    "truth_authority_authorization_grant_only": True,
...
REQUIRED_SOURCE_BOUNDARY_TRUE = (
    ...
    "source_memory_overwrite_closeout_preserved",
    "selected_candidate_bound_to_memory_overwrite_closeout",
...
class TruthAuthorityAuthorizationGrantRecord:
    selected_candidate_id: str
    selected_candidate_digest: str
    source_truth_authority_authorization_request_digest: str
    source_memory_overwrite_closeout_receipt_digest: str
    truth_authority_authorization_grant_digest: str
    grant_scope: str = "truth_authority_authorization_grant_only"
...
@dataclass(frozen=True)
class TruthAuthorityAuthorizationGrantReceipt:
    version: str
    source_version: str
    status: str
    source_truth_authority_authorization_request_digest: str
    selected_candidate_id: str
    selected_candidate_digest: str
    truth_authority_authorization_grant: dict[str, Any] | None
    blockers: list[str]
...
def _source_request_record(receipt: Mapping[str, Any]) -> Mapping[str, Any]:
    return _m(receipt.get("truth_authority_authorization_request"))


def _source_blockers(receipt: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    if receipt.get("version") != SOURCE_VERSION:
        blockers.append("source_truth_authority_authorization_request_version_invalid")
    if receipt.get("status") != "PLANOS_TRUTH_AUTHORITY_AUTHORIZATION_REQUEST_READY":
        blockers.append("source_truth_authority_authorization_request_not_ready")
    ...
    if not receipt.get("receipt_digest"):
        blockers.append("source_truth_authority_authorization_request_digest_missing")
    record = _source_request_record(receipt)
    if not record:
        blockers.append("source_truth_authority_authorization_request_record_missing")
    if record:
        ...
        if _truthy(record.get("memory_overwrite_closeout_preserved")) is not True:
            blockers.append("source_record_memory_overwrite_closeout_preserved_missing")
        if _truthy(record.get("cycle_closed_preserved")) is not True:
            blockers.append("source_record_cycle_closed_preserved_missing")
        if _truthy(record.get("truth_authority_authorization_requested")) is not True:
            blockers.append("source_record_truth_authority_authorization_requested_missing")
        for field in (
            "truth_authority_authorization_granted",
...
def build_truth_authority_authorization_grant(
    *,
    truth_authority_request: Mapping[str, Any],
) -> TruthAuthorityAuthorizationGrantReceipt:
    source = _m(truth_authority_request)
    ...
    if record:
        if str(record.get("selected_candidate_id", "")) != selected_id:
            blockers.append("selected_candidate_id_truth_authority_request_mismatch")
        if str(record.get("selected_candidate_digest", "")) != selected_digest:
            blockers.append("selected_candidate_digest_truth_authority_request_mismatch")

    grant = None
    if selected_id and selected_digest and not blockers:
        grant_obj = TruthAuthorityAuthorizationGrantRecord(
            selected_candidate_id=selected_id,
            selected_candidate_digest=selected_digest,
            source_truth_authority_authorization_request_digest=str(source.get("receipt_digest", "")),
            source_memory_overwrite_closeout_receipt_digest=str(
                record.get("source_memory_overwrite_closeout_receipt_digest", "")
            ),
            truth_authority_authorization_grant_digest=sha({
                "source_truth_authority_authorization_request_digest": source.get("receipt_digest"),
                "source_memory_overwrite_closeout_receipt_digest": record.get(
                    "source_memory_overwrite_closeout_receipt_digest"
                ),
                "selected_candidate_id": selected_id,
                "selected_candidate_digest": selected_digest,
                "scope": "truth_authority_authorization_grant_only",
            }),
        )
    ...
    status = (
        ...
        if not blockers
        else "PLANOS_TRUTH_AUTHORITY_AUTHORIZATION_GRANT_BLOCKED"
    )
    partial = {
        "version": VERSION,
        ...
        "selected_candidate_id": selected_id,
        "selected_candidate_digest": selected_digest,
        "truth_authority_authorization_grant": grant,
        "blockers": blockers,
        "boundary": dict(TRUTH_AUTHORITY_AUTHORIZATION_GRANT_BOUNDARY),
    }
    return TruthAuthorityAuthorizationGrantReceipt(receipt_digest=sha(partial), **partial)


def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print(
            "usage: kuuos_planos_truth_authority_authorization_grant_v0_53.py TRUTH_AUTHORITY_REQUEST.json",
            file=sys.stderr,
        )
        return 2
    with open(argv[1], "r", encoding="utf-8") as handle:
        payload = json.load(handle)
    receipt = build_truth_authority_authorization_grant(truth_authority_request=payload)
