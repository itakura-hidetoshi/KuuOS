...
from runtime import kuuos_current_root_sequence_v0_74 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_75"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
CURRENT_STATUS_SURFACE_ARTIFACT_FRONTIER = "kuuos_current_status_surface_artifact_v0_75"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "current-status-surface-artifact-v0-75",
        "unittest",
        "tests.test_kuuos_current_status_surface_artifact_v0_75",
        True,
        "Committed status surface artifact must match the v0.74 status surface output.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-75",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_75",
        True,
        "This layer connects the current status surface artifact into the current runtime root.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if CURRENT_STATUS_SURFACE_ARTIFACT_FRONTIER != "kuuos_current_status_surface_artifact_v0_75":
        issues.append("current_status_surface_artifact_frontier")
    return tuple(issues)
