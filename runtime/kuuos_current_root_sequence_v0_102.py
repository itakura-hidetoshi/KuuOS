...
from runtime import kuuos_current_root_sequence_v0_101 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_102"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
COMPLETION_RECEIPT_FRONTIER = "kuuos_self_organization_completion_receipt_v0_102"
SELF_CHECK_TARGET = "runtime.kuuos_current_root_sequence_v0_102:run_self_check"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "completion-receipt-v0-102",
        "unittest",
        "tests.test_kuuos_self_organization_completion_receipt_v0_102",
        True,
        "Completion receipt v0.102 is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-102",
        "callable",
        SELF_CHECK_TARGET,
        True,
        "Current root sequence v0.102 self-check is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if COMPLETION_RECEIPT_FRONTIER != "kuuos_self_organization_completion_receipt_v0_102":
        issues.append("completion_receipt_frontier")
    return tuple(issues)
