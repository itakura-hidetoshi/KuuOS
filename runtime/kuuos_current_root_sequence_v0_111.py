...
from runtime import kuuos_current_root_sequence_v0_110 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_111"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
CANDIDATE_QUEUE_FRONTIER = "kuuos_self_organization_candidate_queue_v0_111"
SELF_CHECK_TARGET = "runtime.kuuos_current_root_sequence_v0_111:run_self_check"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "candidate-queue-v0-111",
        "unittest",
        "tests.test_kuuos_self_organization_candidate_queue_v0_111",
        True,
        "Candidate queue v0.111 is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-111",
        "callable",
        SELF_CHECK_TARGET,
        True,
        "Current root sequence v0.111 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if CANDIDATE_QUEUE_FRONTIER != "kuuos_self_organization_candidate_queue_v0_111":
