...
from runtime import kuuos_current_root_sequence_v0_96 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_97"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
CANDIDATE_QUEUE_FRONTIER = "kuuos_self_organization_candidate_queue_v0_97"
SELF_CHECK_TARGET = "runtime.kuuos_current_root_sequence_v0_97:run_self_check"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "candidate-queue-v0-97",
        "unittest",
        "tests.test_kuuos_self_organization_candidate_queue_v0_97",
        True,
        "Candidate queue v0.97 is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-97",
        "callable",
        SELF_CHECK_TARGET,
        True,
        "Current root sequence v0.97 self-check is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if CANDIDATE_QUEUE_FRONTIER != "kuuos_self_organization_candidate_queue_v0_97":
        issues.append("candidate_queue_frontier")
    return tuple(issues)
