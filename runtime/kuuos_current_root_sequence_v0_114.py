...
from runtime import kuuos_current_root_sequence_v0_113 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_114"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
SELECTED_NEXT_ACTION_FRONTIER = "kuuos_self_organization_selected_next_action_v0_114"
SELF_CHECK_TARGET = "runtime.kuuos_current_root_sequence_v0_114:run_self_check"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "selected-next-action-v0-114",
        "unittest",
        "tests.test_kuuos_self_organization_selected_next_action_v0_114",
        True,
        "Selected next action v0.114 is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-114",
        "callable",
        SELF_CHECK_TARGET,
        True,
        "Current root sequence v0.114 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if SELECTED_NEXT_ACTION_FRONTIER != "kuuos_self_organization_selected_next_action_v0_114":
