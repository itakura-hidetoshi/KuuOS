...
from runtime import kuuos_current_root_sequence_v0_80 as previous
from runtime.kuuos_current_root_sequence_v0_41 import CurrentRootStep

VERSION = "kuuos_current_root_sequence_v0_81"
DEPENDS_ON = previous.VERSION
CURRENT_ROOT = "runtime/kuuos_current_check.py"
SELECTION_POLICY_FRONTIER = "kuuos_self_organization_selection_policy_v0_81"

CURRENT_ROOT_STEPS: tuple[CurrentRootStep, ...] = previous.CURRENT_ROOT_STEPS + (
    CurrentRootStep(
        "selection-policy-v0-81",
        "unittest",
        "tests.test_kuuos_self_organization_selection_policy_v0_81",
        True,
        "Selection policy is verified.",
    ),
    CurrentRootStep(
        "current-root-sequence-v0-81",
        "unittest",
        "tests.test_kuuos_current_root_sequence_v0_81",
        True,
        "Current root sequence v0.81 is verified.",
    ),
)
...
def sequence_issues() -> tuple[str, ...]:
    ...
    if SELECTION_POLICY_FRONTIER != "kuuos_self_organization_selection_policy_v0_81":
